...
class IFB():
    ...
    def __init__(self,server=None,client_key=None,client_secret=None, version=8.0):
        ...
        try:
            self.host = f"https://api.iformbuilder.com/exzact/api/v{str(self.version).replace('.','')}/{self.server}/"
    ...
    def requestAccessToken(self):
        ...
        """
        try:
            token_endpoint = f"{self.host}oauth/token"
            jwt_payload = {
                'iss': self.client_key,
                'aud': token_endpoint,
                'iat': time.time(),
