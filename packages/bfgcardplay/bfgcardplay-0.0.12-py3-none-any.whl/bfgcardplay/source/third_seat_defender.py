...
class ThirdSeatDefender(ThirdSeat):
    ...
    def _select_card_if_void(self, player: Player, trick: Trick) -> Card:
        ...
        self._record_void(player, trick)
        # Trump if appropriate
        if player.trump_suit:
            (value_0, value_1) = self._trick_card_values(trick, player.trump_suit)
            if player.trump_cards:
                unplayed_cards = player.total_unplayed_cards_by_suit(trick.suit)
    ...
    @staticmethod
    def _record_void(player: Player, trick: Trick):
        """Add void to partner's partners_voids."""
        manager = global_vars.manager
        partners_voids = manager.partners_voids[player.partner_seat]
        if trick.suit not in partners_voids:
            partners_voids.append(trick.suit)
