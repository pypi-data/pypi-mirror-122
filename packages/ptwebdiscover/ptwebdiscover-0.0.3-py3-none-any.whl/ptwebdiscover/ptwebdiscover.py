...
class ptwebdiscover():
    def __init__(self, args):
        ...
        self.args.charset           = ptmisclib.get_charset(args.charsets) if not args.charsets and not args.wordlist else ptmisclib.get_charset(["lowercase"])
    ...
    def run(self, args):
        ...
        for self.directory_finished, self.directory in enumerate(self.directories):
            ...
            ptmisclib.ptprint( ptmisclib.out_title_ifnot("Check " + self.domain_with_protocol + self.directory, self.args.json))
        ...
        if self.args.backups:
            ...
            self.prepare_backup()
            ptmisclib.clear_line_ifnot(condition = self.args.json)
            ptmisclib.ptprint( ptmisclib.out_title_ifnot("Search for backups", self.args.json))
        ...
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Finished in {ptmisclib.time2str(time.time()-self.start_time)} - discovered: {len(self.findings)} items", "INFO", self.args.json))
    ...
    def process_notvisited_urls(self):
        ...
        if self.args.parse:
            ptmisclib.clear_line_ifnot(condition = self.args.json)
            ptmisclib.ptprint( ptmisclib.out_title_ifnot("Checking not visited sources", self.args.json))
            while True:
    ...
    def search_for_backup_of_all(self, domain):
        ptmisclib.clear_line_ifnot(condition = self.args.json)
        ptmisclib.ptprint( ptmisclib.out_title_ifnot("Search for completed backups of the website", self.args.json))
        self.start_dict_time = time.time()
        self.counter = 0
    ...
    def output_result(self, findings, findings_details, technologies):
        ...
        if findings:
            ...
            ptmisclib.ptprint( ptmisclib.out_title_ifnot("Discovered sources", self.args.json))
            if self.args.tree:
            ...
            ptmisclib.clear_line_ifnot(condition=self.args.json)
        if technologies:
            ptmisclib.ptprint( ptmisclib.out_title_ifnot("Discovered technologies", self.args.json))
            ...
            if self.args.with_headers:
                for line_detail in line_list_details:
                    if line_detail[0] == line:
                        is_detail = True
                        ptmisclib.ptprint( ptmisclib.out_ifnot("[" + str(line_detail[1]) + "]  " + line + "\n", condition=self.args.json), end="")
                        if self.args.output:
                        ...
                        try:
                            for key, value in line_detail[2].items():
                                ...
                                ptmisclib.ptprint( ptmisclib.out_ifnot(" " * 7 + key + " : " + value, "ADDITIONS", condition=self.args.json, colortext=True))
                            break
                        except:
                            pass
                ptmisclib.ptprint( ptmisclib.out_ifnot("\n", condition=self.args.json))
            if not is_detail:
                ptmisclib.ptprint( ptmisclib.out_ifnot(line, condition=self.args.json))
                #TODO repaire JSON
                if self.args.json:
                    print(line)
                if self.args.output:
    ...
    def check_url_availability(self, url, proxies, headers, auth, method, position):
        ...
        if self.args.without_availability:
            return response
    ...
    def change_schema_when_redirect_from_http_to_https(self, response, old_extract):
        ...
        if old_extract.scheme == "http" and new_extract.scheme == "https" and old_extract.netloc == new_extract.netloc:
            ...
            ptmisclib.ptprint( ptmisclib.out_ifnot(f"Returned status code {response.status_code}. Site redirected to {target_location}. Check target in -u option.\n", "ERROR", self.args.json), end="\n", clear_to_eol=True)
    ...
    def print_configuration(self, args):
        ptmisclib.ptprint( ptmisclib.out_title_ifnot("Settings overview", self.args.json))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"URL................: {self.args.nochanged_url}", "INFO", self.args.json))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Discovery-type.....: Brute force", "INFO", self.args.json or args.wordlist or args.parse_only or args.backups_only))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Discovery-type.....: Complete backups only", "INFO", self.args.json or not args.backups_only))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Discovery-type.....: Dictionary", "INFO", self.args.json or not args.wordlist))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Discovery-type.....: Crawling", "INFO", self.args.json or not args.parse_only))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Wordlist...........: {str(args.wordlist)}", "INFO", self.args.json or not args.wordlist))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Extensions.........: {self.args.extensions}", "INFO", self.args.json))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Method.............: {self.args.method}", "INFO", self.args.json))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"String starts......: {self.args.begin_with}", "INFO", self.args.json or not self.args.begin_with))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Is in response.....: {self.args.string_in_response}", "INFO", self.args.json or not self.args.string_in_response))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Is not in response.: {self.args.string_not_in_response}", "INFO", self.args.json or not self.args.string_not_in_response))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Charset............: {''.join(self.args.charset)}", "INFO", self.args.json or args.wordlist or args.parse_only))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Length-min.........: {self.args.length_min}", "INFO", self.args.json or args.parse_only))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Length-max.........: {self.args.length_max}", "INFO", self.args.json or args.parse_only))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Keyspace...........: {self.keyspace}", "INFO", self.args.json or args.parse_only))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Delay..............: {self.args.delay}s", "INFO", self.args.json))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Threads............: {self.args.threads}", "INFO", self.args.json))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Recurse............: {self.args.recurse}", "INFO", self.args.json))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Parse content......: {self.args.parse}", "INFO", self.args.json))
        ptmisclib.ptprint( ptmisclib.out_ifnot(f"Search for backups.: {self.args.backups}", "INFO", self.args.json))
