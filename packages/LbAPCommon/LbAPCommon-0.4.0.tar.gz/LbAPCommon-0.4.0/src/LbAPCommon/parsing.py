    ...
    for job_name, job_data in jobs_data.items():
        ...
        if "bk_query" in job_data["input"]:
            if job_data["input"]["bk_query"].lower() in bk_query_to_job:
                ...
                bk_query_to_job[
                    job_data["input"]["bk_query"].lower()
                ] = job_name.lower()

    for k, v in bk_query_to_job.items():
        if ("up" in v and "up" not in k) or ("down" in v and "down" not in k):
            raise ValueError(
                f"The magnet polarity specified in the job name ({v}) does not match with its input: {k}"
            )
    ...
    for query in bk_query_to_job:
        ...
        if query_polarity:
            ...
            if not both_polarities:
                warnings.append(
                    f"{query} has been requested as input for one job"
                    " but its opposite polarity counterpart has not been requested for any jobs."
                    " Are you sure you do not want the other polarity in another job?"
                )
