...
class PyocsLogin:
    # 全局信息
    __Login_Address = "https://ocs-api.gz.cvte.cn/users/login"  # OCS登陆站点
    __OCS_Test_URL = "http://ocs-api.gz.cvte.cn/tv"  # 测试用站点
    Access_URL = ['http://ocs-api.gz.cvte.cn/tv', 'https://ocs-api.gz.cvte.cn/tv']  # 任何你想要访问的OCS站点
    _instance = None
    _cookies_file = ""
    ...
    def get_ocs_login_cookies(account, passwd):
        _get_ocs_login_cookies_url = "https://faas.gz.cvte.cn/function/get-ocs-login-cookies"
        headers = {
            "account": account,
            "passwd": passwd
        }
        req = requests.get(_get_ocs_login_cookies_url, headers=headers)
        cookies_dict = json.loads(req.text)
