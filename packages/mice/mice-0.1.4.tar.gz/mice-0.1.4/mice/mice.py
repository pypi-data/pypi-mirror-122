...
class MICE:
    ...
    _check_dropping(opt_ml)
        Checks whether or not to drop the last iteration from the hierarchy

    _check_max_cost(extra_eval=0)
        Checks if maximum cost is exceeded

    _check_restart(opt_ml)
        Checks whether or not to restart the hierarchy.

    _check_samp_sizes(opt_ml)
        Checks if the current sample sizes are larger than the optimal sample
        sizes computed.

    _compute_bias()
        Compute an approximation of the expected square norm of the bias.
    """
    ...
    def evaluate(self, x):
        ...
        if self._check_max_cost(extra_eval=self.m_min):
        ...
        while not self._check_samp_sizes(opt_ml):
            for delta, m_opt in zip(self.deltas, opt_ml):
                ...
                if self._check_max_cost(extra_eval=m_to_sample):
                    return np.full(self.dim, np.nan)
        ...
        #                  self.norm(f_estim), len(self.deltas)]
        self._update_log()
    ...
    def _check_max_cost(self, extra_eval=0):
        ...
        if self.counter + extra_eval > self.max_cost:
            self.terminate = True
            # self.log_list[-1] = ['end', self.counter, None, None, None]
            # self.log_list = pd.DataFrame(self.log_list, columns=['event', 'num_grads',
            #                                            'vl', 'bias_rel_err',
            #                                            'grad_norm'])
