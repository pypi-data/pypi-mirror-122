...
def _run_multiple_products(crop_residue_products: list, crop_products: list):
    def run_product(product: dict):
        term_id = product.get('term', {}).get('@id')
        values = [_multiple_crop_product_values(p, term_id) for p in crop_products]
...
def _should_run(cycle: dict):
    ...
    return should_run, crop_residue_products, single_crop, crop_products


def run(cycle: dict):
    should_run, crop_residue_products, single_crop, crop_products = _should_run(cycle)
    return (
        _run_single_product(crop_residue_products, single_crop) if single_crop else
        _run_multiple_products(crop_residue_products, crop_products)
    ) if should_run else []
