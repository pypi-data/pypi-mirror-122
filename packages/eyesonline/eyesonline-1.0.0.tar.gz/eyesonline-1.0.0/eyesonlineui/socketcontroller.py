...
class socketController():
	...
	def connect(self,homeurl, creds):
		...
		r = session.post(homeurl+'/login', creds, verify=False)
		if r.status_code == 200:
			token = 'remember_token='
			token += session.cookies['remember_token']
			token += '; session='
			token += session.cookies['session']
			self.sio.connect(homeurl,headers={'Cookie': token}, namespaces=['/classroom'])
			return self.sio
		else:
	...
	def joinRoom(self,room,**kwargs):
		self.activeroom = room
		data = {'room':room}
		data['widgets'] = self.monitorWidgets
		data.update(kwargs)
		self.sio.emit('join',data,namespace='/classroom')
