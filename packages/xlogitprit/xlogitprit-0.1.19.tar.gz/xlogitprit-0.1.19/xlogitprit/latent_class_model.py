...
class LatentClassModel(MultinomialLogit):
    ...
    def _get_member_X_idx(self, class_num):
        ...
        if self.fit_intercept:  # code smell
    ...
    def _expectation_maximisation_algorithm(self, X, y, avail=None, weights=None,
        ...
        if class_thetas is None:
            # note using beta length of 0th class as a proxy
            class_thetas = np.stack([np.repeat(.0, self._get_betas_length(0)) for _ in range(1, self.num_classes)], axis=0)  # class member ship probability
