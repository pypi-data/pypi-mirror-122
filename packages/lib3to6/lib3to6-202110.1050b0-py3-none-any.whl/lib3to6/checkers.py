            ...
            for alias in node.names:
                if alias.name == '*':
                    raise common.CheckError(
                        f'Prohibited from {node.module} import *.', node)
        ...
        for node in ast.walk(tree):
            ...
            for keyword in node.keywords:
                if keyword.arg in PROHIBITED_OPEN_ARGUMENTS:
                    msg = (
                        f"Prohibited keyword argument '{keyword.arg}' to builtin.open."
                        )
                    raise common.CheckError(msg, node)
                if keyword.arg != 'mode':
                    continue
...
class NoAsyncAwait(cb.CheckerBase):
    ...
    def __call__(self, ctx: common.BuildContext, tree: ast.Module) ->None:
        ...
        for node in ast.walk(tree):
            ...
            msg = (
                f"Prohibited use of '{keywords}', which is not supported for target_version='{ctx.cfg.target_version}'."
                )
            raise common.CheckError(msg, node)
    ...
    def __call__(self, ctx: common.BuildContext, tree: ast.Module) ->None:
        for node in ast.walk(tree):
            if isinstance(node, ast.YieldFrom):
                msg = (
                    f"Prohibited use of 'yield from', which is not supported for your target_version={ctx.cfg.target_version}"
                    )
                raise common.CheckError(msg, node)
