...
def parse_module_header(module_source: typ.Union[bytes, str],
    target_version: str) ->ModuleHeader:
    shebang = False
    coding = None
    line: str
...
def get_available_classes(module: object, clazz: CheckerOrFixer) ->typ.Dict[
    str, CheckerOrFixer]:
...
def get_selected_names(names: FuzzyNames, available_names: typ.Set[str]
    ) ->typ.List[str]:
...
def iter_fuzzy_selected_checkers(names: FuzzyNames) ->typ.Iterable[cb.
    CheckerBase]:
...
def parse_imports(tree: ast.Module) ->typ.Tuple[int, int, typ.Set[common.
    ImportDecl]]:
...
def add_required_imports(tree: ast.Module, required_imports: typ.Set[common
    .ImportDecl]) ->None:
...
def transpile_module_data(ctx: common.BuildContext, module_source_data: bytes
    ) ->bytes:
    target_version = ctx.cfg.target_version
