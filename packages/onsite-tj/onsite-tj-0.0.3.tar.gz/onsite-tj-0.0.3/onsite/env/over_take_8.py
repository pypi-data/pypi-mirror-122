...
class OverTake(EnvBase):
    ...
    def __init__(self,sut=None,a_bound=5):
        ...
        self.l = 3  # 车辆轴距，单位：m
        self.dt = 0.1
        self.background_control_high = IDM(exv=22,a_bound=a_bound) # 背景车加速度！！
        self.background_control_low = IDM(exv=11,a_bound=a_bound)
        self.metric = 'danger'
        self.curve = None
        self.curve_tag = None
        ...
        if sut == None:
            ...
            self.sut = sut
        print(
            'overtake场景，输入参数v1, v3, dif_v3_v4, dif_v1_v0, dif_x1_x0, dif_x3_x4, dif_x0_x4, dif_y0_y4')
    ...
    def init(self, scenario):
        ...
        straight_curve = np.concatenate(
            (np.linspace(0, 400, 150).reshape(-1, 1),
    ...
    def update(self, action):
        ...
        a4, rot4 = self.background_control_high.deside_all(state_4)
        state_3 = self.get_state('rf')
        a3, rot3 = self.background_control_high.deside_all(state_3)
        state_1 = self.get_state('f')
        a1, rot1 = self.background_control_low.deside_all(state_1)
    ...
    def judge(self):
        ...
        if self.metric == "danger":
            ...
            self.done[self.time_count > 20] = 1
    ...
    def update_curve(self):
        ...
        # 初始化tag_back,
        tag_lane = np.zeros((sample_shape,)).astype(np.int)
        tag_lane[np.abs(self.vehicle_array[:,0,1] - self.vehicle_array[:,2,1])<0.1] = 1
        dis_back = self.vehicle_array[:,0,0] - self.vehicle_array[:,1,0]

        delta_v = -(self.vehicle_array[:, 0, 2] - self.vehicle_array[:, 1, 2])
        beta_1 = np.clip(delta_v, 0, 1e5).reshape(-1, 1)
        beta_2 = np.clip(delta_v, -1e5, 0).reshape(-1, 1)
        beta_3 = self.vehicle_array[:, 1, 2].reshape(-1, 1)
        beta = np.concatenate((beta_0, beta_1, beta_2, beta_3), axis=1)
        dis_back_cap = np.multiply(beta, self.x_back).sum(axis=1)

        tag_curve = (dis_back > dis_back_cap) & (tag_lane == 1)
        if tag_curve.sum() > 0:
            bezier = pd.DataFrame(-self.vehicle_array[tag_curve, 2, 1])
            curve = np.array(list(map(self.BezierCurve, bezier[0])))
            curve = curve.reshape(tag_curve.sum(), -1, 2)
...
if __name__ == "__main__":
    ...
    env = ENV()
    # 场景参数定义在此处
    data = pd.read_csv("data/over_take_created.csv")
