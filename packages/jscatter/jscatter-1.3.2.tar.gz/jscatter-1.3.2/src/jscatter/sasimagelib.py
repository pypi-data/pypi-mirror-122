...
class sasImage(SubArray, np.ma.MaskedArray):
    ...
    def lineFindCenter(self, minmax=[2, 50], select='COM', show=False):
        """
        ...
        select : string, default 'com'
            Select center of mass ('com') of the peak or center of Gaussian fit (any other).
        show : bool
        ...
        """
        # take average
        imageav = dA(np.c_[np.r_[0:self.shape[1]], self.mean(axis=0), self.std(axis=0)].T)
        ...
        if select.lower() == 'com':
            ...
            we = 4
            pb.setlimit(A=[0], sigma=[0, 5*we])
            pb.fit(_gauss, {'mean': pb.X[pb.Y.argmax()], 'sigma': we, 'bgr': 0, 'A': 2*we*pb.Y.max()},
                            {}, {'x': 'X'}, output=show)
    ...
    def lineAverage(self, sigma=3, minmax=[2, 50], isdark=False):
        """
        Line average of image and conversion to wavevector q for line collimation cameras.

        Remember to set .detector_distance to calibrated value.
        A spike filter is applied to remove cosmic ray peaks.

        Parameters
        ----------
        sigma : float, default 3
            Singular peaks (from spikes in time) are filtered out if spike counts
             are ``abs(count - mean) > sigma * std``.
             If 0 no filter is applied.
        minmax : [int,int], default [2,50]
            Interval for determination of center in lineFindCenter in pixel units.
        isdark : bool
            For dark current measurements only do average.
             - No lineFindCenter (there is no primary beam), set from other masurement.
             - Q scale is virtual but calculated.
        ...
        Notes
        -----
        - detector_distance in attributes is used.
        - The primary beam is automatically detected using :func:`~lineFindCenter` if center is None.
        - Correction for flat detector projected to Ewald sphere included.

        For Kratky camera setups as SAXSpace from Anton Paar
        each column in the image corresponds to a timeframe (e.g. 10s counting) that is averaged over a pixel line.
        ...
        ::
         ...
         image = js.sas.sasImage('ADHc44mg_T10_305mm_360Frames.tif')
         image.lineFindCenter()

         p=js.grace()

         data = image.lineAverage()
         p.plot(data)

         step = 10
         for t in np.r_[:image.shape[0]:step]:
             data = image[t:t+step].lineAverage()
             p.plot(data)


        """
        if (not hasattr(self, 'center') or not isinstance(self.center, numbers.Real)) and not isdark:
            self.lineFindCenter(minmax=minmax)

        if sigma>0:
            # filter single peaks
            # line images contain a time series that has singular peaks from cosmic rays
            peaks = np.abs(self - self.mean(axis=0)) > (sigma * self.std(axis=0))
            if 0:
                # in newer numpy version this simplifies to this
                average = np.mean(self, axis=0, where=~peaks)
                std = np.std(self, axis=0, where=~peaks)
            else:
                image = self.copy()
                image[peaks] = 0
    ...
    def recalibrateDetDistance(self, center=None, number=500, fcenter=1., fwhm=0.1, peaks=None, showfits=False):
        ...
        - To check the result use showfits=True.
          For bad result try to change the detectordistance to a more reasonable value.
