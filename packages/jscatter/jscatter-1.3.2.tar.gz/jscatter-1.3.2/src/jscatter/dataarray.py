...
class dataArrayBase(np.ndarray):
    ...
    def concatenate(self, others, axis=1, isort=None, missing=None):
        ...
        for a in attribs:
            if missing is None:
                setattr(data, a, [getattr(one, a, None) for one in [self] + others])
            elif missing in ['Error', 'error', 'e']:
                # raise AttributeError if missing
                setattr(data, a, [getattr(one, a) for one in [self] + others])
                ...
                # drop
                setattr(data, a, [getattr(one, a) for one in [self] + others if hasattr(one, a)])
