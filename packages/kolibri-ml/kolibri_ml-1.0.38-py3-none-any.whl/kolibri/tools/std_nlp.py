...
class StdNLP(Component):
    ...
    defaults = {
        # name of the language model_type to load - if it is not set
        # we will be looking for a language model_type that is named
        # after the language of the model_type, e.g. `en`
        "model": None,
    ...
    def create(cls, cfg):
        ...
        # if no model_type is specified, we fall back to the language string
        if not nlp_model_name:
            nlp_model_name = cfg.language
            component_conf["model"] = cfg.language

        logger.info("Trying to load nlp model_type with "
    ...
    def cache_key(cls, model_metadata):
        ...
        # as the model_type name if no explicit name is defined
    ...
    def ensure_proper_language_model(nlp):
        ...
        """Checks if the language model_type is properly loaded.

        Raises an exception if the model_type is invalid."""

        if nlp is None:
            raise Exception("Failed to load the language model_type. "
                            "Loading the model_type returned 'None'.")
