...
class TextIndexer(BaseIndexer):
    ...
    def doc2bow(self, doc, allow_update=False, return_missing=False):
        ...
        if allow_update or return_missing:
            missing = sorted(x for x in iteritems(counter) if x[0] not in self.vocab2idx)
            if allow_update:
                for w, _ in missing:
                    # new id = number of ids made so far;
                    # NOTE this assumes there are no gaps in the id sequence!
                    self.vocab2idx[w] = len(self.vocab2idx)
        result = {self.vocab2idx[w]: freq for w, freq in iteritems(counter) if w in self.vocab2idx}
