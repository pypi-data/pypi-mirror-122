...
from .utils import get_file_ext, active_obj, path
...
def _get_page(type, index):
    try:
        ...
        raise ValueError('Invalid page type') from e

def _get_sheet(index, type):
    bk = _get_page(type, index)
    return bk.Layers(0) if bk else None

def get_page(type, index=0) -> Union[WBook, MBook, GPage]:
    """get WBook, MBook, GPage by index"""
...
def find_graph(name='') -> GPage:
    ...
    if isinstance(name, int):
        page = _get_page(_GPAGE_TYPE, name)
    else:
        if len(name)==0:
...
def pages(type_='') -> Union[WBook, MBook, GPage]:
    """
