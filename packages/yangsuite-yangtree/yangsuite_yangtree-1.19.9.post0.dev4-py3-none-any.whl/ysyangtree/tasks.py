...
class TaskHandler(object):
                ...
                if f.endswith('.tsk'):
                    ...
                    with open(os.path.join(path, f)) as fd:
                        try:
                            ...
                            cat, rep_name = '', ''
                            cat = set_default('', cat_rep['category'], '')
                            rep_name = set_default('', cat_rep['name'], '')
                            if cat in tasks:
                                if (not any(rep_name in x for x
                                            in tasks[cat])):
                                    tasks[cat].append([rep_name, f])
    ...
    def save_replay_yaml(self, task, yaml_file_path=''):
        ...
        try:
            ...
            if segments:
                for counter, segment in enumerate(segments, 1):
                    test_section = {}
                    action_name = 'apply_yang_content_' + str(counter)
                    test_section[action_name] = []
                    # TODO returns block with get.
                    yang = {
                        'yang': {
                            'device': 'uut',
                            'connection': 'netconf',
                    ...
                    test_section[action_name].append(yang)
            ...
            if yaml_file_path:
                ...
                with open(yaml_file_path, mode="w+") as file:
                    yaml.safe_dump(task_string, file, default_flow_style=False,
                                   sort_keys=False)
            else:
                return task_string
        except Exception as e:
    ...
    for i, seg in enumerate(task['segments']):
        ...
        try:
            ...
            if yang:
                ...
                operation = set_default(yang, 'proto-op', 'custom')
                if operation == 'custom':
