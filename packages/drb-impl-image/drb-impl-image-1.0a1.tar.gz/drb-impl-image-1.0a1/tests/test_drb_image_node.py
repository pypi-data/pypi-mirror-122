...
class TestDrbTifNode(unittest.TestCase):
    ...
    def test_format_name(self):
        self.assertTrue(
            DrbImageFactory().valid(str(self.image_tif_one)))
        ...
        node_value = root_node.get_named_child(DrbImageNodesValueNames.FORMAT
                                               .value, occurrence=1)
    ...
    def test_shape(self):
        ...
        root_node = node.get_named_child(IMAGE)[0]

        node_value = root_node.get_named_child(DrbImageNodesValueNames.WIDTH
                                               .value, occurrence=1)
        self.assertIsNotNone(node_value)
        self.assertEqual(node_value.value, 248)

        list_node_value = root_node.get_named_child(DrbImageNodesValueNames
                                                    .HEIGHT.value)
        self.assertIsNotNone(list_node_value)

        node_value = list_node_value[0]
    ...
    def test_type_band_count(self):
        ...
        node_value = root_node.get_named_child(DrbImageNodesValueNames.TYPE
                                               .value, occurrence=1)
        self.assertIsNotNone(node_value)
        self.assertEqual(node_value.value, 'uint8')

        node_value = root_node.get_named_child(DrbImageNodesValueNames
                                               .NUM_BANDS.value, occurrence=1)
        self.assertIsNotNone(node_value)
    ...
    def test_image_remove_add_attributes(self):
        node = self.open_node(str(self.image_tif_one))

        root_node = node.get_first_child()

        with self.assertRaises(NotImplementedError):
            root_node.add_attribute("toto")
        with self.assertRaises(NotImplementedError):
            root_node.remove_attribute("toto")
        root_node.close()
    ...
    def test_image_parent(self):
        ...
        level1 = root_node.get_first_child()
    ...
    def test_image_get_children(self):
        node = self.open_node(str(self.image_tif_one))
        ...
        with self.assertRaises(DrbException):
            root_node.get_named_child(DrbImageNodesValueNames.FORMAT.value,
                                      occurrence=2)
        with self.assertRaises(DrbException):
            root_node.get_named_child('toto', occurrence=1)
        root_node.close()

        self.assertIsNotNone(root_node.get_last_child())

    def test_image_get_children_at(self):
        node = self.open_node(str(self.image_tif_one))

        root_node = node.get_first_child()

        first_val = root_node.get_child_at(0)
        self.assertIsNotNone(first_val)
        self.assertEqual(first_val.name, DrbImageNodesValueNames.FORMAT.value)

        with self.assertRaises(DrbException):
            root_node.get_child_at(15)
        with self.assertRaises(DrbException):
            root_node.get_child_at(-10)
        root_node.close()
    ...
    def test_image_with_crs(self):
        ...
        crs_node = root_node.get_named_child(DrbImageNodesValueNames.CRS.value,
                                             occurrence=1)
    ...
    def test_image_3_bands(self):
        ...
        num_bands_node = root_node.get_named_child(
            DrbImageNodesValueNames.NUM_BANDS.value,
            occurrence=1)
    ...
    def test_image_unknown_tags(self):
        ...
        format1_node = root_node.get_named_child('Format1', occurrence=1)
