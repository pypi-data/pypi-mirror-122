...
class TestDrbNodeFactoryNetcdf(unittest.TestCase):
    ...
    def test_Valid(self):
        self.assertTrue(DrbImageFactory().valid(
            str(self.image_tif_one)))
        self.assertTrue(DrbImageFactory().valid(
            str(self.image_png)))
        self.assertTrue(DrbImageFactory().valid(
            str(self.image_jp2)))
    ...
    def test_base_node_get_named_child(self):
        node = self.open_node(str(self.image_tif_one))

        child_img = node.get_named_child(IMAGE, occurrence=1)
        self.assertIsInstance(child_img, DrbImageNode)
        list_children = node.get_named_child(IMAGE)
        self.assertIsNotNone(list_children)
        self.assertEqual(list_children[0], child_img)
    ...
    def test_base_node_get_child(self):
        ...
        self.assertEqual(node.get_child_at(0), node.get_first_child())
        self.assertEqual(node.get_first_child(), node.get_last_child())
    ...
    def test_base_node_attribute(self):
        ...
        with self.assertRaises(NotImplementedError):
            node.add_attribute(DrbFileAttributeNames.DIRECTORY.value)
