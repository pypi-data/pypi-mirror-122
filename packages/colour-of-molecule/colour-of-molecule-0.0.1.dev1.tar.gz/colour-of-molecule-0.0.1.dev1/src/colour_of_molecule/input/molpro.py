...
class EomReader(MainReader):
    ...
    def __call__(self, log_file_path):
        ...
        def homo_lumo_for_MO(dct, homo, mos):
            ...
            if not isinstance(mos, list) and len(mos) is not 2:
...
class MrciReader(MainReader):
    ...
    def __call__(self, log_file_path):
        ...
        for transition in state_transitions.values():
            # print("TRANS: ", transition.num_initial, "->", transition.num_final, transition.energy.in_units("nm"), "\t",
...
class Rs3Reader(MainReader):
    def __call__(self, log_file_path):
        print("ERROR:   Rs3 type is not implemented.")
        pass
    pass
    ...
    class MolproSubtype:
        def __get__(self, instance, owner):
            def check_tuple_re_func(tup):
                ...
                if not isinstance(tup, tuple) and len(tup) is not 2:
