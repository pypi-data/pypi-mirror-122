...
def find_closest_time(prompt_time, subset_time_list, min_before):
...
def match_feature(prompt_local_datetime_series, df_logs_combined):
    ...
    for idx in prompt_local_datetime_series.index:
        ...
        if df_logs_combined.shape[0] == 0:
            matched_mims_summary_list = ["NF"] * len(min_before_list)
            matched_readings_list = ["NF"] * len(min_before_list)
            matched_start_time_list = ["NF"] * len(min_before_list)
        else:
