...
def cn_admin_change_tx_status(request, tx_id):
    ...
    if not tx:
        message_service.post_error("Transaction not found")
        return HttpResponseForbidden() if is_ajax else redirect("cashnet:transaction_index")

    has_issues = False
    status_code = request.POST.get('status')
    if status_code and status_code in tx.status_options:
    ...
    if is_ajax:
        return HttpResponseForbidden() if has_issues else HttpResponse('success')
    else:
        return redirect("cashnet:transaction_detail", tx.id)
