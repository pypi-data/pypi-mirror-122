...
class Binomial(Distribution):
    ...
    def pdf(self, k):
        ...
        binomial_coeff = math.factorial(self.n) / (math.factorial(self.n - k) * math.factorial(k))
        p_part = (self.p ** k) * (1 - self.p) ** (self.n - k)

        return binomial_coeff * p_part
    ...
    def cdf(self, k):
        ...
        Returns:
            float: cumulative probability - the sum of probability from 0 to floor(k)
        """

        try:
            assert k <= self.n, 'K is not less than or equal to size {}'.format(self.n)
        ...
        i = 0
        while i <= floor_k:
            cumulative_probability += self.pdf(i)
            i += 1
