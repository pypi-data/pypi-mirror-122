...
class CheckerRegistry():
    ...
    def get_checkers(self):
        if not self.checkers:
            self.checkers = {key: checker_class() for key, checker_class in self.checkerClasses.items()}
        return self.checkers


    def get_desired_state(self):
        self.desired_states = {key: checker.force_state() for key, checker in self.get_checkers().items()}
