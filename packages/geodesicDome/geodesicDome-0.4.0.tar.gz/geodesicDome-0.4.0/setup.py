...
``GeodesicDome(freq=0)``
   ...
   Args: freq (int, optional): The frequency of the geodesic dome.
   Defaults to 0.
   ...
   ``find_neighbours(self, index: numpy.int64, depth=1) ‑> numpy.ndarray``
      Finds the neighbours of a given vertex on the geodesic dome to a
      certain depth (defaults to 1 if not provided)

      Args: index (np.int64): The index of the vertex to search from
      depth (int, optional): The depth of neighbours to return. Defaults
      to 1.

      Returns: np.ndarray: An array containing the indices of all the
      vertex’s neighbours
   ...
   ``tessellate(self, freq=1) ‑> None``
      Tessellates the geodesic dome a given number of times (tessellates
      once if no arguments provided)

      Args: freq (int, optional): The number of times to tessellate.
      Defaults to 1.""",
