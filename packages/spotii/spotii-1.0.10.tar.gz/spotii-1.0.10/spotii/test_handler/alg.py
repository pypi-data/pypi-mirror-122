...
##MAX_STDD = 25
    ...
    def calculation(self, image):
        ...
        reduce = 15
        weak_height = calibration.IDEAL_BAR_HEIGHT -reduce*2+10
        weak_row_start = calibration.FINAL_BAR_ROW + (reduce -8)        
...
##        second_blc=self.balance(clh,198)
##        return [blc ,clh, second_blc]
        ...
        mn_list, std_list = self. av_mean_draw(block, sample_width, av)
...
class Detection(Algorithm):
    def calculation(self,img):
        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
        #Red color rangle  169, 100, 100 , 189, 255, 255
        lower_range = np.array([self.v[0],self.v[1],self.v[2]])
        upper_range = np.array([self.v[3],self.v[4],self.v[5]])
        mask = cv2.inRange(hsv, lower_range, upper_range)
        return [mask]
...
detection = Detection('detection',[126,92,246,176,92,246])
