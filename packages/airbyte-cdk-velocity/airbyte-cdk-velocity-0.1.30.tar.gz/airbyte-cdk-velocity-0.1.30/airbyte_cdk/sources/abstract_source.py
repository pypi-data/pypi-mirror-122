...
class AbstractSource(Source, ABC):
    ...
    def _read_incremental(
        ...
        for slice in slices:
            ...
            logger.info(f"records number: {list(records)}")

            logger.info(f"records first: {list(records)[0]}")
            logger.info(f"records status: {not list(records)}")


            if not list(records):
                logger.info(f"came inside: {type(records)}")
                stream_state = stream_instance.get_updated_state_empty(stream_state, slice)
