...
def finalize(site):
    ...
    site.taxonomies = _TAXONOMIES


def register_plugin():
    signals.site_init.connect(init_taxonomies)
    signals.readers_finished.connect(add_pages)
    signals.processors_init.connect(finalize)
