...
class MidiHandler:	# Quisk calls this to make the Midi handler instance.
  ...
  def __init__(self, app, conf):
    ...
    self.controls = {}		# Control object and associated function for each name
    self.controls["Vol"] =	(self.app.sliderVol,	self.app.ChangeVolume)
    self.controls["STo"] =	(self.app.sliderSto,	self.app.ChangeSidetone)
    self.controls["Rit"] =	(self.app.ritScale,	self.app.OnRitScale)
    self.controls["Ys"] =	(self.app.sliderYs,	self.app.ChangeYscale)
    self.controls["Yz"] =	(self.app.sliderYz,	self.app.ChangeYzero)
    self.controls["Zo"] =	(self.app.sliderZo,	self.app.OnChangeZoom)
  def OnReadMIDI(self, byts):	# Quisk calls this for any Midi bytes received.
    for byt in byts:
      ...
      if len(self.midi_message) == 3:
        #print ("0x%2X  %d  %d" % tuple(self.midi_message))
        ...
        if status == 0x90:	# Note On
          ...
          try:
            name = self.app.local_conf.MidiNoteDict[str(self.midi_message[1])]
          except:
            pass
          else:
  ...
  def ControlKnob(self, name):
    ...
    if name in ("Vol", "STo", "Rit", "Ys", "Yz", "Zo", ):
      ctrl, func = self.controls[name]
      ...
      pass #print ("Unknown knob name", name)
