...
class MidiKnob(QuiskControl):
  ...
  def MakeMenu(self):
    self.menu = wx.Menu()
    for name in ("Vol", "STo", "Rit", "Ys", "Yz", "Zo", "Tune"):
...
class RadioSection(BaseWindow):		# The pages for each section in the second-level notebook for each radio
  ...
  def MakeControls(self):
    ...
    if self.section == "Keys":
      ...
      'These MIDI devices generate note on/off messages with a note number that depends on the button or key. '\
      'They generate a control number and value for knobs and jog wheels. '\
      'You can assign the number to a Quisk button or slider. '\
      'Look on the Config/Status screen to see the note or control number received by Quisk. '\
      'Then enter the number here and press "Enter".'
      ...
      ctrl = self.AddPopupMenuHelp(MidiButton, 2, "Assign Midi note to", "Button", hlp)
      ctrl.handler = self.OnMidiMenu
      self.NextRow()
      hlp = \
      'A "knob" is a Midi control that rotates left and right one turn. It sends a value from 0 to 127. '\
      'Select a row and then choose a Quisk control to assign to the knob.'
      ...
      'Select a row and then choose a Quisk control to assign to the jog wheel.'
      ...
      self.Bind(wx.EVT_LIST_ITEM_SELECTED, self.OnMidiListSelected)
      self.NextRow()
  ...
  def OnMidiMenu(self, ctrl, jog_tag=None):
    ...
    if index < 0:
      return
    if jog_tag is None:		# Change to a new name possibly including a jog tag
      text = ctrl.value
    else:		# Change the jog tag on the current item
