...
from gemican import __version__
...
def main():
    parser = argparse.ArgumentParser(
        description="A kickstarter for Gemican",
...
needed by Gemican.
    ...
    try:
        with open(os.path.join(CONF['basedir'], 'gemicanconf.py'),
                  'w', encoding='utf-8') as fd:
            conf_python = dict()
            for key, value in CONF.items():
                conf_python[key] = repr(value)

            _template = _jinja_env.get_template('gemicanconf.py.jinja2')
