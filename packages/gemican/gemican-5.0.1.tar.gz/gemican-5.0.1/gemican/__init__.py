...
# Combines all paths to `gemican` package accessible from `sys.path`
# Makes it possible to install `gemican` and namespace plugins into different
...
# gemican.log has to be the first gemican module to be loaded
# because logging.setLoggerClass has to be called before logging.getLogger
from gemican.log import init as init_logging
from gemican.generators import (ArticlesGenerator,  # noqa: I100
                                PagesGenerator, SourceFileGenerator,
                                StaticGenerator, TemplatePagesGenerator)
from gemican.plugins import signals
from gemican.plugins._utils import get_plugin_name, load_plugins
from gemican.readers import Readers
from gemican.server import GeminiServer
from gemican.settings import coerce_overrides, read_settings
from gemican.utils import (FileSystemWatcher, clean_output_dir, maybe_pluralize)
from gemican.writers import Writer

try:
    __version__ = __import__('pkg_resources') \
        .get_distribution('gemican').version
except Exception:
    __version__ = "unknown"

DEFAULT_CONFIG_NAME = 'gemicanconf.py'
...
def parse_arguments(argv=None):
    ...
    parser.add_argument('--version', action='version', version=__version__,
                        help='Print the gemican version and exit.')

    parser.add_argument('-r', '--autoreload', dest='autoreload',
                        action='store_true',
                        help='Relaunch gemican each time a modification occurs'
...
def autoreload(args, excqueue=None):
    ...
    gemican, settings = get_instance(args)
...
def main(argv=None):
    ...
    logger.debug('Gemican version: %s', __version__)
