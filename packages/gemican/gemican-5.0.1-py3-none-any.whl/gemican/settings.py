...
from gemican.log import LimitFilter
...
def handle_deprecated_settings(settings):
    ...
    for old, new, doc in [
            ...
            ('FILES_TO_COPY', 'STATIC_PATHS and EXTRA_PATH_METADATA',
                'https://github.com/getgemican/gemican/'
    ...
    url_settings_url = \
        'http://docs.getgemican.com/en/latest/settings.html#url-settings'
...
def configure_settings(settings):
    ...
    if 'PATH' not in settings or not os.path.isdir(settings['PATH']):
        raise Exception('You need to specify a path containing the content'
                        ' (see gemican --help for more information)')
    ...
    # lookup the theme in "gemican/themes" if the given one doesn't exist
    ...
    if 'TIMEZONE' not in settings:
        logger.warning(
            ...
            'https://docs.getgemican.com/en/latest/settings.html#TIMEZONE '
            'for more information')

    # fix up pagination rules
    from gemican.paginator import PaginationRule
    pagination_rules = [
