...
from gemican import Gemican
from gemican.generators import StaticGenerator
from gemican.settings import read_settings
from gemican.tests.support import (LoggedTestCase, locale_available,
                                   mute, unittest)
...
SAMPLE_CONFIG = os.path.join(SAMPLES_PATH, "gemican.conf.py")
SAMPLE_FR_CONFIG = os.path.join(SAMPLES_PATH, "gemican.conf_FR.py")
...
class TestGemican(LoggedTestCase):
    # general functional testing for gemican. Basically, this test case tries
    # to run gemican in different situations and see how it behaves

    def setUp(self):
        super().setUp()
        self.temp_path = mkdtemp(prefix='gemicantests.')
        self.temp_cache = mkdtemp(prefix='gemican_cache.')
        self.maxDiff = None
    ...
    def test_order_of_generators(self):
        ...
        gemican = Gemican(settings=read_settings(path=None))
        generator_classes = gemican._get_generator_classes()
    ...
    def test_basic_generation_works(self):
        # when running gemican without settings, it should pick up the default
        ...
        gemican = Gemican(settings=settings)
        mute(True)(gemican.run)()
    ...
    def test_module_load(self):
        """Test loading via python -m gemican --help displays the help"""
        output = subprocess.check_output([
            sys.executable, '-m', 'gemican', '--help'
