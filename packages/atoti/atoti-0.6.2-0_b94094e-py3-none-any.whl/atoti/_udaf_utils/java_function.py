...
class JavaFunction(ABC):
    @abstractmethod
    def add_method_source_codes(self, method_codes: Set[str]):
        """Adds the source code of methods to be declared in the compiled Java class."""
        ...

    @abstractmethod
    def update_class_imports(self, additional_imports: Set[str]):
    ...
    def get_output_type_function(
        self,
    ) -> Callable[[List[JavaOperationElement], JavaApi], DataType]:
...
class CustomJavaFunction(JavaFunction):
    ...
    def get_output_type_function(self):
        def output_type(_ignored: List[JavaOperationElement], _java_api: JavaApi):
...
class ExistingJavaFunction(JavaFunction):
    ...
    def _get_signature_from_java(
        ...
        java_signatures: Any = java_api.gateway.jvm.io.atoti.udaf.util.AtotiUdafCompiler.getMethodSignatures(full_class, method_name)  # type: ignore
    ...
    def _get_output_type_function(
        ...
        def output_type_function(
            operation_elements: List[JavaOperationElement], java_api: JavaApi
        ) -> DataType:
            java_types = [
...
def is_signature_valid(
    signatures: Sequence[Sequence[DataType]],
    operation_elements: Sequence[JavaOperationElement],
) -> bool:
...
def get_output_type_from_java(
    *, java_class: str, java_method: str, types: List[str], java_api: JavaApi
) -> DataType:
    java_types = ListConverter().convert(types, java_api.gateway._gateway_client)
    output_type: str = java_api.gateway.jvm.io.atoti.udaf.util.AtotiUdafCompiler.getMethodOutputType(java_class, java_method, java_types)  # type: ignore
