...
from terality_serde.json_encoding import (
    ArrayEncoding,
    decode_json_indices_inplace,
    decode_json_columns_inplace,
)
...
def _download_ndarray_from_metadata(
    ...
    df = _download_df(
        ...
        indices_encoding=ndarray_metadata.indices_encoding,
        cols_encoding=ndarray_metadata.cols_encoding,
    )
...
def _download_df(
    ...
    transfer_id: str,
    indices_encoding: List[Optional[ArrayEncoding]],
    cols_encoding: List[Optional[ArrayEncoding]],
) -> pd.DataFrame:
    ...
    df = decode_json_indices_inplace(df, indices_encoding)
    df = decode_json_columns_inplace(df, cols_encoding)
    return df
...
def _download_index_from_metadata(
    ...
    df = _download_df(
        aws_credentials,
        index_metadata.transfer_id,
        index_metadata.indices_encoding,
        index_metadata.cols_encoding,
    )
    if len(df.columns) > 1:
...
def _download_series_from_metadata(
    ...
    df = _download_df(
        ...
        indices_encoding=series_metadata.indices_encoding,
        cols_encoding=series_metadata.cols_encoding,
    )
    series = df.iloc[:, 0]
...
def _download_df_from_metadata(
    ...
    df = _download_df(
        ...
        indices_encoding=df_metadata.indices_encoding,
        cols_encoding=df_metadata.cols_encoding,
    )
