...
from terality_serde.json_encoding import (
    ArrayEncoding,
    encode_json_indices_inplace,
    encode_json_columns_inplace,
    has_float_nan,
...
def _check_if_array_needs_encoding(array: pd.array) -> Optional[ArrayEncoding]:
...
def _upload_df(
...
) -> Tuple[str, List[Optional[ArrayEncoding]], List[Optional[ArrayEncoding]]]:
    ...
    df = encode_json_indices_inplace(df, indices_encoding)
    df = encode_json_columns_inplace(df, cols_encoding)
    ...
    return import_id, cols_encoding, indices_encoding
...
def _upload_ndarray_and_get_metadata(
    ...
    transfer_id, cols_encoding, indices_encoding = _upload_df(
        aws_credentials, pd.DataFrame(ndarray)
    )
    return NDArrayMetadata(
        transfer_id=transfer_id,
        cols_encoding=cols_encoding,
        indices_encoding=[None] * len(indices_encoding),
    )
...
def _upload_index_and_get_metadata(
    ...
    transfer_id, cols_encoding, indices_encoding = _upload_df(
        aws_credentials, index.to_frame(index=False)
    )
...
def _upload_series_and_get_metadata(
    ...
    transfer_id, cols_encoding, indices_encoding = _upload_df(
        aws_credentials, series.copy(deep=True).to_frame()
    )
    return PandasSeriesMetadata(
        transfer_id=transfer_id,
        cols_encoding=cols_encoding,
        indices_encoding=indices_encoding,
...
def _upload_df_and_get_metadata(
    ...
    transfer_id, cols_encoding, indices_encoding = _upload_df(aws_credentials, df.copy(deep=True))
