...
def MakeCSV(inst): #CRÉATION DU FICHIER CSV
    fieldnames = []
    global writer
    if inst['a_allinfos']:
    ...
    writer = csv.DictWriter(open(inst['path']+inst['username'].replace('.', '_')+'_'+inst['target']+'.csv', 'w', newline=''), fieldnames = fieldnames)
    writer.writeheader()
...
def ExportCSV(inst, InfoList): #EXPORTATION DES DONNÉES VERS LE FICHIER CSV
    if inst['a_allinfos']:
        ...
        fieldnames = ['ID', 'Username', 'FullName', 'Page', 'Biography', 'IsPrivate', 'Followers', 'Following', 'Posts', 'External Link', 'IsBusiness', 'IsProfessional', 'IsVerified']
    writer = csv.DictWriter(open(inst['path']+inst['username'].replace('.', '_')+'_'+inst['target']+'.csv', 'a', newline=''), fieldnames = fieldnames)
    if inst['a_allinfos']:
