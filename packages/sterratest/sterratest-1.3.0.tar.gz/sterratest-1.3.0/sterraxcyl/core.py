...
def GetInfoPage(FollList):
    ...
    files.MakeCSV(inst) if inst['a_csv'] else files.MakeExcel(inst)
...
def Target():
    def exportPrint(): #PRINT INFOS SUR LA LISTE EXPORTÉE
        print(' {} {} '.format(inst['logos'][4 if inst['a_csv'] else 1], 'csv' if inst['a_csv'] else 'xlsx')+inst['target']+' of '+bold(inst['username'])+' succesfully exported in "'+bold(inst['path'])+'" directory, under "'+bold((inst['username']).replace('.', '_')+'_'+inst['target']+('.csv"' if inst['a_csv'] else '.xlsx"')))
...
def Login():
    ...
    try:
        ...
        username = input(inst['logos'][6]+' Enter an instagram username == >> ')
        password = input(inst['logos'][6]+' Enter its password == >> ')
        credentials = open(path, 'w')
        credentials.write(json.dumps({'username': username, 'password': password}))
        print(' {} Credentials are stored in "{}" file.'.format(inst['logos'][0], bold(path)))
        Login()
...
def main():
    ...
    optional.add_argument('-e', '--express-mode', action='store_true', help='sends ultra fast requests to get the table faster ! DEACTIVATED IF MORE THAN 200 ACCOUNTS IN THE LISTS TO AVOID BLOCKING YOUR ACCOUNT BY INSTAGRAM !')
    optional.add_argument('-h', '--help', action='help', default=argparse.SUPPRESS, help='show this help message and exit')
    optional.add_argument('-p', '--path', metavar='P', help='folder path where files will be exported and the credentials stored (by default in "sterraxcyl/" in the directory where you launch the program)')
    ...
    inst = {
        ...
        'target': args.target,
        'path': 'sterraxcyl/' if args.path == None else args.path,
    ...
    Login()
    print('\n~ If found this program useful, you can share it (that would help me) ~\n~ If you have suggestions, i am open, all my socials in my decription ~')
