    ...
    */
    ...
    function decomposeXPathListKeys(xpath) {
        ...
        for (let i = 0; i < xpath.length;) {
            ...
            result.push([leaf_xpath, value, multi_key_flag]);
        }
        return result;
    };
            ...
            for (let cfg of cfgs) {
                ...
                for (let node of entryJson.slice(1)) {
                    ...
                    if (cfg.extend_xpath) {
                        ...
                        nxpath = nxpath.concat("/"+node.data.prefix+":"+node.text);
                    }
                    if (cxpath == nxpath) {
     ...
     */
    function getCfgNodeId(tree, xps) {
        let node_id = 1;
    ...
    function extendListEntry(addListEntry, nodeId, lcfg) {
        if (!addListEntry[nodeId]) {
            ...
            for (let cfg of addListEntry[nodeId]) {
                if (JSON.stringify(cfg) == JSON.stringify(lcfg)) {
        ...
        for (let cfg of replayCfgs) {
            ...
            if (listPaths.length > 0) {
                ...
                for (let lpath of listPaths) {
                    ...
                    xps = xp.split("/");
                    node_id = getCfgNodeId(treedata, xps);
                    if (node_id > 1) {
