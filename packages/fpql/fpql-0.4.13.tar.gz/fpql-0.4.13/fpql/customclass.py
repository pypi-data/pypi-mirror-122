...
class OIS:
    def __init__(self, rates: List[pymodels.Rate],
                setting:pymodels.DepoSetting=None, index=None, 
                qlcalendar=None, date:str=None):
        self.__rates = rates
        #self.__setting = setting
        #self.__current_date = current_date
        if index:
        ...
        if date:
            ...
            self.__date = ql.Settings.instance().evaluationDate.ISO()
    ...
    def helpers(self):
        ...
        if self.__setting:
            ...
            cal = self.__qlcalendar
            if self.__date:
                today = ql.Date(self.__date, '%Y-%m-%d')
            else:
                today = ql.Settings.instance().evaluationDate
            
            start_date = cal.advance(today, 
                            ql.Period(setting.start_basis, ql.Days), 
                            qlc.ql_business_day[setting.business_day], 
                            False)
            #print('start_date:',start_date)
            ois_helpers = []
            for rate in self.__rates:
                if rate.tenor in ['1W', '2W', '3W']:
                    eom = False
                else:
                    eom = setting.eom
                
                end_date = cal.advance(start_date, 
                        ql.Period(rate.tenor), 
                        qlc.ql_business_day[setting.business_day], 
                        eom)
                ois_helper = ql.DatedOISRateHelper(start_date, 
                                                        end_date, 
...
class Curve:
    def __init__(self, day_count: qle.DayCount, method: qle.PiecewiseMethods = qle.PiecewiseMethods.logcubicdiscount,
    ...
    @property
    def parbond(self):
        return self.__parbond

    @parbond.setter
    def parbond(self, parbond):
        self.__parbond = parbond 
    ...
    def piecewise(self, date: str = None):
        if date:
            vdate = ql.Date(date, '%Y-%m-%d')
        else:
            vdate = ql.Settings.instance().evaluationDate
