...
class Edge2Client(object):
    ...
    def new_k8s_upstream_from(self, up_id, k8s_services=None, delete_origin=False):
        if not self.app_id:
            raise Exception('no active application selected')
        if not up_id:
            raise Exception('no active upstream selected')
        ...
        upstream = get_k8s_upstream(self, up_id)
        ...
        if upstream.get('enable_checker'):
            health_checker = upstream.get('checker')
        else
            health_checker = None

        if upstream.get('disable_ssl_verify'):
            ssl = False
        else
            ssl = True

        k8s_up_id = put_k8s_upstream(self, up_id=None,
            http_verb='POST', name=name, k8s_services=k8s_services,
            health_checker=health_checker, ssl=ssl, group=group)
        ...
        if not k8s_up_id:
            warning.warn('create k8s upstream failed.')
            return None
    ...
    def search_app(self, app_domain=None, upstream_ip=None,
        ...
        if (type_list is not None) and
            isinstance(type_list)
    ...
    def search_upstream_by_name(self, name, page=None, pagesize=None):
        return self.search_app(upstream_domain=name, page=page, pagesize=pagesize, k8s=False)

    def search_k8s_upstream_by_name(self, name, page=None, pagesize=None):
        return self.search_app(upstream_domain=name, page=page, pagesize=pagesize, k8s=True)
