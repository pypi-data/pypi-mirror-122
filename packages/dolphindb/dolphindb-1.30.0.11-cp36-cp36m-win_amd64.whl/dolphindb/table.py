...
class Table(object):
    ...
    def _setSort(self, by, ascending=True):
        if isinstance(by, list) or isinstance(by, tuple):
            self.__sort = [str(x) for x in by]
        else:
            self.__sort = [str(by)]
    ...
    def _setCsort(self, by, ascending=True):
        if isinstance(by, list) or isinstance(by, tuple):
            self.__csort = [str(x) for x in by]
        else:
            self.__csort = [str(by)]
    ...
    def sort(self, by, ascending=True):
        ...
        sortTable._setSort(by, ascending)
        return sortTable
    ...
    def csort(self, by, ascending=True):
        csortTable = copy.copy(self)
        csortTable._setCsort(by, ascending)
        return csortTable
    ...
    def showSQL(self):
        import re
        queryFmt = 'select {top} {select} from {table} {where} {groupby} {csort} {having} {orderby} {limit}'
        fmtDict = {}
