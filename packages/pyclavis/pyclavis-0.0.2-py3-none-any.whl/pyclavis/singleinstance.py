...
def check_instance(info_name=None, pnam=None, showmbox=True):
    ...
    port = 0
    try:
        print("reading last known socket", pnam)
        with open(pnam) as f:
            port = f.read()
            port = port.strip()
            port = int(port)
            print("last known port", port)
    except Exception as ex:
        print("app-socket read", ex)
    ...
    print("new free port", port)

    try:
        print("writing last known socket", pnam)
        with open(pnam, "w") as f:
            f.write(str(port))
    except Exception as ex:
        print("app-socket write", ex)
