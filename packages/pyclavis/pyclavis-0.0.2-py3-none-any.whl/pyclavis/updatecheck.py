...
def getversions(repo, opts=None, python_bin=None, callb=None):
    ...
    cont = "".join(lines)
    print("download version info", cont)
...
def check_download_version(repo_url, thisversion, opts=None, callb=None):

    thisversion = stripversion(thisversion)
    thisversion = semver.parse_version_info(thisversion)

    ver = getversions(repo_url, opts=opts, callb=callb)
