...
class Tile(TkMixin):
    def __init__(self, parent=None, idn=None, tk_root=None, **kwargs):
        ...
        if idn != None:
            if gt(idn) != None:
                raise Exception("already defined", idn)
            else:
    ...
    def unregister_idn(self):
        for el in [self, *self._elems, *self._frame]:
            if isinstance(el, Tile) and el.idn:
                del _global_reg[self.idn]
...
class TileEntry(Tile):
    ...
    def _create_entry(self):
        te = ttk.Entry(self.frame, textvariable=self._var)
        self._bind_focus(te)
        return te
    ...
    def _on_change(self, oval, nval):
        print(self, "on_change", "'" + oval + "'", "'" + nval + "'")
...
class TileEntryListbox(TileEntry):
    ...
    def on_double_click(self, ref_self):
        print("on_double_click", ref_self)
    ...
    def on_select(self, ref_self):
        print(self.__class__.__name__, "on_select", ref_self.get_val())

    def on_unselect(self, ref_self):
        print(self.__class__.__name__, "on_unselect", ref_self.get_val())
    ...
    def set_val(self, val):
        ...
        if val:
            ...
            self._lastsel = self._listb.curselection()
