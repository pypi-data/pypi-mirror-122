            ...
            if ask:
                ...
                if len(ck) == len(collect):
                    ...
                    if not os.path.exists(os.path.join(self.orip.parent, "auto.tvg")):
                        with open(
                            os.path.join(self.orip.parent, "auto.tvg"), "w"
                        ) as aur:
                        ...
                        try:
                            with open(
                                os.path.join(self.orip.parent, "auto.tvg")
                            ) as aur:
                                rd = ast.literal_eval(aur.read())
                        except:
                            with open(
        ...
        try:
            async with TelegramClient(
                ...
                async for message in client.iter_messages(
                    ...
                    if not self.plat.startswith("win"):
                        mtx = (
                            "".join(filter(lambda x: x in string.printable, mtx))
                            if mtx is not None
                            else None
                        )
                        msg = (
                            "".join(filter(lambda x: x in string.printable, msg))
                            if msg is not None
                            else None
                        )
                    if message.out:
