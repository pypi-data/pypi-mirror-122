...
class _Options(object):
    ...
    def __init__(self, **kwargs):
        ...
        for field in fields:
            value = (
                kwargs[field] if field in kwargs else self._defaults.get(field)
            )
...
class _GetsurfOptions(_Options):
    ...
    def __init__(self, **kwargs):
        ...
        if self.align in ["left", "center", "right"]:
            self.align = [0, 0.5, 1][
                ["left", "center", "right"].index(self.align)
            ]
        if self.lineheight is None:
            self.lineheight = DEFAULT_LINE_HEIGHT
        if self.pspace is None:
    ...
    def resolveoutlineshadow(self):
        ...
        self._spx = (
            ...
            if self.shadow is None
                ...
                ceil(s * self.fontsize * SHADOW_UNIT) for s in self.shadow
            )
        )
...
def getfont(**kwargs):
    ...
    if options.sysfontname is not None:
        ...
        try:
            ...
            raise IOError(
                "unable to read font filename: %s" % options.getfontpath()
            )
...
def _fitsize(text, size, **kwargs):
    ...
    def fits(fontsize):
        ...
        wmax, hmax = 0, 0
        for span in _wrap(
            text, fontsize=fontsize, width=width, **opts.towrapoptions()
        ):
...
def _applyshade(color, shade):
    f = exp(-0.4 * shade)
    r, g, b = [
        min(max(int(round((c + 50) * f - 50)), 0), 255) for c in color[:3]
    ]
...
def _resolveangle(angle):
    ...
    angle %= 360
    return (
        int(round(angle / ANGLE_RESOLUTION_DEGREES)) * ANGLE_RESOLUTION_DEGREES
    )
...
def _splitbytags(
    text, tagspec0, color0, underlinetag, boldtag, italictag, colortag
):
    colortag = {k: _resolvecolor(v, color0) for k, v in colortag.items()}
    tags = sorted(
        (set([underlinetag, boldtag, italictag]) | set(colortag.keys()))
        - set([None])
    )
    if not tags:
        yield text, tagspec0
...
class _Span:
    ...
    def render(self):
        if self.gcolor is None:
            ...
            if self.background is not None and not _istransparent(
                self.background
            ):
            ...
            gsurf0 = _gradsurf(
                h, 0.5 * asc, asc, self.tagspec.color, self.gcolor
            )
...
def _wrap(text, **kwargs):
    ...
    for jpara, para in enumerate(text.replace("\t", "    ").split("\n")):
        ...
        textandtags = list(
            _splitbytags(para, tagspec0, options.color, *tagargs)
        )
        _, tagspec0 = textandtags[-1]
        for line, linewidth in _wrapline(
            textandtags, options.width, getfontbytagspec
        ):
            if not line:
                jline += 1
                continue
...
def getsurf(text, **kwargs):
    ...
    if options.angle:
        ...
        _unrotated_size[
            (surf.get_size(), options.angle, text)
        ] = surf0.get_size()
    elif options.alpha < 1.0:
        surf = _fadesurf(
            getsurf(text, **options.update(alpha=1.0)), options.alpha
        )
        ...
        color = (0, 0, 0) if _istransparent(options.color) else options.color
        surf0 = getsurf(
            text, **options.update(color=color, ocolor=None, owidth=None)
        )
        oopts = {
        ...
        for span in spans:
            span.setdetails(
                options.antialias, options.gcolor, options.background
            )
            span.render()
        ...
        if not spans:
            ...
            for span in spans:
                span.y = int(
                    round(span.jline * linesize + span.jpara * parasize)
                )
...
def layout(text, **kwargs):
    ...
    x0, y0 = _blitpos(
        options.angle, options.pos, options.anchor, (sw, sh), None
    )
    ...
    if options.shadow is not None:
        spx, spy = (
            ceil(s * options.fontsize * SHADOW_UNIT) for s in options.shadow
        )
...
def draw(text, pos=None, **kwargs):
    ...
    pos = _blitpos(
        options.angle, options.pos, options.anchor, tsurf.get_size(), text
    )
