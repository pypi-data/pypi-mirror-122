...
class WhereValues___init__TC(unittest.TestCase):
    ...
    def test____module_constants(self):
        constants = (
            (wherevalues.DOUBLE_QUOTE_STRING, '".*?"'),
...
class WhereValues_lexTC(unittest.TestCase):
    ...
    def test_is_upper(self):
        ...
        w.lex()
        self.assertEqual(w.tokens, ["fieldname", "in", "value"])

    def test_lex_01(self):
        w = wherevalues.WhereValues("fieldname not from v")
        w.lex()
        self.assertEqual(w.tokens, ["fieldname", "not", "from", "v"])

    def test_lex_02(self):
        w = wherevalues.WhereValues("fieldname from v to w like *as in set")
        w.lex()
        self.assertEqual(
            w.tokens,
    ...
    def test_lex_03(self):
        w = wherevalues.WhereValues("fieldname above in side below not side")
        w.lex()
        self.assertEqual(
            w.tokens,
    ...
    def test_lex_04(self):
        ...
        w.lex()
        self.assertEqual(
            w.tokens, ["fieldname", "above", "in side", "below", "not side"]
        )
...
class WhereValues_parseTC(unittest.TestCase):
    ...
    def test__duplicate_keywords(self):
        ...
        self._vc(
            "field not in v not in", field="field", in__set="v", in_=False
        )
    ...
    def test_from_keyword(self):
        self._vc(
            "field from v", field="field", from_value="v", valid_phrase=True
        )
        self._vc(
            "field from v w",
            field="field",
    ...
    def test_above_keyword(self):
        self._vc(
            "field above v", field="field", above_value="v", valid_phrase=True
        )
        self._vc(
            "field above v w",
            field="field",
    ...
    def test_to_keyword(self):
        self._vc("field to v", field="field", to_value="v", valid_phrase=True)
        self._vc(
            "field to v w", field="field", to_value="v w", valid_phrase=True
        )
        self._vc(
            "field to v     w",
            field="field",
    ...
    def test_below_keyword(self):
        self._vc(
            "field below v", field="field", below_value="v", valid_phrase=True
        )
        self._vc(
            "field below v w",
            field="field",
    ...
    def test_in_keyword(self):
        self._vc("field in v", field="field", in__set="v", valid_phrase=True)
        self._vc(
            "field in v w", field="field", in__set="v w", valid_phrase=True
        )
        self._vc(
            "field in v     w", field="field", in__set="v w", valid_phrase=True
        )
    ...
    def find_values(self, dbset, valuespec):
        if valuespec.above_value and valuespec.below_value:
            for v in self._db[dbset][valuespec.field]:
                if (
                    v > valuespec.above_value
                    and v < valuespec.below_value
                    and valuespec.apply_pattern_and_set_filters_to_value(v)
                ):
                    yield v
...
class ValuesClause_evaluate_node_resultTC(unittest.TestCase):
    ...
    def test_above_and_below_02(self):
        ...
        vc.below_value = "s"
        vc.evaluate_node_result(self.p)
        self.assertEqual(vc.result, ["p", "q", "r"])
    ...
    def test_above_and_to_03(self):
        ...
        vc.to_value = "z"
        vc.evaluate_node_result(self.p)
        self.assertEqual(vc.result, ["p", "q", "r", "z"])
    ...
    def test_from_and_to_03(self):
        ...
        vc.to_value = "z"
        vc.evaluate_node_result(self.p)
        self.assertEqual(vc.result, ["cz", "p", "q", "r", "z"])
    ...
    def test_from_and_below_03(self):
        ...
        vc.below_value = "z"
        vc.evaluate_node_result(self.p)
        self.assertEqual(vc.result, ["cz", "p", "q", "r"])
    ...
    def test_above_02(self):
        ...
        vc.above_value = "d"
        vc.evaluate_node_result(self.p)
        self.assertEqual(vc.result, ["p", "q", "r", "z", "zz"])
    ...
    def test_below_02(self):
        ...
        vc.evaluate_node_result(self.p)
        self.assertEqual(
            vc.result, ["aa", "ab", "ad", "b", "cx", "cy", "cz", "p", "q", "r"]
        )
    ...
    def test_from_03(self):
        ...
        vc.from_value = "cz"
        vc.evaluate_node_result(self.p)
        self.assertEqual(vc.result, ["cz", "p", "q", "r", "z", "zz"])
    ...
    def test_all_not_in_set(self):
        ...
        vc.evaluate_node_result(self.p)
        self.assertEqual(
            vc.result, ["aa", "ab", "ad", "b", "cx", "cy", "cz", "z", "zz"]
        )
    ...
    def test_all_like_01(self):
        ...
        vc.evaluate_node_result(self.p)
        self.assertEqual(vc.result, ["cz", "z", "zz"])
    ...
    def test_all_like_02(self):
        ...
        vc.evaluate_node_result(self.p)
        self.assertEqual(
            vc.result, ["aa", "ab", "ad", "b", "cx", "cy", "p", "q", "r"]
        )
    ...
    def test_all_like_in_set_03(self):
        ...
        vc.evaluate_node_result(self.p)
        self.assertEqual(
            vc.result, ["aa", "ab", "ad", "b", "cx", "cy", "p", "q"]
        )
