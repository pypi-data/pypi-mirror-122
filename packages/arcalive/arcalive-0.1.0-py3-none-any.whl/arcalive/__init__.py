...
class ArcaAPI():
    ...
    def post_article(self, channel, name, content, category=None, copy_humor=False):
        ...
        for x in a:
            ...
            try:
                ...
                if post_num == '공지':
                    ...
                    post['view'] = int(soup2.find('span', 'vcol col-view').text.replace('\n', ''))
                    try:
                        post['comments'] = int(soup2.find('span', 'comment-count').text.replace('\n', '').replace('[', '').replace(']', ''))
                    except:
                        post['comments'] = 0
    ...
    def post_comment(self, id, message, reply_to=None):
        ...
        html = self.curSession.get(URL)
        self.check_for_error(html)
        csrf = self.get_article_csrf(URL)
        payload = {'_csrf': csrf,
                   'contentType': 'text',
                   'content': message,
                   'parentId': reply_to
