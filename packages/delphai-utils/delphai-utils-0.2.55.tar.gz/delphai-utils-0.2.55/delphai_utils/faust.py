...
class FaustApp():
  ...
  def __init__(self, id: str, broker: str) -> None:
    try:
      ...
      pass
    self.app = faust.App(id=id, broker=broker, web_enabled=False, **self.faust_config)
    ...
    default_logging_config['formatters']['standard']['format'] = '[%(asctime)s] [%(levelname)s] [%(name)s] %(message)s'
    ...
    self.logging_config = always_merger.merge(default_logging_config, provided_logging_config)
