...
class CountMatrix():
    ...
    def _filter_by_row_label(counts: pd.DataFrame,
        ...
        return counts.loc[labels.map(lambda label: query not in str(label))]
...
def main(args):
    ...
    stacks.write_coordinates('coordinates.tsv')
    ...
    count_matrix.to_csv(f'read_counts_{filename_parameters}.tsv',
                        na_rep=parsed_args.undefined_representation)
    frequency_matrix = count_matrix.calculate_frequencies()
    frequency_matrix.to_csv(f'haplotype_frequencies_{filename_parameters}.tsv',
                            na_rep=parsed_args.undefined_representation)
    ...
    if parsed_args.plot >= PLOT_SUMMARY:
        frequency_matrix.plot_haplotype_counts("haplotype_counts_frequencies",
    ...
    if parsed_args.discrete_calls:
        ...
        dosage_matrix.to_csv(f'haplotypes_{filename_parameters}_discrete_calls.tsv',
                             na_rep=parsed_args.undefined_representation)
        dosage_matrix.write_total_calls(
            f'haplotypes_{filename_parameters}_discrete_calls_total.tsv',
            na_rep=parsed_args.undefined_representation
        )
        if parsed_args.plot >= PLOT_SUMMARY:
            dosage_matrix.plot_haplotype_counts("haplotype_counts_discrete_calls",
                                                parsed_args.plot_type)
        if parsed_args.dosage_filter:
            ...
            if parsed_args.plot >= PLOT_SUMMARY:
                ...
                orig_dosage_matrix.write_locus_correctness_completeness(
                    dosage_matrix,
                    'locus_completeness_correctness.tsv',
                    na_rep=parsed_args.undefined_representation
                )
                orig_dosage_matrix.write_sample_correctness_completeness(
                    dosage_matrix,
                    'sample_completeness_correctness.tsv',
                    na_rep=parsed_args.undefined_representation
                )
                dosage_matrix.plot_haplotype_counts(
                    "haplotype_counts_discrete_calls_filtered",
                    parsed_args.plot_type
                )
                if parsed_args.locus_correctness_filter:
                    ...
                    new_bed = (f"{filename_parameters}_"
        ...
        dosage_matrix.to_csv(
            f'haplotypes_{filename_parameters}_discrete_calls_filtered.tsv',
            na_rep=parsed_args.undefined_representation)
        dosage_matrix.write_population_frequencies(
            f'haplotypes_{filename_parameters}_pop_hf.tsv',
            na_rep=parsed_args.undefined_representation)
