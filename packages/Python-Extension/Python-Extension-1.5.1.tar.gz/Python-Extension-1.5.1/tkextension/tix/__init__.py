...
class AskAnswer(Layout):
    def __init__(self, tkinter_attribute, msg='', width=100, validate=None, validatecommand=None, invalidcommand=None):
        self.tk = tkinter_attribute
        self.frame = Frame(self.tk, width=width)
        # Tkinter
        if msg != '':
            self.label = Label(self.frame, text=msg)
            self.label.place(relx=0.5, rely=0.05, relwidth=1, relheight=0.1, anchor='center')
        if validate != None:
            ...
            if invalidcommand == None:
                self.entry = Entry(self.frame, validate=validate, validatecommand=validatecommand)
            else:
                self.entry = Entry(self.frame, validate=validate, validatecommand=validatecommand, invalidcommand=invalidcommand)
        if msg != '':
            self.entry.place(relx=0.5, rely=0.55, relwidth=1, relheight=0.9, anchor='center')
        else:
            self.entry.place(relx=0.5, rely=0.5, relwidth=1, relheight=1, anchor='center')
        # Window
    def config(self, msg=''):
        if msg != '':
            self.label = Label(self.frame, text=msg)
