...
def resample(
  ...
  for kernel_num, kernel in enumerate(kernel_list) : 
    ...
    for template_seed in template_seeds : 
      ...
      data = data_transform(input_data, config, scale = [ float(i) for i in scale.split(":")] if scale else None )
...
def main() : 
  ...
  parser.add_argument('--scale', type=str, default = None, 
                      help="List of scale factors for input data e.g. '1:1:1.15' for 15%% upscaling of multiplicity")
