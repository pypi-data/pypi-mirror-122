from mjooln.filesystem import *


class Store:
    _folder = HOME.append('store')
    _extension = 'txt'
    ...
    @classmethod
    def _get_file(cls, key):
        key = Key.glass(key)
        type = cls._type if cls._type is not None else str(key.words()[0])
    ...
    @classmethod
    def exists(cls, key):
        return cls._get_file(key).exists()

    @classmethod
    def get(cls, key, crypt_key=None, password=None):
        file = cls._get_file(key)
