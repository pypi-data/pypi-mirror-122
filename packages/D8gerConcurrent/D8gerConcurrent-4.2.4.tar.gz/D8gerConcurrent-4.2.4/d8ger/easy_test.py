...
def parseRequest():
    ...
    if (body_file_name is not None) and (not str.isspace(body_file_name)):
        try:
            ...
            exit(0)
    content = json.dumps(request_body) if isJson else request_body
    print("HTTP请求参数: \n\t请求url=[{}]\n\t请求方式=[{}]\n\t请求Headers=[{}]\n\t请求Body数据=[{}]\n".format(
            url,
            method,
...
def execute_http(i):
    ...
    cost_microsecond_time = (execute_end_time - execute_start_time).microseconds / 1000
    print("#############第[{}]次请求#############\n\texecute_start_time=[{}], execute_end_time=[{}], 耗时=[{}ms]\n\t响应结果:{}\n".format(
        ...
        cost_microsecond_time,
        response_text)
    )
    return cost_microsecond_time
