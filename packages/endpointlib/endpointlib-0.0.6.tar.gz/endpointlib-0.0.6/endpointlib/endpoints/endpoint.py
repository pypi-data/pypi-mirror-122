...
class Endpoint:
    ...
    # Must be called with 'async with self.get_mqtt_async_client() as x' because MQTTAsyncClient is an async context manager
    def get_mqtt_async_client(self):
        return self.get_mqtt_async_client(id=self.create_id(), host=self.host, port=self.port)

    # Must be called with 'async with self.get_mqtt_async_client(id, host, port) as x' because MQTTAsyncClient is an async context manager
    def get_mqtt_async_client(self, id, host, port):
        return MQTTAsyncClient(id, host, port)
