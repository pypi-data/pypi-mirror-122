...
class Camera(BaseCamera):
    ...
    @classmethod
    def __init__(self, recommended, res ):
        print("D... init Camera class", recommended, res)
    ...
    def init_cam(res):
        ...
        vids = recommend_video(Camera.recommended)
        if len(vids)>0:
            vidnum = vids[0]
    ...
    def frames():
        ...
        """
        cap = Camera.init_cam(  Camera.res)
        nfrm = 0
        if Camera.recommended=="":
            wname = "none "+Camera.res
        else:
            wname = Camera.recommended + " "+ Camera.res
        frame_prev = None
        while True:
        ...
        #            with timeout(3): #--- this may help when no CAP_V4L2; MAYNOT
                    print("D... reading frame (frames iter)" )
                    ret, frame = cap.read()
                    BaseCamera.nframes+=1

                    #wname = f"res {frame.shape[1]}x{frame.shape[0]}"
                    nfrm+=1
                    print("D... got frame (frames iter)",nfrm, "ret=", ret)
        #        except TimeoutError:
        #            timeoutok = True
        #            nfrm = 0
