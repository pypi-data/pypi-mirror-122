...
class V4L2_CTL():
    ...
    def _list_controls(self):
        ...
        output = sp.check_output(['v4l2-ctl', '-d', self.device, '-l']).decode(
...
def main( devid ):
    ...
    while(True):# https://github.com/nrsyed/computer-vision/blob/master/real_time_histogram/real_time_histogram.py
        ...
        #gg = cc.get_gain()
        #gex = cc.get_exposure_absolute()
        #print(np.arange(bins))
        #print("===================")
        #print(hist.sum(), "SUM",hist )
        #print("===================")
        #print(hist*np.arange(bins))
        ...
        #print(hist.mean(), hist.min(), hist.max(), hist.std() )
