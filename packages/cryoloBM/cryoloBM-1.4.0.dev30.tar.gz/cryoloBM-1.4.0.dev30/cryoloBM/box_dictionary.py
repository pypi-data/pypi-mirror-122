...
class Box_dictionary:
    ...
    def resize_3D_filament(self, new_boxsize: int, f: str) -> None:
        """
        Resize the 3D filament, in a similar way did in import_data._collect
        :param new_boxsize: the new boxsize value
        :param f: filename
        """

        # clean the filament dicts
        for n in self.filament3D_dictionary[f]:
            self.filament3D_dictionary[f][n] = list()  # delete all the present filaments
        for n in self.filament_dictionary[f]:
            self.filament_dictionary[f][n] = list()  # delete all the present filaments

        for z in self.cryolo_filament_dictionary[f]:
            for fil in self.cryolo_filament_dictionary[f][z]:
                offset = new_boxsize / 2  # for convertion cryolo.BBox to MySketch obj
                filament = helper_filament.picked_filament(box_size=new_boxsize, is_tomo=True)
                filament.begin_fil = [fil.boxes[0].x + offset, fil.boxes[0].y + offset]
                filament.end_fil = [fil.boxes[-1].x + offset, fil.boxes[-1].y + offset]
                self.add_picked_filament(fil=filament.get_rect_sketch(color=fil.meta["color"]), f=f, n=z,
                                         still_picking=False, fil_id=fil.meta["id"])
