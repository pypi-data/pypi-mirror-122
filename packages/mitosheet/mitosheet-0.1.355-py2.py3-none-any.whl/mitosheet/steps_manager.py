...
class StepsManager():
    ...
    def sheet_json(self) -> str:
        ...
        passed around
        """
        return dfs_to_json(self.curr_step.dfs, self.curr_step.column_ids)
    ...
    @property
    def column_ids_array_json(self) -> str:
        """
        Returns a list of mappings from column ids -> column headers,
        for each sheet.

        We take special care to return the column ids array in the same order  
        as the columns in the dataframe itself. 
        """
        column_ids_array = []
        for sheet_index, df in enumerate(self.curr_step.dfs):
            column_id_to_column_header_map = {
                self.curr_step.column_ids.get_column_id_by_header(sheet_index, column_header): column_header
                for column_header in df.keys()
            }
            column_ids_array.append(column_id_to_column_header_map)

        return json.dumps(column_ids_array)
