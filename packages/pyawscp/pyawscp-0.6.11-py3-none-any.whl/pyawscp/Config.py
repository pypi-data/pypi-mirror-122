...
class Config:
    ...
    def __init__(self):
        ...
        self.noColor                  = False # :XXX   Implement later, it is not on the file.ini (yet)
    ...
    def assumeRoleSession(self, roleArn, mfaToken):
        if mfaToken:
            tempCredentials = self.botoSessionCredentialsOwner().client('sts').assume_role(
                RoleArn=roleArn,
    ...
    def saveCookieTempCredentials(self, tempCredentials):
        ...
        file.set("COOKIE","profile",           self.awsProfile)
        file.set("COOKIE","region",            self.awsRegion)
        file.set("COOKIE","assume-role",       self.assumeRole)
        file.set("COOKIE","mfa-serial",        self.mfaSerial)
    ...
    def createCookieRoleSession(self):
        ...
        if self.mfaSerial and not self.mfaPassCode:
           output = "\n\n\n"
           output += Style.RED  + "=".ljust(97,"=") +"\n"
           output += Style.BLUE + " The MFA " + Style.RESET + "\"" + Style.GREEN + self.assumeRole + Style.RESET + "\"" + Style.BLUE + " is configured for the environment" + Style.RESET + "\n" 
    ...
    def createCookieMFATokenSession(self):
        ...
        if self.mfaSerial:
            ...
            tempCredentials = boto3.client('sts').get_session_token(DurationSeconds=3600)
        self.saveCookieTempCredentials(tempCredentials)
