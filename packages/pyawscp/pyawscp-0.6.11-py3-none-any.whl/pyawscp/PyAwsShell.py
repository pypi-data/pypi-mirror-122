...
class PyAwsShell(cmd.Cmd):
    ...
    def complete_set(self,text,line,begidx,endidx):
        return [i for i in ('tags','profile','region,''verbose','tables-line','chunk-size','threshold','assume-role','mfa-serial','mfa') if i.startswith(text)]
    ...
    def do_set(self,arg):
        ...
        if "tags" in arg:
            ...
            try:
                ...
                role_service = RoleService(self.preferences)
                role         =  role_service.search_role_by_name(value)
                if not role or len(role) < 1:
                    Messages.showWarning(f"Role Name \"{Style.IBLUE}{value}{Style.IGREEN}\" not found!\n    Check them with the command {Style.IYELLOW}listRoles{Style.RESET}")
                    return None
                role_arn = role[0]['role-arn']
                value = f"{role_arn} ({value})"
            self.preferences.assumeRole=role_arn
            ...
            try:
                ...
                mfa_service = MfaService(self.preferences)
                mfa         = mfa_service.search_mfa_by_username(value)
                if not mfa or len(mfa) < 1:
                    Messages.showWarning(f"MFA Serial \"{Style.IBLUE}{value}{Style.IGREEN}\" not found!\n    Check them with the command {Style.IYELLOW}listMfas{Style.RESET}")
                    return None
                mfa_serial = mfa[0]['mfa-device']
                value = f"{mfa_serial} ({value})"
            self.preferences.mfaSerial=mfa_serial
            ...
            self.preferences.startSessionWithTheNewCookie()
    ...
    def complete_get(self,text,line,begidx,endidx):
        return [i for i in ('tags','profile','region','verbose','tables-line','chunk-size','threshold','assume-role','mfa-serial') if i.startswith(text)]
    def do_get(self,arg):        
    ...
    def do_environment(self, arg):    
        ...
        output +=Style.BLUE + "                    " + Style.BLUE + "Command to set..:" + Style.GREEN+ " set assume-role arn:aws:iam::999999999999:role/developer" + Style.RESET + f" {Style.CYAN}or\n"
        output +=Style.BLUE + "                    " + Style.BLUE + "                 " + Style.GREEN+ " set assume-role {NAME}" + Style.RESET + "\n"
        output +=Style.BLUE + "                    " + Style.BLUE + "                 " + Style.GREEN+ f"     {Style.CYAN}where {{NAME}} is the " + f"Role's Name from the list of {Style.IYELLOW}listRoles{Style.CYAN} command\n"
        ...
        output +=Style.BLUE + "                    " + Style.BLUE + "Command to set..:" + Style.GREEN+ " set mfa-serial arn:aws:iam::999999999999:mfa/joe.dotz@there.com" + Style.RESET + f" {Style.CYAN}or\n"
        output +=Style.BLUE + "                    " + Style.BLUE + "                 " + Style.GREEN+ " set mfa-serial {USERNAME}" + Style.RESET + "\n"
        output +=Style.BLUE + "                    " + Style.BLUE + "                 " + Style.GREEN+ f"     {Style.CYAN}where {{USERNAME}} is the " + f"Username's MFA Device/Serial from the list of {Style.IYELLOW}listMfas{Style.CYAN} command\n"
        output +=Style.BLUE + "                    " + Style.BLUE + "Command to clean:" + Style.GREEN+ " unset mfa-serial" + Style.RESET + "\n"
        output += "\n"
    ...
    #########################################################################################################
    def do_sessionLogoff(self, arg):
        "Remove a running temporary AWS Session (Token)\n"
        if self.isWindows():
           self.executeFunction(PyAwsShell.sessionLogoff,arg,self.preferences)  
        else:    
           self.executeFunction(self.sessionLogoff,arg,self.preferences)
    def sessionLogoff(self, arg, preferences):
        if self.preferences.isValidCookieTempCredentials():
           ...
           output += Style.GREEN + "AWS Temporary Session (Token) removed!"
           output += "\n"
        else:
           output = CookieTempCredentials.invalidCookie()
        print(output)
    def help_sessionLogoff(self):
        print(self.helpFunction(Functions.SESSION_LOGOFF))            
    #def complete_roleLogoff(self,text,line,begidx,endidx):
    #    return ([i for i in ('graph','thin','save','verbose') if i.startswith(text)])

    #########################################################################################################
    def do_sessionInfo(self, arg):
        "Show info of your AWS Session\n"
        if self.isWindows():
           self.executeFunction(PyAwsShell.sessionInfo,arg,self.preferences)  
        else:    
           self.executeFunction(self.sessionInfo,arg,self.preferences)
    def sessionInfo(self, arg, preferences):
        aws_session = AwsSession(preferences)
        print(aws_session.session_info())
    def help_sessionInfo(self):
        print(self.helpFunction(Functions.SESSION_INFO))            
    #########################################################################################################
    def do_listProfiles(self, arg):
        "Show synchronized AWS Credential Profiles\n"
        if self.isWindows():
           self.executeFunction(PyAwsShell.listProfiles,arg,self.preferences)  
        else:    
           self.executeFunction(self.listProfiles,arg,self.preferences)
    def listProfiles(self, arg, preferences):
        config = preferences
        config.commandArguments = arg
        credentials_service = CredentialsService(config)
        print(credentials_service.list_profiles_view())
    def help_listProfiles(self):
        print(self.helpFunction(Functions.LIST_PROFILES))
    def complete_listProfiles(self,text,line,begidx,endidx):
        return ([i for i in ('roles','mfas') if i.startswith(text)])  
    #########################################################################################################
    def do_listRoles(self, arg):
        "Show synchronized AWS Credential Profiles\n"
        if self.isWindows():
           self.executeFunction(PyAwsShell.listRoles,arg,self.preferences)  
        else:    
           self.executeFunction(self.listRoles,arg,self.preferences)
    def listRoles(self, arg, preferences):
        config = preferences
        config.commandArguments = arg
        roles_service = RoleService(config)
        print(roles_service.list_roles_view())
    def help_listRoles(self):
        print(self.helpFunction(Functions.LIST_ROLES))
    #########################################################################################################
    def do_syncCredentials(self, arg):
        "Load and/or Synchronize AWS Credentials in PyAwsCp\n"
        if self.isWindows():
           self.executeFunction(PyAwsShell.syncCredentials,arg,self.preferences)  
        else:    
           self.executeFunction(self.syncCredentials,arg,self.preferences)
    def syncCredentials(self, arg, preferences):
        config = preferences
        config.commandArguments = arg
        credentials_service = CredentialsService(config)
        print(credentials_service.loadAndSyncProfiles())
    def help_syncCredentials(self):
        print(self.helpFunction(Functions.SYNC_CREDENTIALS))
    #########################################################################################################
    def do_addRoleProfile(self, arg):
        "Add a Role associating to an AWS Profile Credential\n"
        role_service = RoleService(self.preferences)
        role_service.add_role()
    def help_addRoleProfile(self):
        print(self.helpFunction(Functions.ADD_ROLE_PROFILE))
    #########################################################################################################
    def do_removeRoleProfile(self, arg):
        "Remove a Role associated with an AWS Profile Credential\n"
        role_service = RoleService(self.preferences)
        role_service.remove_role()
    def help_removeRoleProfile(self):
        print(self.helpFunction(Functions.REMOVE_ROLE_PROFILE))
    #########################################################################################################
    def do_listMfas(self, arg):
        "Show list of MFAs Device AWS Credential Profiles\n"
        if self.isWindows():
           self.executeFunction(PyAwsShell.listMfas,arg,self.preferences)  
        else:    
           self.executeFunction(self.listMfas,arg,self.preferences)
    def listMfas(self, arg, preferences):
        config = preferences
        config.commandArguments = arg
        mfa_service = MfaService(config)
        print(mfa_service.list_mfas_view())
    def help_listMfas(self):
        print(self.helpFunction(Functions.LIST_MFAS))    
    #########################################################################################################
    def do_addMfaProfile(self, arg):
        "Add a MFA Device associating to an AWS Profile Credential\n"
        mfa_service = MfaService(self.preferences)
        mfa_service.add_mfa()
    def help_addMfaProfile(self):
        print(self.helpFunction(Functions.ADD_MFA_PROFILE))
    #########################################################################################################
    def do_removeMfaProfile(self, arg):
        "Remove a MFA Device associated with an AWS Profile Credential\n"
        mfa_service = MfaService(self.preferences)
        mfa_service.remove_mfa()
    def help_removeMfaProfile(self):
        print(self.helpFunction(Functions.REMOVE_MFA_PROFILE))     
    #########################################################################################################
    def do_importAwsSeeData(self, arg):
        "Import AwsSee Data (Profile, Role and MFA)\n"
        if self.isWindows():
           self.executeFunction(PyAwsShell.importAwsSeeData,arg,self.preferences)  
        else:    
           self.executeFunction(self.importAwsSeeData,arg,self.preferences)
    def importAwsSeeData(self, arg, preferences):
        config = preferences
        config.commandArguments = arg
        service = CredentialsService(config)
        service.import_from_awsee()
    def help_importAwsSeeData(self):
        print(self.helpFunction(Functions.IMPORT_AWSEE_DATA))   
    #########################################################################################################
    def do_rollbackAwsSeeData(self, arg):
        "Rollback a snapshot from a backup performed during the AwsSee data import\n"
        if self.isWindows():
           self.executeFunction(PyAwsShell.rollbackAwsSeeData,arg,self.preferences)  
        else:    
           self.executeFunction(self.rollbackAwsSeeData,arg,self.preferences)
    def rollbackAwsSeeData(self, arg, preferences):
        config = preferences
        config.commandArguments = arg
        service = CredentialsService(config)
        service.rollback_import_from_awsee()
    def help_rollbackAwsSeeData(self):
        print(self.helpFunction(Functions.ROLLBACK_AWSEE_DATA))   
