...
from pyawscp.Credentials import Credentials
...
class PyAwsShell(cmd.Cmd):
    ...
    #    return ([i for i in ('graph','thin','save','verbose') if i.startswith(text)])
    #########################################################################################################
    def do_listCredentials(self, arg):
        "Show AWS Credentials Configured\n"
        if self.isWindows():
           self.executeFunction(PyAwsShell.listCredentials,arg,self.preferences)  
        else:    
           self.executeFunction(self.listCredentials,arg,self.preferences)
    def listCredentials(self, arg, preferences):
        config = preferences
        config.commandArguments = arg
        credentials = Credentials(config)
        print(credentials.list_credentials_formatted())
