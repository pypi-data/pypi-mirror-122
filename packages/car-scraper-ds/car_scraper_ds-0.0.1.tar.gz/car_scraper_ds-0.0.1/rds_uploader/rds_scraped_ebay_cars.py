...
class DataHandling:
    ...
    def __init__(self):
        ...
        self.USER = input('Enter your user: ')  # postgres
        self.PASSWORD = input('Enter your password: ') 
        self.PORT = 5432
        self.DATABASE = 'postgres'
        #self.cur = None
        self.df = None
        self.conn = psycopg2.connect(f"dbname=self.DATABASE user=self.USER password=self.PASSWORD host=18.132.161.210 port=self.PORT")
        self.cur = self.conn.cursor()

        #psycopg2.connect(f"dbname=postgres user=postgres password={password} host=3.21.144.229 port=5432")
