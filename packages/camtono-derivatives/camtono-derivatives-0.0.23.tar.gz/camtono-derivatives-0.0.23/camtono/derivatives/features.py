def inject_feature_variables(feature, filter_set_inputs, default_inputs, feature_input, feature_map, table_prefix,
    ...
    if feature.get('grain') != output_grain and mapping_feature is not None:
        ...
        feature_ast, updated_feature = translate_feature_grain(feature=feature, mapping_feature=mapping_feature,
                                                               output_grain=output_grain, mapping_table_name=mapping_table_name, feature_table_name=table_name)
...
def translate_feature_grain(feature_table_name, mapping_table_name, feature, mapping_feature, output_grain):
    ...
    ast = {
        'select': [dict(value='mapping.{}'.format(i['display_name']) if i['display_name'] == output_grain else 'base.{}'.format(i['display_name']), name=i['display_name']) for i in new_columns],
        'from': [
