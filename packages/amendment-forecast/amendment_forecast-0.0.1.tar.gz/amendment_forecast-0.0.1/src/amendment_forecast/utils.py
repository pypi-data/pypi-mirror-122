...
# FRom pycharm
# from src.forecast.models import DATE_COLUMN_NAME, VALUE_COLUMN_NAME
...
def consolidate_scores(metrics: dict, average_actual: float):
    ...
    metrics["composite"] = (metrics["ae_composite"] + metrics["accuracy_mape"] + metrics["accuracy_rmse"]) / 3

    return_metrics = ["accuracy_mape", "accuracy_rmse", "accuracy_mae", "accuracy_medae", "composite"]
