...
class BaseModel:
    ...
    def evaluate(self, dataframe: pd.DataFrame, train_end_date: datetime, frequency: str,
        ...
        result_package["predictions"] = predicted
        result_package["actual"] = actual
        result_package["forecast"] = forecast
...
class GreyKite(BaseModel):
    ...
    def run(self, dataframe: pd.DataFrame, train_end: datetime, frequency: str) -> tuple:
        ...
        result = forecaster.run_forecast_config(  # result is also stored as `forecaster.forecast_result`.
            ...
            config=ForecastConfig(
                ...
                forecast_horizon=len(dataframe) - sum(train_mask)
            )
        )
        ...
        return backtest["actual"], backtest["forecast"]
...
class Naive(BaseModel):
    ...
    def run(self, dataframe: pd.DataFrame, train_end: datetime, frequency: str) -> tuple:
        ...
        actual = []
        predicted = []
        for index, row in dataframe_test.head().iterrows():
            ...
            predicted_value = previous_value * growth_rate
...
class SARIMA(BaseModel):
    ...
    def full_forecast(self, train_df, forecast_period_list):
        ...
        # make a one-step prediction
        yhat = model.forecast(steps=len(forecast_period_list))
