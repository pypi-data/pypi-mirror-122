        ...
        if years_outside_of_training > 0:
            for yy in range(1, int(year) + 1):
                if yy > (2 * training_years):
                    ...
                    multiplier *= 0.95
        degraded_accuracies[year] = multiplier * consolidated_metrics["composite"]
...
df = pd.read_csv("/Users/makanicartwright/projects/amendmentai/forecast_testing/test_output/curves/1-1-6_100000_month_modern_brand/sample_curves.csv")
run_forecast_ensemble(df, "period", "sales", 3, frequency="month")