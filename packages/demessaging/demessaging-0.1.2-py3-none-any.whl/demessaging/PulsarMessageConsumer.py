...
class PulsarMessageConsumer(PulsarConnection):
    ...
    def __init__(self, pulsar_config: dict, handle_request, handle_response=None, module_info: dict = None):
        ...
        if PulsarConfigKeys.QUEUE_SIZE in pulsar_config:
            self.request_semaphore = threading.BoundedSemaphore(value=pulsar_config.get(PulsarConfigKeys.QUEUE_SIZE))
        self.pool = concurrent.futures.ThreadPoolExecutor(max_workers=pulsar_config.get(PulsarConfigKeys.MAX_WORKERS))
    ...
    def check_config(self):
        for key in PulsarConfigKeys:
            ...
            if key == PulsarConfigKeys.QUEUE_SIZE or key == PulsarConfigKeys.MAX_WORKERS:
                # skip optinal keys
                continue
        ...
        if PulsarConfigKeys.QUEUE_SIZE in self.pulsarConfig and PulsarConfigKeys.MAX_WORKERS in self.pulsarConfig and \
                self.pulsarConfig[PulsarConfigKeys.QUEUE_SIZE] < self.pulsarConfig[PulsarConfigKeys.MAX_WORKERS]:
                ...
                if msg_type == MessageType.PING:
                    ...
                    if self.request_semaphore is None:
                        ...
                        if self.request_semaphore.acquire(blocking=False):
                            ...
                            self.loop.run_in_executor(self.pool, self.handle_request_via_queue, msg)
                        else:
