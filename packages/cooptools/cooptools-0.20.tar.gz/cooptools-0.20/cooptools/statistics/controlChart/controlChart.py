...
def qualify_data_sequence(data,
    ...
    for ii in range(start + x - 1, stop):
        ...
        if qualifiers is not None:
            rule_triggers.append(qualifiers)
...
def control(data: List[Union[int, float]],
    ...
    for ii in range(0, len(data)):
        ...
        out_of_control[ii] = len(out_of_limit_high[ii]) > 0 or \
                             len(out_of_limit_low[ii]) > 0 or \
                             len(two_of_three_high_2stdev[ii]) > 0 or \
                             len(two_of_three_low_2stdev[ii]) > 0 or \
                             len(four_of_five_high_1stdev[ii]) > 0 or \
                             len(four_of_five_low_1stdev[ii]) > 0 or \
                             len(seven_high[ii]) > 0 or \
                             len(seven_low[ii]) > 0 or \
                             len(seven_trend_high[ii]) > 0 or \
                             len(seven_trend_low[ii]) > 0 or \
                             len(eight_out_of_1stdev[ii]) > 0 or \
                             len(fifteen_within_1stdev[ii]) > 0 or \
                             len(fourteen_alternate[ii]) > 0 if ii >= set_baseline_periods - 1 else None
