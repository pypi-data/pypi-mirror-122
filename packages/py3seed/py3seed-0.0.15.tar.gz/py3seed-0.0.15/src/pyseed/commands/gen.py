    ...
    for d in os.listdir(seeds_dir):  # Blueprints
        ...
        if os.path.isdir(p):
            ...
            for dd in os.listdir(p):  # Views
                ...
                seed_context = {}  # Seed can be shared in different view, so do not add blueprint/view in its context
                ...
                with open(pp) as f:  # Seeds defined in views
                    for line in f:
                        ...
                        if key_value_found:
                            ...
                            #
                            # NOTES:
                            # 1. Varibles name should be in snake format, i.e, two_words
                            # 2. Varibles name should start with view_ or seed_
                            # 3. Varibles can be accessed in view/seed templates
                            #
                            value = _parse_varible_value(key, value)
                            view[key] = value
                            seed_context[key] = value
                        else:
                            row = {'columns': []}
                            if '|' in line:  # Nested column, e.g, a,b|c,d
                                for c in line.split('|'):
                                    ...
                                    for cc in c.split(','):
                                        cc = cc.strip()
                                        seed = _parse_seed(cc, models, seed_context)
                                        if seed:
                                            view['seeds'].append(seed)
                                            context['seeds'][seed['name']] = seed
                                        column.append(cc)
                                    #
                                ...
                                for c in line.split(','):
                                    c = c.strip()
                                    seed = _parse_seed(c, models, seed_context)
                                    if seed:
                                        view['seeds'].append(seed)
                                        context['seeds'][seed['name']] = seed
...
def _parse_varible_value(key, value):
    ...
    key = key.lower()
    if key.startswith('view_') or key.startswith('seed_'):
        key = key[5:]
    value = value.strip()
...
def _parse_seed(column, models, seed_context={}):
    ...
    if found:
        action = tokens[-1]
        return {'model': found, 'includes': includes, 'action': action, **_generate_names(column), **seed_context}
    else:
        return None
...
def _recursive_render(t_base, o_base, name, context, env):
    ...
    if match_list:
        ...
        if key == 'blueprints':
            ...
            out_key = '__seed'
            out_values = context['seeds'].values()  # {name:seed]
            out_names = [t_name.replace(syntax, v['name']) for v in out_values]
        else:
