...
def _send_endpoint_test_request_response_bodies(
    ...
    interaction_to_send = None
    for result in endpoint_context.results:
        for interaction in result.interactions:
            if interaction.status != levo_commons.status.Status.success:
                interaction_to_send = interaction
    if interaction_to_send:
        content = get_formatted_request_response(
            interaction_to_send.status,
            interaction_to_send.recorded_at,
            interaction_to_send.request,
            interaction_to_send.response,
        )
        attachment = {
