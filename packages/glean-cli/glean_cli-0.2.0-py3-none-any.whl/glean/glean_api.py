...
def create_build_from_git_revision(
    session: Session, project_id: str, git_revision: Optional[str], git_path: Optional[str], deploy: bool
):
    ...
    return _graphql_query(
        ...
        mutation CreateBuild($projectId: String!, $buildSpec: BuildSpecInput!, $deploy: Boolean!) {
            createBuild( projectId: $projectId, buildSpec: $buildSpec, deploy: $deploy ) {
                id,
                resources {
                    added { models { name }, savedViews { name } }
                    updated { models { name }, savedViews { name } }
                    deleted { models { name }, savedViews { name } }
                },
                warnings,
                errors
            }
