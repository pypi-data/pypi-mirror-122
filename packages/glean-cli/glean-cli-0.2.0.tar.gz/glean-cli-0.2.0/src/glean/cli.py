...
local_path_argument = click.argument("filepath", type=click.Path(exists=True), default=".")
...
def preview(ctx, git_revision, git_path, filepath):
    ...
    build_results = _create_build_using_options(
        ctx, git_revision=git_revision, git_path=git_path, filepath=filepath, deploy=False
    )
...
def deploy(ctx, git_revision, git_path, filepath, preview):
    ...
    build_results = _create_build_using_options(
        ctx, git_revision=git_revision, git_path=git_path, filepath=filepath, deploy=True
    )
...
def _create_build_using_options(ctx, git_revision=None, git_path=None, filepath=None, deploy=False):
    s = Session()
    project_id = login(s, ctx.obj["credentials"])
    if git_revision:
        return create_build_from_git_revision(s, project_id, git_revision, git_path, deploy)
    else:
