...
class CamController(BaseController.BaseController):
    ...
    def _camcheckComplete(self, results, cmd, d, commandName="UNKNOWN", report=False):
        ...
        for line in results:
            ...
            if "UNACKNOWLEDGED" in line:
                ...
                iacked = False
            if len(tokens) > 0:
                if tokens[0] in CamForthTranslations.keyword_translations:
                    # Do the translations
                    keyName = CamForthTranslations.keyword_translations[tokens[0]]
                    keyword = "%s%s" % (cam[:3].upper(), keyName)
                    errors.append(keyword)
                    if len(tokens) > 1 and tokens[1][0] != "#":
                        errorValues.append("%s=%s" % (keyword, tokens[1]))
                    if keyName in (
                        "FillFault",
                        "LN2Empty",
                        "RedIonPump",
    ...
    def _readVoltageLine(self, line, cmd):
        ...
        """
        line = line.strip()
        if len(line) == 0 or line[0] == "#" or line.find("NONE") >= 0 or line == "ok":
            return "", ""
        ...
        try:
            tokens = line.split()
            board, name, kind = tokens[0].split("_")
            spec = board[:3]

            if tokens[0] == "2NDARY_DEWAR_PRESS":
                key = tokens[0]
            else:
                key = board[3:] + "_" + name
