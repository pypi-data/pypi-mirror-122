...
class ExposureDelegate(BaseDelegate):
    ...
    def _mechCards(self, specname):
        ...
        if self.icc.exposure.flavor in ("bias", "dark"):
            hartmann = ", ".join(status.get("Hartmann", "Unknown"))
        else:
            hartmann = (
                "Out"
    ...
    def _expstart_fits_cards(self):
        ...
        cards.extend(actorFits.plateCards(models, cmd=self.cmd))
        ...
        # cards.extend(actorFits.guiderCards(models, cmd=self.cmd))
        # cards.append(self.grabGuiderFrameName("GUIDER1", "The first guider image"))
...
class ExposureCmd(object):
    ...
    def testCards(self, cmd):
        ...
        # cards["plate"] = actorFits.plateCards(self.icc.models, cmd=cmd)
        # cards["guider"] = actorFits.guiderCards(self.icc.models, cmd=cmd)
