...
class HouseEstateWeeklyPriceChange(Document, MongoMixin):
    meta = {
        "strict": False,
        "indexes": [
            {"fields": ("estate_name", "week_begin_date", "location.city", "location.district"), "unique": True},
    ...
    last_week_snapshot_date = DateTimeField(required=True)
    last_week_quotation = EmbeddedDocumentField(HouseQuotation)
    current_week_snapshot_date = DateTimeField(required=True)
