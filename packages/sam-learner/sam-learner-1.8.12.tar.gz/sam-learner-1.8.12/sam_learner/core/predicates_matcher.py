...
def prefer_parameterized_predicates(possible_matches: List[ComparablePredicate]) -> List[ComparablePredicate]:
	"""Select predicates by preference for predicates that their signature is completely filled with parameters.

	Note:
		The domains that we are currently investigating don't have contradictions.
		On the other hand, constants might create contradictions in the algorithm, so to prevent that
		we prefer fully parameterized predicates over partially parametrized predicates (if exist).

	:param possible_matches: the predicates that were matched in this execution step.
	:return: the predicates that passed the selection.
	"""
	output_predicates = []
	predicate_name_to_matches_mapping = defaultdict(list)
	for match in possible_matches:
		predicate_name_to_matches_mapping[match.name].append(match)

	for predicate_name, possible_matches_list in predicate_name_to_matches_mapping.items():
		parameterized_predicates = []
		for possible_match in possible_matches_list:
			if all([sig[0].startswith("?") for sig in possible_match.signature]):
				parameterized_predicates.append(possible_match)

		if len(parameterized_predicates) > 0:
			output_predicates += parameterized_predicates

		else:
			output_predicates += possible_matches_list
...
class PredicatesMatcher:
	...
	def search_for_parameters_in_constants(
			self, possible_precondition: ComparablePredicate, grounded_predicate: ComparablePredicate) -> NoReturn:
		"""

		:param possible_precondition:
		:param grounded_predicate:
		:return:
		"""
	...
	def match_predicate_to_action_literals(
		...
		self.search_for_parameters_in_constants(possible_precondition, grounded_predicate)
	...
	def get_possible_literal_matches(
		...
		filtered_matches = prefer_parameterized_predicates(possible_matches)
