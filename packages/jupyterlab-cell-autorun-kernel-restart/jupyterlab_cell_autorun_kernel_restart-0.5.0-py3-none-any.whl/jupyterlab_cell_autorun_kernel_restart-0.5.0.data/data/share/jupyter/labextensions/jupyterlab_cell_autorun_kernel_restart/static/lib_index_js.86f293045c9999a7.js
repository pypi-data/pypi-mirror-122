...
class ReInit {
    constructor(app, nbtracker, mainmenu) {
        ...
        this.command_id_run_scene = 'cell-autorun-kernel-restart:run-scene';
        if (verbose)
    ...
    // **** setup helpers **********************************************************************************************************************
    setupReinitMenu() {
        this.reinit_menu = new _lumino_widgets__WEBPACK_IMPORTED_MODULE_5__.Menu({ commands: this.app.commands });
        this.reinit_menu.title.label = 'ReInit';
        this.reinit_menu.addItem({ command: this.command_id_do_reinit });
        this.reinit_menu.addItem({ command: this.command_id_run_scene });
    ...
    setupGlobalCommands() {
        ...
        this.app.commands.addCommand(this.command_id_run_scene, {
            label: 'Run Scene',
            execute: () => { this.doKernelInitialization(); }
        });
        this.app.commands.addKeyBinding({
            command: this.command_id_run_scene,
            args: {},
            keys: ['Accel Alt R'],
    ...
    async doReInit() {
        var _a, _b, _c;
        const result = await (new _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.Dialog({
            title: 'Do you really want to re-initialize the kernel with scene "' + this.getCurrentNotebookPresentScene() + '"?',
            buttons: [_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.Dialog.cancelButton(), _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.Dialog.okButton({ label: 'Restart' })]
        }).launch());
    ...
    async deletePresentScene() {
        ...
        const dialog = new _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.Dialog({
            ...
            buttons: [_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.Dialog.okButton({ label: 'Delete' }), _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.Dialog.cancelButton()]
        });
...
//# sourceMappingURL=lib_index_js.86f293045c9999a7.js.map