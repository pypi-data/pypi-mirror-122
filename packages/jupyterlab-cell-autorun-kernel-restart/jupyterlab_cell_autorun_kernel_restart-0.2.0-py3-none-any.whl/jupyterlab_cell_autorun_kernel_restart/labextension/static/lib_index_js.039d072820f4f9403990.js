...
class KernelReInitButton extends _jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.ToolbarButton {
    ...
    setupMainMenu() {
        ...
        this.submenu.title.label = 'ReInit';
        this.submenu.addItem({ command: 'cell-autorun-kernel-restart:toggle-autorun' });
        this.submenu.addItem({ type: 'separator' });
        this.app.commands.addCommand('test', {
            label: 'Test',
            isToggled: () => { return !!this.tog_test; },
            execute: () => { console.log('test'); this.tog_test = !this.tog_test; }
        });
        this.submenu.addItem({ command: 'test' });
        this.mainmenu.addMenu(this.submenu);
    }
    updateScenesInMenu(nbpanel) {
        var scene_list = this.getReinitDataCell(nbpanel).model.metadata.get('scenes');
        console.log('scene_list', scene_list);
    }
...
//# sourceMappingURL=lib_index_js.039d072820f4f9403990.js.map