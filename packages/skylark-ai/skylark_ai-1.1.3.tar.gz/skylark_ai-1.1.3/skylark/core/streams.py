...
class StreamClient:
	def __init__(self, fps, batch_size, sampling_rate, service, show_stream, scale_coordinates, token, show_processed_stream=False, save_raw_frames=False, scale_percent=100, quality=100):
		self.fps = fps
		self.frames = []
		self.json = ""
		self.syncid = -1
	...
	def createTasks(self):
		...
		self.tasksThread = threading.Thread(target=self.run_tasks, args=(asyncio.get_event_loop(), tasks))
	...
	async def on_network_status_change(self):
		...
		# 		[t.cancel() for t in asyncio.Task.all_tasks(loop)]
