...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return ATJlj if e else ATJlX
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->ATJlp:
...
@click.option("--revert",is_flag=ATJlj,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:ATJlR):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(ATJlp)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:ATJlM):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,ATJlp)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:ATJlM):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,ATJlp)
@pod.command(name="reset",help="Reset the local state to get a fresh LocalStack instance")
def cmd_pod_reset():
 from localstack_ext.bootstrap import pods_client
 pods_client.reset_local_state()
def print_table(columns:Dict[ATJlM,ATJlM],rows:List[Dict[ATJlM,Any]],formatters:Dict[ATJlM,Callable[[Any],ATJlM]]=ATJlp):
 from rich.table import Table
 if formatters is ATJlp:
  formatters=ATJla()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  ATJlD().__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->ATJlp:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->ATJlp:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->ATJlp:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->ATJlp:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
