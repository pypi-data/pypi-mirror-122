...
def read_api_key(raise_if_missing=VWLTI):
 key=(os.environ.get(ENV_LOCALSTACK_API_KEY)or "").strip()
 if not key and raise_if_missing:
  raise VWLTq("Unable to retrieve API key. Please configure $%s in your environment"%ENV_LOCALSTACK_API_KEY)
 return key
def fetch_key():
 ...
 try:
  ...
  result=requests.post("%s/activate"%config.API_URL,json.dumps(data),verify=VWLTS)
  if result.status_code>=400:
   content=result.content
   content_type=result.headers.get("Content-Type")
   raise VWLTq('Received error activating key (code %s): ctype "%s" - %s'%(result.status_code,content_type,content))
  key_base64=json.loads(to_str(result.content))["key"]
  cache_key_locally(api_key,key_base64)
 except VWLTq as e:
  if log_license_issues():
   api_key=VWLTM(api_key_configured()or "")
   api_key_prefix=api_key[:3]
   LOG.warning('Error activating API key "%s..."(%s): %s %s'%(api_key_prefix,VWLTw(api_key),e,traceback.format_exc()))
...
def cache_key_locally(api_key,key_b64):
 ...
 timestamp=VWLTM(VWLTP(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in VWLTr(VWLTw(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 configs["cached_key"]={"timestamp":VWLTP(timestamp),"key_hash":md5(api_key),"key":key_b64}
...
def load_cached_key(api_key):
 ...
 if cached_key.get("key_hash")!=md5(api_key):
  raise VWLTq("Cached key was created for a different API key")
 now=now_utc()
 if(now-cached_key["timestamp"])>MAX_KEY_CACHE_DURATION_SECS:
  raise VWLTq("Cached key expired")
 timestamp=VWLTM(cached_key["timestamp"])
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get("finalized"):
  try:
   key=fetch_key()
   if not key:
    raise VWLTq("Unable to fetch and validate API key from environment")
   if key!="test":
    decrypt_files(key)
...
def api_key_configured():
 return read_api_key(raise_if_missing=VWLTS)
def is_logged_in():
 configs=localstack_config.load_config_file()
...
def get_auth_headers():
 ...
 login_info=configs.get("login")
 if not login_info:
  raise VWLTq("Please log in first")
