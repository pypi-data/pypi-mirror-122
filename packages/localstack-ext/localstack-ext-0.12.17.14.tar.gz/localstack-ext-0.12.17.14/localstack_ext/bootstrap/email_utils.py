...
SENT_EMAILS=[]
EMAIL_BLACKLIST=FfwBj()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",FfwBC(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=FfwBS,from_name=FfwBS,smtp_host=FfwBS,smtp_user=FfwBS,smtp_pass=FfwBS,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if FfwBb(recipients,FfwBR)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=FfwBS):
 ...
 result["From"]=formataddr((FfwBC(Header(from_name,"utf-8")),from_email))
 return result
