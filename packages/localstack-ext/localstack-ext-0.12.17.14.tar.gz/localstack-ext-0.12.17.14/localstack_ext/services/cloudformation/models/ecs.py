...
class ECSService(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return JDUzs(ECSService,self).get_cfn_attribute(attribute_name)
 @JDUzb
 def get_deploy_templates():
...
class ECSTaskDefinition(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return task_defs and{"taskDefinitionArn":task_defs[0]}or JDUzf
 @JDUzb
 def get_deploy_templates():
  return{"create":{"function":"register_task_definition","parameters":lambda_convert_types(lambda_keys_to_lower(),{".memory":JDUzQ,".cpu":JDUzQ})}}
