...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or WIhxk
 @WIhxg
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return WIhxq(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @WIhxL
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if WIhxC(origins,WIhxB)else origins.get("Items")
  origins_to_create=WIhxd([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if WIhxC(aliases,WIhxB)else aliases.get("Items")
  aliases_to_create=WIhxd(aliases)
  for distr in distrs:
   origins=WIhxd([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=WIhxd(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",WIhxX)
   config["Enabled"]=WIhxt(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if WIhxC(custom_errors,WIhxR)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=WIhxt(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":WIhxX}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=WIhxt(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=WIhxk):
    parent=config if parent is WIhxk else parent
    items=parent.get(key)or[]
    if WIhxC(items,WIhxR)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":WIhxv(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if WIhxC(cache_behavs,WIhxR)else cache_behavs)
