...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=xSUWl(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not xSUWd
   return result
  except xSUWI:
   props=self.props
   ...
   if parts[0]in candidates and xSUWk(parts)>1:
