...
def canonicalize_name(name:UmwWK)->UmwWK:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[UmwWu])[0]
 def get_physical_resource_id(self,attribute=UmwWu,**kwargs):
  return self.props.get("Id")
 def get_cfn_attribute(self,attribute_name):
  if attribute_name=="NameServers":
   return[self.props.get("Name")]
  return UmwWi(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @UmwWa
