...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return paHEO if e else paHEi
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->paHEq:
...
@click.option("--revert",is_flag=paHEO,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:paHEV):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(paHEq)
  status.stop()
...
def print_table(columns:Dict[paHEl,paHEl],rows:List[Dict[paHEl,Any]],formatters:Dict[paHEl,Callable[[Any],paHEl]]=paHEq):
 from rich.table import Table
 if formatters is paHEq:
  formatters=paHEQ()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  paHEI(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->paHEq:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->paHEq:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->paHEq:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->paHEq:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
