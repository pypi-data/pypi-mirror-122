...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or sBHXG
 @sBHXn
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return sBHXV(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @sBHXr
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if sBHXq(origins,sBHXi)else origins.get("Items")
  origins_to_create=sBHXa([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if sBHXq(aliases,sBHXi)else aliases.get("Items")
  aliases_to_create=sBHXa(aliases)
  for distr in distrs:
   origins=sBHXa([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=sBHXa(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",sBHXv)
   config["Enabled"]=sBHXM(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if sBHXq(custom_errors,sBHXR)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=sBHXM(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":sBHXv}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=sBHXM(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=sBHXG):
    parent=config if parent is sBHXG else parent
    items=parent.get(key)or[]
    if sBHXq(items,sBHXR)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":sBHXh(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if sBHXq(cache_behavs,sBHXR)else cache_behavs)
