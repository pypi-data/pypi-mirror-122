...
def canonicalize_name(name:tJlDX)->tJlDX:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[tJlDr])[0]
 def get_physical_resource_id(self,attribute=tJlDr,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return tJlDW(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @tJlDw
