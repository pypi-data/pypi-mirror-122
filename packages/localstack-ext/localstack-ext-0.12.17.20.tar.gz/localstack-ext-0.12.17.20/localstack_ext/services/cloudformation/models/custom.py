...
class CustomResource(GenericBaseModel):
 ...
 def get_deploy_templates():
  def create_custom_resource(resource_id,resources,resource_type,func,stack_name,*args):
   ...
   if aRBbY(service_token).startswith("arn:aws:lambda"):
   ...
   result=aRBbQ
   try:
    result=retry(fetch_result,retries=aRBbA(CUSTOM_RESOURCES_RESULT_POLL_TIMEOUT/2),sleep=2)
    result=json.loads(to_str(result))
   except aRBbF:
    bucket_objects=map_all_s3_objects(buckets=[CUSTOM_RESOURCES_RESULTS_BUCKET])
    LOG.info("Unable to fetch CF custom resource result from s3://%s/%s . Existing keys: %s"%(CUSTOM_RESOURCES_RESULTS_BUCKET,result_key,aRBbM(bucket_objects.keys())))
    raise
