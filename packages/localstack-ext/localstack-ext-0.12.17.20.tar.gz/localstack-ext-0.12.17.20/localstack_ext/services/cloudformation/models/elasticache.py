...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=hWJiV(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not hWJil
   return result
  except hWJis:
   props=self.props
   ...
   if parts[0]in candidates and hWJiD(parts)>1:
