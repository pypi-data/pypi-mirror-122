...
SENT_EMAILS=[]
EMAIL_BLACKLIST=uPzlk()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",uPzlM(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=uPzlT,from_name=uPzlT,smtp_host=uPzlT,smtp_user=uPzlT,smtp_pass=uPzlT,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if uPzlm(recipients,uPzlF)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=uPzlT):
 ...
 result["From"]=formataddr((uPzlM(Header(from_name,"utf-8")),from_email))
 return result
