...
class ApiGatewayV2DomainName(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["DomainName"]==domain_name]or[zVfWi])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Authorizer(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["Name"]==auth_name]or[zVfWi])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Api(GenericBaseModel):
 ...
 def fetch_details(cls,api_name):
  ...
  result=([a for a in apis if a["Name"]==api_name]or[zVfWi])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2IntegrationResponse(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[zVfWi])[0]
 def get_physical_resource_id(self,attribute=zVfWi,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return zVfWn(ApiGatewayV2IntegrationResponse,self).get_cfn_attribute(attribute_name)
 @zVfWH
 def get_deploy_templates():
...
class ApiGatewayV2Integration(GenericBaseModel):
 ...
 @zVfWq
 def fetch_details(cls,api_id,zVfWu,method,uri=zVfWi):
  client=aws_stack.connect_to_service("apigatewayv2")
  resp=client.get_integrations(ApiId=api_id)
  if "Items" not in resp:
   return zVfWi
  integrations=[r for r in resp["Items"]if zVfWu==r.get("IntegrationType")and uri in[zVfWi,r.get("IntegrationUri")]and method in[zVfWi,r.get("IntegrationMethod")]]
  return(integrations or[zVfWi])[0]
...
class ApiGatewayV2Stage(GenericBaseModel):
 ...
 def fetch_details(cls,api_id,stage_name):
  client=aws_stack.connect_to_service("apigatewayv2")
  return client.get_stage(ApiId=api_id,StageName=stage_name)or zVfWi
 def get_physical_resource_id(self,attribute,**kwargs):
  if attribute in REF_ID_ATTRS:
