...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=zpKyb,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if zpKyt(perm["IpProtocol"])!=zpKyt(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",zpKyb)
   vpc_id=zpKyb
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or zpKyb
    vpc_id=groups and groups[0].get("VpcId")or zpKyb
   dst_group_id=result.pop("DestinationSecurityGroupId",zpKyb)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=zpKyt(result.get("IpProtocol"))
   description=result.pop("Description",zpKyb)
   cidr_ipv6=result.pop("CidrIpv6",zpKyb)
