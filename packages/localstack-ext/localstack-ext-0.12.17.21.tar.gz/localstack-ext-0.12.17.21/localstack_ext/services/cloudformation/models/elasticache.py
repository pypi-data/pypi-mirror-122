...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=agGyT(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not agGyn
   return result
  except agGyL:
   props=self.props
   ...
   if parts[0]in candidates and agGyW(parts)>1:
