...
class LambdaLayerVersion(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return DWGfQ(LambdaLayerVersion,self).get_cfn_attribute(attribute_name)
 def fetch_state(self,stack_name,resources):
  ...
  return layers[-1]if layers else DWGfx
 @DWGfX
...
class LambdaLayerVersionPermission(awslambda.LambdaPermission):
 ...
 def layer_name_and_version(params):
  ...
  version_number=DWGfi(parts[7]if DWGfy(parts)>7 else 1)
...
class LambdaAlias(awslambda.LambdaPermission):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=[lr for lr in layers if func_version in(DWGfx,lr.get("FunctionVersion"))]
  return(result or[DWGfx])[0]
