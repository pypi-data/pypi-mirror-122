...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or hRVMS
 @hRVMg
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return hRVMd(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @hRVMr
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if hRVMy(origins,hRVMe)else origins.get("Items")
  origins_to_create=hRVMY([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if hRVMy(aliases,hRVMe)else aliases.get("Items")
  aliases_to_create=hRVMY(aliases)
  for distr in distrs:
   origins=hRVMY([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=hRVMY(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",hRVMz)
   config["Enabled"]=hRVMc(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if hRVMy(custom_errors,hRVMX)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=hRVMc(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":hRVMz}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=hRVMc(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=hRVMS):
    parent=config if parent is hRVMS else parent
    items=parent.get(key)or[]
    if hRVMy(items,hRVMX)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":hRVMP(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if hRVMy(cache_behavs,hRVMX)else cache_behavs)
