...
SENT_EMAILS=[]
EMAIL_BLACKLIST=zbWGO()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",zbWGF(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=zbWGd,from_name=zbWGd,smtp_host=zbWGd,smtp_user=zbWGd,smtp_pass=zbWGd,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if zbWGm(recipients,zbWGE)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=zbWGd):
 ...
 result["From"]=formataddr((zbWGF(Header(from_name,"utf-8")),from_email))
 return result
