...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return CyKne if e else CyKnE
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->CyKnA:
...
@click.option("--revert",is_flag=CyKne,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:CyKnH):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(CyKnA)
  status.stop()
...
def print_table(columns:Dict[CyKnX,CyKnX],rows:List[Dict[CyKnX,Any]],formatters:Dict[CyKnX,Callable[[Any],CyKnX]]=CyKnA):
 from rich.table import Table
 if formatters is CyKnA:
  formatters=CyKnc()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  CyKnI(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->CyKnA:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->CyKnA:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->CyKnA:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->CyKnA:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
