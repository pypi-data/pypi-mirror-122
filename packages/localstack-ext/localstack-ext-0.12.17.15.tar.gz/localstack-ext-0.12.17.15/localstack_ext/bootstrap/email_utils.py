...
SENT_EMAILS=[]
EMAIL_BLACKLIST=OteLo()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",OteLl(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=OteLX,from_name=OteLX,smtp_host=OteLX,smtp_user=OteLX,smtp_pass=OteLX,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if OteLu(recipients,OteLf)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=OteLX):
 ...
 result["From"]=formataddr((OteLl(Header(from_name,"utf-8")),from_email))
 return result
