...
class PodInfo:
 def __init__(self,name=LQRbf,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(LQRbs):
 BACKEND="_none_"
 def __init__(self,pod_name=LQRbf,config=LQRbf):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self)->PodInfo:
  raise LQRbj("Not implemented")
 def pull(self):
  raise LQRbj("Not implemented")
 ...
 def get(cls,pod_name,pre_config=LQRbf):
  ...
  raise LQRbj('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=LQRbe
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=LQRbJ)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=LQRbJ
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise LQRbj("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 def get_state_zip_from_instance(self,get_content=LQRbe):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise LQRbj("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:LQRbh=LQRbf):
...
class CloudPodManagerManaged(CloudPodManager):
 ...
 def push(self)->PodInfo:
  ...
  zip_data_content=self.get_state_zip_from_instance(get_content=LQRbJ)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(LQRbT(zip_data_content)/1000.0)))
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise LQRbj("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=LQRbT(zip_data_content)
  return result
 def pull(self):
 ...
 def presigned_url(self,mode):
  ...
  if response.status_code>=400:
   raise LQRbj("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=LQRbJ)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise LQRbj('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=LQRbH(self,"clone_dir",LQRbf)
...
class PodConfigManagerMeta(LQRbo):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=LQRbf
   for manager in cls.CHAIN:
    try:
     tmp=LQRbH(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif LQRbG(tmp,LQRbI)and LQRbG(result,LQRbI):
       result.extend(tmp)
    except LQRbj:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not LQRbf:
    return result
   raise LQRbj('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(LQRbs,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @LQRbd
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise LQRbj('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(LQRbs):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=LQRbJ).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=LQRbA([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=LQRbe):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except LQRbj:
   if safe:
    return{}
   raise LQRbj('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(LQRbs):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise LQRbj("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise LQRbj("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def push_state(pod_name,args,**kwargs):
 pre_config=kwargs.get("pre_config",LQRbf)
...
def pull_state(pod_name,args,**kwargs):
 pre_config=kwargs.get("pre_config",LQRbf)
 if not pod_name:
  raise LQRbj("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull()
 print("Done.")
def reset_local_state():
 ...
 if result.status_code>=400:
  raise LQRbj("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->LQRbh:
 try:
...
def get_persisted_resource_names(data_dir)->List[LQRbh]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and LQRbT(os.listdir(entry.path))>0:
    names.append(entry.name)
