...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return YfbWh if e else YfbWs
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->YfbWH:
...
@click.option("--revert",is_flag=YfbWh,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:YfbWx):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(YfbWH)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:YfbWk):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,YfbWH)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:YfbWk):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,YfbWH)
@pod.command(name="reset",help="Reset the local state to get a fresh LocalStack instance")
def cmd_pod_reset():
 from localstack_ext.bootstrap import pods_client
 pods_client.reset_local_state()
def print_table(columns:Dict[YfbWk,YfbWk],rows:List[Dict[YfbWk,Any]],formatters:Dict[YfbWk,Callable[[Any],YfbWk]]=YfbWH):
 from rich.table import Table
 if formatters is YfbWH:
  formatters=YfbWG()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  YfbWP().__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->YfbWH:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->YfbWH:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->YfbWH:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->YfbWH:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
