...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=BUNmE(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not BUNmR
   return result
  except BUNmO:
   props=self.props
   ...
   if parts[0]in candidates and BUNmr(parts)>1:
