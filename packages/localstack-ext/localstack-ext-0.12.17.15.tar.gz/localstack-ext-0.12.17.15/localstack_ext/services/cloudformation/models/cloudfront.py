...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or BweKO
 @BweKm
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return BweKW(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @BweKE
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if BweKo(origins,BweKr)else origins.get("Items")
  origins_to_create=BweKf([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if BweKo(aliases,BweKr)else aliases.get("Items")
  aliases_to_create=BweKf(aliases)
  for distr in distrs:
   origins=BweKf([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=BweKf(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",BweKx)
   config["Enabled"]=BweKu(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if BweKo(custom_errors,BweKU)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=BweKu(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":BweKx}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=BweKu(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=BweKO):
    parent=config if parent is BweKO else parent
    items=parent.get(key)or[]
    if BweKo(items,BweKU)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":BweKI(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if BweKo(cache_behavs,BweKU)else cache_behavs)
