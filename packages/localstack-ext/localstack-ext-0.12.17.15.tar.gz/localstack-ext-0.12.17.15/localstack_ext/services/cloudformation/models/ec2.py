...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=izNtX,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if izNte(perm["IpProtocol"])!=izNte(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",izNtX)
   vpc_id=izNtX
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or izNtX
    vpc_id=groups and groups[0].get("VpcId")or izNtX
   dst_group_id=result.pop("DestinationSecurityGroupId",izNtX)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=izNte(result.get("IpProtocol"))
   description=result.pop("Description",izNtX)
   cidr_ipv6=result.pop("CidrIpv6",izNtX)
