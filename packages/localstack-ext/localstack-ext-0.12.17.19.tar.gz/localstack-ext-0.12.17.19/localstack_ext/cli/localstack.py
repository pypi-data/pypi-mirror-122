...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return HiecN if e else Hiecy
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->Hiecu:
...
@click.option("--revert",is_flag=HiecN,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:Hieck):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(Hiecu)
  status.stop()
...
def print_table(columns:Dict[Hiecm,Hiecm],rows:List[Dict[Hiecm,Any]],formatters:Dict[Hiecm,Callable[[Any],Hiecm]]=Hiecu):
 from rich.table import Table
 if formatters is Hiecu:
  formatters=Hiech()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  Hiecn(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->Hiecu:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->Hiecu:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->Hiecu:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->Hiecu:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
