...
SENT_EMAILS=[]
EMAIL_BLACKLIST=vFjbW()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",vFjba(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=vFjbs,from_name=vFjbs,smtp_host=vFjbs,smtp_user=vFjbs,smtp_pass=vFjbs,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if vFjbm(recipients,vFjbR)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=vFjbs):
 ...
 result["From"]=formataddr((vFjba(Header(from_name,"utf-8")),from_email))
 return result
