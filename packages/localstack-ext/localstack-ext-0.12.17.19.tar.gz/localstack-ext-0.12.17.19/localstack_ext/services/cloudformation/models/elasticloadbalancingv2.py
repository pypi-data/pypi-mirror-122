...
class ELBV2TargetGroup(GenericBaseModel):
 ...
 def get_physical_resource_id(self,attribute=niyHF,**kwargs):
 ...
 def get_deploy_templates(cls):
  ...
  def modify_target_group_attributes(resource_id,resources,*args,**kwargs):
   ...
   if attrs:
    ...
    group_arn=props.get("TargetGroupArn")
    for attr in attrs:
     attr["Value"]=niyHr(attr.get("Value",""))
...
class ELBV2Listener(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  port_candidates=[niyHr(config.EDGE_PORT),niyHr(config.EDGE_PORT_HTTP)]
  result_filtered=[ls for ls in result if ls.get("LoadBalancerArn")==lb_arn and(niyHr(port or "")in[niyHr(ls.get("Port")),""]or niyHr(ls.get("Port"))in port_candidates)and ls.get("Protocol")==proto]
  if result and not result_filtered:
   LOG.debug("No matching entry when filtering ELBv2 listeners %s for props %s"%(result,props))
  return(result_filtered or[niyHF])[0]
 @niyHz
 def get_deploy_templates():
  def create_params(params,**kwargs):
   ...
   for action in result.get("DefaultActions",[]):
    ...
    config["Port"]=config.get("Port")and niyHr(config["Port"])
   return result
...
class ELBV2LoadBalancer(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return niyHU(ELBV2LoadBalancer,self).get_cfn_attribute(attribute_name)
 @niyHz
 def get_deploy_templates():
