...
def canonicalize_name(name:vIpjr)->vIpjr:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[vIpjw])[0]
 def get_physical_resource_id(self,attribute=vIpjw,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return vIpje(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @vIpja
