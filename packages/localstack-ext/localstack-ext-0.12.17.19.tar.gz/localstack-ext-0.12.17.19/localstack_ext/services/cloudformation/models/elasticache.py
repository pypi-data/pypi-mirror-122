...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=tneIl(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not tneIO
   return result
  except tneIN:
   props=self.props
   ...
   if parts[0]in candidates and tneIm(parts)>1:
