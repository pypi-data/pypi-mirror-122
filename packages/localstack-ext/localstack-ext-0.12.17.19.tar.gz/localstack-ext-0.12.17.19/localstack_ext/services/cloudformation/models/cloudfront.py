...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or wCzEO
 @wCzEA
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return wCzEH(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @wCzEp
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if wCzES(origins,wCzEl)else origins.get("Items")
  origins_to_create=wCzEv([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if wCzES(aliases,wCzEl)else aliases.get("Items")
  aliases_to_create=wCzEv(aliases)
  for distr in distrs:
   origins=wCzEv([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=wCzEv(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",wCzER)
   config["Enabled"]=wCzEu(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if wCzES(custom_errors,wCzEV)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=wCzEu(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":wCzER}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=wCzEu(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=wCzEO):
    parent=config if parent is wCzEO else parent
    items=parent.get(key)or[]
    if wCzES(items,wCzEV)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":wCzEg(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if wCzES(cache_behavs,wCzEV)else cache_behavs)
