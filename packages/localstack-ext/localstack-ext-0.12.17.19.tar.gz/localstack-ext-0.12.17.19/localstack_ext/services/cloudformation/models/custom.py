...
class CustomResource(GenericBaseModel):
 ...
 def get_deploy_templates():
  def create_custom_resource(resource_id,resources,resource_type,func,stack_name,*args):
   ...
   if ScOke(service_token).startswith("arn:aws:lambda"):
   ...
   result=ScOks
   try:
    result=retry(fetch_result,retries=ScOkL(CUSTOM_RESOURCES_RESULT_POLL_TIMEOUT/2),sleep=2)
    result=json.loads(to_str(result))
   except ScOkw:
    bucket_objects=map_all_s3_objects(buckets=[CUSTOM_RESOURCES_RESULTS_BUCKET])
    LOG.info("Unable to fetch CF custom resource result from s3://%s/%s . Existing keys: %s"%(CUSTOM_RESOURCES_RESULTS_BUCKET,result_key,ScOkl(bucket_objects.keys())))
    raise
