...
SENT_EMAILS=[]
EMAIL_BLACKLIST=QyvDf()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",QyvDt(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=QyvDj,from_name=QyvDj,smtp_host=QyvDj,smtp_user=QyvDj,smtp_pass=QyvDj,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if QyvDi(recipients,QyvDV)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=QyvDj):
 ...
 result["From"]=formataddr((QyvDt(Header(from_name,"utf-8")),from_email))
 return result
