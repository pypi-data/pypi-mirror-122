...
class PodInfo:
 def __init__(self,name=adCIz,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(adCIc):
 BACKEND="_none_"
 def __init__(self,pod_name=adCIz,config=adCIz):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self)->PodInfo:
  raise adCIB("Not implemented")
 def pull(self):
  raise adCIB("Not implemented")
 ...
 def get(cls,pod_name,pre_config=adCIz):
  ...
  raise adCIB('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=adCIr
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=adCIl)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=adCIl
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise adCIB("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 def get_state_zip_from_instance(self,get_content=adCIr):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise adCIB("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:adCIg=adCIz):
...
class CloudPodManagerManaged(CloudPodManager):
 ...
 def push(self)->PodInfo:
  ...
  zip_data_content=self.get_state_zip_from_instance(get_content=adCIl)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(adCIQ(zip_data_content)/1000.0)))
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise adCIB("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=adCIQ(zip_data_content)
  return result
 def pull(self):
 ...
 def presigned_url(self,mode):
  ...
  if response.status_code>=400:
   raise adCIB("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=adCIl)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise adCIB('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=adCIe(self,"clone_dir",adCIz)
...
class PodConfigManagerMeta(adCIS):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=adCIz
   for manager in cls.CHAIN:
    try:
     tmp=adCIe(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif adCIF(tmp,adCIy)and adCIF(result,adCIy):
       result.extend(tmp)
    except adCIB:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not adCIz:
    return result
   raise adCIB('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(adCIc,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @adCIj
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise adCIB('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(adCIc):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=adCIl).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=adCIk([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=adCIr):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except adCIB:
   if safe:
    return{}
   raise adCIB('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(adCIc):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise adCIB("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise adCIB("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def push_state(pod_name,args,**kwargs):
 pre_config=kwargs.get("pre_config",adCIz)
...
def pull_state(pod_name,args,**kwargs):
 pre_config=kwargs.get("pre_config",adCIz)
 if not pod_name:
  raise adCIB("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull()
 print("Done.")
def reset_local_state():
 ...
 if result.status_code>=400:
  raise adCIB("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->adCIg:
 try:
...
def get_persisted_resource_names(data_dir)->List[adCIg]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and adCIQ(os.listdir(entry.path))>0:
    names.append(entry.name)
