...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=QbYeU(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not QbYeV
   return result
  except QbYeI:
   props=self.props
   ...
   if parts[0]in candidates and QbYeP(parts)>1:
