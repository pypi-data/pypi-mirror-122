...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or LFAOe
 @LFAOd
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return LFAOp(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @LFAOk
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if LFAOG(origins,LFAOW)else origins.get("Items")
  origins_to_create=LFAOQ([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if LFAOG(aliases,LFAOW)else aliases.get("Items")
  aliases_to_create=LFAOQ(aliases)
  for distr in distrs:
   origins=LFAOQ([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=LFAOQ(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",LFAOt)
   config["Enabled"]=LFAOE(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if LFAOG(custom_errors,LFAOi)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=LFAOE(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":LFAOt}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=LFAOE(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=LFAOe):
    parent=config if parent is LFAOe else parent
    items=parent.get(key)or[]
    if LFAOG(items,LFAOi)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":LFAOB(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if LFAOG(cache_behavs,LFAOi)else cache_behavs)
