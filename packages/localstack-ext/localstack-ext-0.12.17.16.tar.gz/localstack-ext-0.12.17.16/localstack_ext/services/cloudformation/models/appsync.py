...
class AppSyncResolver(GenericBaseModel):
 ...
 def fetch_details(cls,api_id,type_name,field_name,data_source_name):
  ...
  return(result or[FQkKS])[0]
 def get_deploy_templates():
  def get_resolver_params(params,**kwargs):
   params=keys_to_lower(clone(params))
   req_template=params.pop("requestMappingTemplateS3Location",FQkKS)
   res_template=params.pop("responseMappingTemplateS3Location",FQkKS)
   if req_template:
    tmpl=aws_utils.download_s3_object(req_template,as_str=FQkKC)
    if tmpl:
     params["requestMappingTemplate"]=tmpl
   if res_template:
    tmpl=aws_utils.download_s3_object(res_template,as_str=FQkKC)
    if tmpl:
...
class GraphQLSchema(GenericBaseModel):
 ...
 @FQkKq
 def fetch_details(cls,api_id,definition=FQkKS,s3_location=FQkKS):
  client=aws_stack.connect_to_service("appsync")
  try:
   schema=client.get_introspection_schema(apiId=api_id,format="SDL")["schema"]
   schema=schema.read()if FQkKM(schema,"read")else schema
...
class AppSyncApiKey(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return FQkKz(AppSyncApiKey,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute=FQkKS,**kwargs):
  props=self.props
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=[k for k in keys if k.get("description")==props.get("Description")and props.get("Expires")in[FQkKS,k.get("expires")]]
  return(result or[FQkKS])[0]
 @FQkKq
...
class AppSyncDataSource(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return FQkKz(AppSyncDataSource,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute=FQkKS,**kwargs):
  return self.props.get("dataSourceArn")
 def fetch_state(self,stack_name,resources):
  props=self.props
  ...
  return([s for s in sources if s["name"]==ds_name]or[FQkKS])[0]
class AppSyncFunctionConfig(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return FQkKz(AppSyncFunctionConfig,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute,**kwargs):
...
class GraphQLAPI(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  if not props.get("apiId"):
   ...
   if not details:
    raise FQkKg('Unable to find GraphQL API named "%s"'%props.get("Name"))
  ...
  if attribute_name=="GraphQLUrl":
   return appsync_api.get_graphql_url(props["apiId"],use_domain_name=FQkKC)
  return FQkKz(GraphQLAPI,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute,**kwargs):
  props=self.props
 ...
 def fetch_details(cls,api_name):
  ...
  return([a for a in apis if a["name"]==api_name]or[FQkKS])[0]
