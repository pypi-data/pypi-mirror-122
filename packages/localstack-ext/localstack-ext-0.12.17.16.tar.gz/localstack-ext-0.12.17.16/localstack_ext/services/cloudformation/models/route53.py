...
def canonicalize_name(name:WKInv)->WKInv:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[WKInP])[0]
 def get_physical_resource_id(self,attribute=WKInP,**kwargs):
  return self.props.get("Id")
 def get_cfn_attribute(self,attribute_name):
  if attribute_name=="NameServers":
   return[self.props.get("Name")]
  return WKInY(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @WKInm
