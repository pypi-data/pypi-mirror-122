...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return EbzBo if e else EbzBX
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->EbzBG:
...
@click.option("--revert",is_flag=EbzBo,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:EbzBt):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(EbzBG)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:EbzBs):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,EbzBG)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:EbzBs):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,EbzBG)
@pod.command(name="reset",help="Reset the local state to get a fresh LocalStack instance")
def cmd_pod_reset():
 from localstack_ext.bootstrap import pods_client
 pods_client.reset_local_state()
def print_table(columns:Dict[EbzBs,EbzBs],rows:List[Dict[EbzBs,Any]],formatters:Dict[EbzBs,Callable[[Any],EbzBs]]=EbzBG):
 from rich.table import Table
 if formatters is EbzBG:
  formatters=EbzBM()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  EbzBF().__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->EbzBG:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->EbzBG:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->EbzBG:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->EbzBG:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
