...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or VdFhn
 @VdFhu
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return VdFhE(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @VdFhq
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if VdFhA(origins,VdFho)else origins.get("Items")
  origins_to_create=VdFhT([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if VdFhA(aliases,VdFho)else aliases.get("Items")
  aliases_to_create=VdFhT(aliases)
  for distr in distrs:
   origins=VdFhT([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=VdFhT(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",VdFhw)
   config["Enabled"]=VdFht(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if VdFhA(custom_errors,VdFhv)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=VdFht(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":VdFhw}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=VdFht(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=VdFhn):
    parent=config if parent is VdFhn else parent
    items=parent.get(key)or[]
    if VdFhA(items,VdFhv)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":VdFhP(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if VdFhA(cache_behavs,VdFhv)else cache_behavs)
