...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=fpAlH(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not fpAlo
   return result
  except fpAlh:
   props=self.props
   ...
   if parts[0]in candidates and fpAlX(parts)>1:
