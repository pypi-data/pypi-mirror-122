...
class ApiGatewayV2DomainName(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["DomainName"]==domain_name]or[pheXN])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Authorizer(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["Name"]==auth_name]or[pheXN])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Api(GenericBaseModel):
 ...
 def fetch_details(cls,api_name):
  ...
  result=([a for a in apis if a["Name"]==api_name]or[pheXN])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2IntegrationResponse(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[pheXN])[0]
 def get_physical_resource_id(self,attribute=pheXN,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return pheXL(ApiGatewayV2IntegrationResponse,self).get_cfn_attribute(attribute_name)
 @pheXt
 def get_deploy_templates():
...
class ApiGatewayV2Integration(GenericBaseModel):
 ...
 @pheXx
 def fetch_details(cls,api_id,pheXl,method,uri=pheXN):
  client=aws_stack.connect_to_service("apigatewayv2")
  resp=client.get_integrations(ApiId=api_id)
  if "Items" not in resp:
   return pheXN
  integrations=[r for r in resp["Items"]if pheXl==r.get("IntegrationType")and uri in[pheXN,r.get("IntegrationUri")]and method in[pheXN,r.get("IntegrationMethod")]]
  return(integrations or[pheXN])[0]
...
class ApiGatewayV2Stage(GenericBaseModel):
 ...
 def fetch_details(cls,api_id,stage_name):
  client=aws_stack.connect_to_service("apigatewayv2")
  return client.get_stage(ApiId=api_id,StageName=stage_name)or pheXN
 def get_physical_resource_id(self,attribute,**kwargs):
  if attribute in REF_ID_ATTRS:
