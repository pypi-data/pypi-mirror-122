...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=FXRur,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if FXRuC(perm["IpProtocol"])!=FXRuC(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",FXRur)
   vpc_id=FXRur
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or FXRur
    vpc_id=groups and groups[0].get("VpcId")or FXRur
   dst_group_id=result.pop("DestinationSecurityGroupId",FXRur)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=FXRuC(result.get("IpProtocol"))
   description=result.pop("Description",FXRur)
   cidr_ipv6=result.pop("CidrIpv6",FXRur)
