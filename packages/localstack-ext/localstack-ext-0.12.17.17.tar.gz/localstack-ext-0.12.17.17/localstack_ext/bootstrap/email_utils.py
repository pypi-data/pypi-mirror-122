...
SENT_EMAILS=[]
EMAIL_BLACKLIST=epECu()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",epECy(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=epECn,from_name=epECn,smtp_host=epECn,smtp_user=epECn,smtp_pass=epECn,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if epECD(recipients,epECs)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=epECn):
 ...
 result["From"]=formataddr((epECy(Header(from_name,"utf-8")),from_email))
 return result
