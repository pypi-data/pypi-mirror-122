...
class AttrDict(PsvdY):
 def __init__(self,*args,**kwargs):
  Psvdb(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and Psvdr(socket,"IPV6_V6ONLY"):
...
class Half(PsvdD):
 def __init__(self,opt,sock,addr,Psvda):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.Psvda=Psvda
  self.name="peer" if self.Psvda else "client"
  self.queue=[]
  self.dest=Psvdh
  self.err=Psvdh
  self.ready=PsvdI
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={"ip6":PsvdI,"bind_port":2223,"addr":"192.168.43.191","port":22}
 opt={"ip6":PsvdI,"bind_port":48429,"addr":"128.0.100.3","port":22}
 opt={"ip6":PsvdI,"port":54237,"bind_addr":"128.0.100.2","bind_port":22}
 opt={"ip6":PsvdI,"port":54237,"bind_addr":"192.168.100.3","bind_port":22}
 server_loop(opt)
