...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or buzhw
 @buzhD
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return buzhG(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @buzhR
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if buzhF(origins,buzhP)else origins.get("Items")
  origins_to_create=buzhO([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if buzhF(aliases,buzhP)else aliases.get("Items")
  aliases_to_create=buzhO(aliases)
  for distr in distrs:
   origins=buzhO([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=buzhO(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",buzhc)
   config["Enabled"]=buzhp(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if buzhF(custom_errors,buzhI)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=buzhp(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":buzhc}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=buzhp(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=buzhw):
    parent=config if parent is buzhw else parent
    items=parent.get(key)or[]
    if buzhF(items,buzhI)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":buzhx(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if buzhF(cache_behavs,buzhI)else cache_behavs)
