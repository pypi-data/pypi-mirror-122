...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=MmzRn(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not MmzRG
   return result
  except MmzRE:
   props=self.props
   ...
   if parts[0]in candidates and MmzRI(parts)>1:
