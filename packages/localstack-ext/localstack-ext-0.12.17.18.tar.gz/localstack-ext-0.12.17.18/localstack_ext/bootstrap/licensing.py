...
class KeyActivationError(vVMFj):
 def __init__(self,message:vVMFL=vVMFg):
  self.message=message
class CachedKeyError(KeyActivationError):
 pass
class InvalidKeyError(KeyActivationError):
 pass
def read_api_key(raise_if_missing=vVMFf):
 key=(os.environ.get(ENV_LOCALSTACK_API_KEY)or "").strip()
 if not key and raise_if_missing:
  raise vVMFj("Unable to retrieve API key. Please configure $%s in your environment"%ENV_LOCALSTACK_API_KEY)
 return key
def truncate_api_key(api_key:vVMFL):
 return '"%s..."(%s)'%(api_key[:3],vVMFU(api_key))
def fetch_key():
 ...
 try:
  ...
  result=requests.post("%s/activate"%config.API_URL,json.dumps(data),verify=vVMFT)
  ...
  if log_license_issues():
   api_key=vVMFL(api_key_configured()or "")
...
def cache_key_locally(api_key,key_b64):
 ...
 timestamp=vVMFL(vVMFa(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in vVMFJ(vVMFU(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 configs["cached_key"]={"timestamp":vVMFa(timestamp),"key_hash":md5(api_key),"key":key_b64}
...
def load_cached_key(api_key):
 ...
 timestamp=vVMFL(cached_key["timestamp"])
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get("finalized"):
  try:
   key=fetch_key()
   if not key:
    raise vVMFj("Unable to fetch and validate API key from environment")
   if key!="test":
    decrypt_files(key)
...
def api_key_configured():
 return read_api_key(raise_if_missing=vVMFT)
def is_logged_in():
 configs=localstack_config.load_config_file()
...
def get_auth_headers():
 ...
 login_info=configs.get("login")
 if not login_info:
  raise vVMFj("Please log in first")
