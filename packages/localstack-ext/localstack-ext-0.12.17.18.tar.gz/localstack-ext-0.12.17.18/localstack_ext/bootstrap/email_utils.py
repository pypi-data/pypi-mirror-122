...
SENT_EMAILS=[]
EMAIL_BLACKLIST=fABVv()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",fABVj(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=fABVW,from_name=fABVW,smtp_host=fABVW,smtp_user=fABVW,smtp_pass=fABVW,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if fABVI(recipients,fABVw)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=fABVW):
 ...
 result["From"]=formataddr((fABVj(Header(from_name,"utf-8")),from_email))
 return result
