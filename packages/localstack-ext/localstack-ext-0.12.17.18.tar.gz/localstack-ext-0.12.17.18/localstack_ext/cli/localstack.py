...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return uOtqc if e else uOtqI
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->uOtqo:
...
@click.option("--revert",is_flag=uOtqc,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:uOtqL):
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(uOtqo)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:uOtqa):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,uOtqo)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:uOtqa):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,uOtqo)
@pod.command(name="reset",help="Reset the local state to get a fresh LocalStack instance")
def cmd_pod_reset():
 from localstack_ext.bootstrap import pods_client
 pods_client.reset_local_state()
def print_table(columns:Dict[uOtqa,uOtqa],rows:List[Dict[uOtqa,Any]],formatters:Dict[uOtqa,Callable[[Any],uOtqa]]=uOtqo):
 from rich.table import Table
 if formatters is uOtqo:
  formatters=uOtqk()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  uOtqA().__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->uOtqo:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->uOtqo:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->uOtqo:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->uOtqo:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
