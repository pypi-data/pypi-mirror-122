...
from typing import MutableMapping, List, Union, Coroutine
...
class BaseExecutor(Module):
    ...
    async def _loop(self):
        ...
        """
        subscribers = []
        for c_name in self.settings.listen_on:
            try:
                s = self.worker.get_subscriber(c_name)
                s.name = c_name
                subscribers.append(s)
            except KeyError as e:
                self.logger.error(f"Can't listen on client {e}. No such client.")

        # future of termination request
        term_future = asyncio.ensure_future(self._terminate_request.wait())
        msg_waiters = {asyncio.ensure_future(c.get()): c for c in subscribers}

        self.logger.debug(f"Listening on {len(msg_waiters)} subscribers: {', '.join(c.name for c in subscribers)}")

        while True:
            if not msg_waiters:
                self.logger.error(f"No subscribers to listen on, executor can't continue")
                raise RuntimeError("No subscribers to listen on")

            try:
                done, pending = await asyncio.wait(tuple(msg_waiters.keys()) + (term_future,),
                ...
                for fut in done:
                    ...
                    # remove completed future from the waiters map
                    client = msg_waiters.pop(fut)

                    if await self._handle_done_waiter(fut, client):
                        # re-append waiter for the client
                        msg_waiters[asyncio.ensure_future(client.get())] = client
    ...
    async def handle(self, task: Task, receiver: AsyncSubscriber) -> Union[asyncio.Future, None]:
        ...
        try:
            ...
            raise TaskFatal(task, reason="No processor to handle this message type") from e
