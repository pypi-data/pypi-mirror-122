...
class TionZonesDevicesData:
    def __init__(self, data: dict):
        ...
        self.heater_mode = data.get("heater_mode")  # 4S: heat, maintenance
        self.heater_type = data.get("heater_type")  # 4S: 1.4kvt
...
class TionApi:
    def __init__(self, email: str, password: str, auth_fname="tion_auth", min_update_interval_sec=10):
        ...
        self._last_update = 0
        self._data: Tion = []
        self._data_raw = None  # for debugging
        self.get_data()
    ...
    def get_data(self, force=False) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                self._data_raw = locations = response.json()
...
class Breezer(TionZonesDevices):
    ...
    def send(self, extra_data: dict = None) -> bool:
        ...
        data = {
            ...
            "heater_mode": "heat" if self.heater_enabled else "maintenance",  # 4S model support
    ...
    def load(self, device_data: TionZonesDevices=None, force=False):
        ...
        if device_data:
            ...
            self._heater_installed = data.heater_installed or data.heater_type is not None  # 4S support (heater_type)
            self.heater_enabled = data.heater_enabled or data.heater_mode == "heat"  # 4S support (heater_mode)
