...
def option_comm_trend_china(contract,start='',end='',power=0,twinx=False):
    ...
    #绘制单折线
    if contract2=='':
        contract1,dict1=option_contract_decode_china(contract1)
        try:
    ...
    if not (contract2==''):
        ...
        contract2,dict2=option_contract_decode_china(contract2)
        try:
...
def option_contract_decode_china(contract):
    """
    例：
    ...
    try:
        ...
        print("  #Error(option_contract_decode_china): contract",contract,"not found")
        return None,None
