...
class QuaSerializingVisitor(QuaNodeVisitor):
    ...
    def enter_qm_grpc_qua_QuaProgram_ForEachStatement(self, node):
        variables = []
        arrays = []
        ...
        self._enter_block(
            f'with for_each_(({",".join(variables)}),({",".join(arrays)})):'
        )
        return True
...
def _measure_statement(node):
    ...
    if len(node.measureProcesses) > 0:
        for process in node.measureProcesses:
            process_type_desc, process_type = process.ListFields()[0]
            if process_type_desc.name == "analog":
                desc, value = process_type.ListFields()[0]
                if desc.name == "demodIntegration":
                    target = ExpressionSerializingVisitor.serialize(
                        value.target.scalarProcess
                    )
                    args.append(f'("{value.integration.name}",{target})')
                elif desc.name == "bareIntegration":
                    args.append(f"()")
                elif desc.name == "rawTimeTagging":
                    args.append(f"()")
