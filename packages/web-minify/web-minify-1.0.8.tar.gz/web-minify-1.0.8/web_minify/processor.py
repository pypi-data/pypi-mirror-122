...
class Processor:
    def __init__(self, settings: dict):
        ...
        self.mode: bool = settings.get("mode", "minify")
        ...
        self.valid_extensions = [f".{ext}" for ext in self.handlers.keys()]
        if self.verbose:
    ...
    def sanity_checks(self):
        valid_modes = ["minify", "beautify"]
        if not self.mode in valid_modes:
            return (False, f"The specified mode {mode} was not valid. NOTE: Valid modes are {pprint.pformat(valid_modes)}")
    ...
    def prepare_handlers(self):
        self.handlers={}
        handlers_mod = importlib.import_module('.handlers', 'web_minify')
        for name, cls in handlers_mod.__dict__.items():
            if isinstance(cls, type):
                handler = cls(self.settings)
                if not handler:
                    logger.warning(f"Could not instanciate handler '{name}', skipping...")
                    continue
    ...
    def get_handler(self, extension):
        return self.handlers.get(extension, None)
    ...
    def process_file(self, input_path: str, output_path: str):
        ...
        handler_errors = None
        was_copied = False
        do_copy = handler is None or vip_file or (extension in ["js", "css", "sass", "html", "svg", "png", "jpeg"] and self.settings.get(f"disable_{extension}", False))
        if do_copy:
            # Perform copy
            ...
            tup=handler.process(original_content)
            logger.info(f"TUP: ({tup}) with {handler.extensions()}")
            _processed_content, handler_errors = tup
        ...
        if handler_errors:
            return False, False, False, ", ".join(handler_errors)
