...
def parse_arguments(defaults):
    ...
    web-minify version {__version__}
...
""",
    ...
    _option_group_general.add_argument("--mode", type=Mode, choices=list(Mode), help="Select mode of operation. Minify will prepare files for deployment, beautify will prepare files for development.", default=defaults.get("minify"))
    ...
    _option_group_general.add_argument("--gzip", action="store_true", help="GZIP Processed files as '*.gz'.", default=defaults.get("gzip"))
...
def main():
    ...
    try:
        ...
        settings["mode"] = mode.value if type(settings["mode"]) is Mode else mode
