...
def ip_address(*args, **kwargs):
    """ Returns a string that represents the ip address
        ...
        Parameters:
            interface: The network interface. Use `ifconfig` (Mac OS X, linux) or `ipconfig` (Windows) to check the interface.
        Example:
            "app_specific_config": {
                "function": "ip_address",
                "parameters": {
    ...
    if _platform.system() == 'Darwin':
        try:
            ...
            # Ensure <= 1 address is returned, as a string, matching other systems
            addr = addrs.get(netifaces.AF_INET, [{}])[0]
            ipaddress = addr.get('addr')
            LOG.debug("On Darwin system, found IP addresses for '{}': {}"
                    .format(interface, ipaddress))
        except ImportError:
            error_message = "Cannot run on Darwin system without 'netifaces' package installed."
            LOG.critical(error_message)
            raise ImportError(error_message)
        except Exception as error_message:
            raise
    else:
        b_interface = bytes(interface, 'ascii')
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        try:
            ipaddress = socket.inet_ntoa(fcntl.ioctl(
                s.fileno(),
                0x8915,  # SIOCGIFADDR
                struct.pack('256s', b_interface[:15]))[20:24])
        except IOError as err:
            if err.errno == 19:
                msg = "ip_address: {0} not configured.".format(b_interface)
                ipaddress = msg
                LOG.warning(msg)
            elif err.errno == 99:
                msg = "ip_address: {0} not connected.".format(b_interface)
                ipaddress = msg
                LOG.error(msg)
            else:
                msg = "ip_address: {!r}: {!r}".format(b_interface, err)
                ipaddress = msg
                LOG.error(msg)
