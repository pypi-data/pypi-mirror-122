...
def preprocessdata(click):
    ...
    for i in list_of_names:
        ...
        try:
            ...
            df = ratparser.generate_frame_from_file(str(packagepath) + cachepath + f'{i}')
            # something's going wrong here when reading file back in from cache
            # if parser.verified:
            #     df = parser.dataframe
    ...
    try:
        ...
        for i in filenames:
            ...
            df1 = df1.drop_duplicates(subset=['llc_trigger_count'])
            for j in filenames:
                if j != i:
                    df2 = pd.read_feather(str(packagepath) + dfpath + f'{j}.feather')
                    df2 = df2.drop_duplicates(subset=['llc_trigger_count'])
