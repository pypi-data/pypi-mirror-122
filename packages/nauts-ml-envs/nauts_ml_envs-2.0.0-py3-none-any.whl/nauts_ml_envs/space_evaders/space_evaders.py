...
meteorite_file = os.path.join(img_folder, "meteorBrown_med1.png")
explosion_file = os.path.join(img_folder, "regularExplosion01.png")
...
class env:
    def __init__(self,num_enemies = 4, num_rounds = 10):
        #Set up screen
    ...
    def get_state_space_high(self):
        obs = [ENV_WIDTH]+([ENV_WIDTH]*self.num_enemies)
        return np.array(obs, dtype=float)
    
    def get_state_space_low(self):
        obs = [0]+([-ENV_WIDTH]*self.num_enemies)
        return np.array(obs, dtype=float)
		
    def get_state_space_size(self):
        discrete_os_win_size = [10]+[20]*(self.num_enemies)
