...
class TestServicesDelete:
    ...
    @patch(
        "sym.flow.cli.helpers.api.SymAPI.get_service", return_value=MOCK_SLACK_SERVICE.id
    )
    ...
    def test_services_delete(
        self, mock_pre_delete_hooks, mock_delete_service, mock_get_service, click_setup
    ):
    ...
    @pytest.mark.parametrize("service_type", [s.value for s in ServiceType])
