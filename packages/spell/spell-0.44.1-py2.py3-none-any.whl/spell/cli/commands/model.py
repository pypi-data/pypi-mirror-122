...
from spell.cli.utils import prettify_time, tabulate_rows, convert_to_local_time
...
@click.option(
    "--dest",
    "-d",
    help="Directory to download to. Defaults to the current directory.",
    default="./"
)
def download(ctx, name, dest):
    ...
    if not version:
        raise ExitException(
            "A model must be specified in the form model_name:version. Version cannot be omitted."
        )
    client = ctx.obj["client"]
