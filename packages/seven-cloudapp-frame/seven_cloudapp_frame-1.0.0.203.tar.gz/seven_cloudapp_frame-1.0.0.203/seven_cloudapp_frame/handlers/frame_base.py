...
@LastEditTime: 2021-10-09 14:54:55
...
class ClientBaseHandler(FrameBaseHandler):
    ...
    def _convert_request_params(self):
        ...
        :last_editors: HuangJianYi
        """
        client_check_params = config.get_value("client_check_params", False)
        server_check_params = config.get_value("server_check_params", False)
        if ("/server/" in self.request.url and server_check_params) or ("/client/" in self.request.url and client_check_params):
            ...
            if self.request.method == "POST" and "Content-Type" in self.request.headers and self.request.headers["Content-type"].lower().find("application/json") >= 0 and self.request.body:
                try:
                    ...
                    self.response_json_error("param_error", "参数错误")
                    return
                if json_params:
                    if json_params.__contains__("par") and json_params.__contains__("dv"):
                        iv = app_id[0, 10] + str(json_params["dv"])[0:6] if len(str(json_params["dv"]))>=6 else ""
                        json_params = json.loads(CryptographyHelper.aes_decrypt(str(json_params["par"]), password, iv))
                ...
                if "par" in self.request.arguments and "dv" in self.request.arguments:
                    ...
                    json_params = json.loads(CryptographyHelper.aes_decrypt(par, password, iv))
                for field in json_params:
                    self.request_params[field] = json_params[field]
        else:
            if self.request.method == "POST" and "Content-Type" in self.request.headers and self.request.headers["Content-type"].lower().find("application/json") >= 0 and self.request.body:
...
def filter_check_params(must_params=None):
    ...
    def check_params(handler):
        def wrapper(self, **args):
            ...
            if self.request_params.__contains__("timestamp") and (now_time - self.request_params["timestamp"] > int(1000 * 60 * 10)):
