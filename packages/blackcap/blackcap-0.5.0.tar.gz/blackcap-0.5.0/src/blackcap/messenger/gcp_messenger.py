...
class GCPMessenger(BaseMessenger):
    ...
    def publisher(self) -> pubsub_v1.PublisherClient:
        """Publisher Client."""
        pub_credentials = jwt.Credentials.from_service_account_info(
            self.service_account_info, audience=self.pub_audience
        )
        return pubsub_v1.PublisherClient(credentials=pub_credentials)

    @property
    def subscriber(self) -> pubsub_v1.SubscriberClient:
        """Subscriber Client."""
        sub_credentials = jwt.Credentials.from_service_account_info(
            self.service_account_info, audience=self.sub_audience
        )
        return pubsub_v1.SubscriberClient(credentials=sub_credentials)
