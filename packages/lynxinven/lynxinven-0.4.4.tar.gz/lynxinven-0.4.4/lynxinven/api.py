...
"""
The lynxinven_api module handles low level requests and authentication
with the LynxInven database server.
"""
...
logger = logging.getLogger('lynxinven')


class LynxInvenAPI(object):
    """
    Basic class for performing LynxInven API requests.
    """
    ...
    def getMinApiVersion():
        ...
        return LynxInvenAPI.MIN_SUPPORTED_API_VERSION

    def __init__(self, base_url, **kwargs):
        """ Initialize class with initial parameters

        Args:
            base_url - Base URL for the LynxInven server, including port (if required)
                       e.g. "http://lynxinven.server.com:8000"
        ...
        if not self.testServer():
            raise ConnectionRefusedError("Could not connect to LynxInven server")
    ...
    def testServer(self):
        ...
        The LynxInven server provides a simple endpoint at /api/
        which contains some simple data (and does not require authentication)
        """

        self.server_details = None

        logger.info("Checking LynxInven server connection...")
        ...
        logger.info("LynxInven server details: " + str(response.text))

        # The details provided by the server should include some specific data:
        server_name = str(self.server_details.get('server', ''))

        if not server_name.lower() == 'lynxinven':
            logger.warning("Server returned strange response (expected 'LynxInven', found '{name}')".format(
                name=server_name
            ))
        ...
        if api_version < LynxInvenAPI.getMinApiVersion():
            raise ValueError(f"Server API version ({api_version}) is older than minimum supported API version ({LynxInvenAPI.getMinApiVersion()})")
    ...
    def request(self, url, **kwargs):
        """ Perform a URL request to the LynxInven API """
    ...
    def downloadFile(self, url, destination):
        """
        Download a file from the LynxInven server.
