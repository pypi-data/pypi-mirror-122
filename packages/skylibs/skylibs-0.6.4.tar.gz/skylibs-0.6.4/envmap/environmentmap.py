...
class EnvironmentMap:
    ...
    def embed(self, vfov, rotation_matrix, image, projection="perspective"):
        image = image.copy()
        h, w, c = image.shape
        resolution = [w, h]
        ar = w/h
        ...
        x, y, z, _ = target.worldCoordinates()
        xyz = np.dstack((x, y, z))
        ...
        u = (np.round(u*(self.data.shape[1] - 1))).astype('int')
        v = (np.round(v*(self.data.shape[0] - 1))).astype('int')
        uv = np.dstack((u, v))

        iu, iv = np.mgrid[0:image.shape[0], 0:image.shape[1]]

        su, sv = self.imageCoordinates()
 
        # raster
        acc = np.zeros_like(self.data, dtype=np.float32)
        du = np.diff(u, axis=1).max()
        dv = np.diff(v, axis=0).max()

        self.data[:] = np.nan
        for dist_v in sorted(range(dv), key=abs):
            i = dist_v - dv//2
            for dist_u in sorted(range(du), key=abs):
                j = dist_u - du//2
                ve = ((v.copy() + i) % self.data.shape[0]) - i
                ue = ((u.copy() + j) % self.data.shape[1]) - j
                mask3 = np.isnan(self.data[ve+i,ue+j,:])
                mask = np.logical_or.reduce((mask3[:,:,0], mask3[:,:,1], mask3[:,:,2]))
                ve = ve[mask]
                ue = ue[mask]
                self.data[ve+i,ue+j,:] = image[mask]

        mask3 = np.isnan(self.data)
        mask = np.logical_or.reduce((mask3[:,:,0], mask3[:,:,1], mask3[:,:,2]))
        self.data[mask] = self.backgroundColor[np.newaxis,:]
