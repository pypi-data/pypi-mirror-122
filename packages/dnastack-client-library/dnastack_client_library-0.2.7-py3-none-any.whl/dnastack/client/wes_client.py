...
def submit_workflow(
    ...
    if attachment_file:
        with open(attachment_file, "rb") as file:
            attachment_file_contents = file.read()
            file.close()
    ...
    if attachment_file_contents:
        files["workflow_attachment"] = (workflow_url, attachment_file_contents)

    if input_params:
        files["workflow_params"] = (None, input_params, "application/json")

    if engine_param:
        files["workflow_engine_parameters"] = (None, engine_param, "application/json")

    if engine_params_file_contents:
        files["workflow_engine_parameters"] = (
            None,
            engine_params_file_contents,
            "application/json",
        )

    if tag:
        files["tags"] = (None, tag, "application/json")

    if tags_file_contents:
        files["tags"] = (None, tags_file_contents, "application/json")
