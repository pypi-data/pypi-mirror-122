# Copyright (C) 2010-2019 by Dr. Dieter Maurer <dieter@handshake.de>
...
class Object(ObjectBase):
  ...
  def _validate(self, value):
    ...
    if self.check_declaration: validate = base_validate
    else:
      # suppress the declaration check via a proxy
      #   the suppression works by forcing the check to unconditionally succeed
      try:
        schema_proxy = make_proxy(self.schema, providedBy=lambda unused: True)
      except TypeError: # this fails for some versions of `zope.interface`
        from copy import copy
        schema_proxy = copy(self.schema)
        schema_proxy.providedBy = lambda unused: True
      validate = make_proxy(self, schema=schema_proxy)._rebind_proxied_method(base_validate)
    validate(value)
    

##########################################################################
## Additional fields -- added in version 3.1

class IFilesystemPath(ITextLine):
  """An absolute or relative path in the filesystem.

  May need to be properly encoded for the actual file system access
  (which Python hopefully does automatically).
  """

@implementer(IFilesystemPath)
class FilesystemPath(TextLine):
  """Field holding a relative or absolute path in the filesystem."""


class IIriRef(ITextLine):
  """an Internanationalized Resource Identifier (= IRI) reference (see RFC 3987).

  also known as `anyURI` by XML-schema.
  """

from re import compile
iri_re = compile(u"(?:"
                 # `iunreservered` (without `ucschar`) + `gen-delims` + `sub-delims`
                 u"[-a-zA-Z0-9._~:/?#[\\]@!$&'()*+,;=]|" # 
                 # `pct-encoded`
                 u"(?:%[0-9a-zA-Z]{2})|"
                 # a superset of `ucschar`
                 u"[^\x00-\x9f]"
                 u")*$")


@implementer(IIriRef)
class IriRef(TextLine):
  """Field holding an IRI reference (see RFC 3987)."""
  def _validate(self, value):
    """currently checks only that *value* does not contain invalid characters."""
    super(IriRef, self)._validate(value)
    if iri_re.match(value) is None:
      raise ValidationError(
        # may want internationalization below
        self.__name__, 'the IriRef contains invalid characters'
