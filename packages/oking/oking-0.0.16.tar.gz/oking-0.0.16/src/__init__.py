...
print('Obtendo configuracoes do logger')
# print('path.abspath(__file__)' + path.abspath(__file__))
# log_file_path = path.join(path.dirname(path.abspath(__file__)), 'logging.conf')
# print('log_file_path' + log_file_path)
# config.basicConfig(log_file_path, disable_existing_loggers=False)

logging.basicConfig(level=logging.DEBUG,
                    format='[%(levelname)s][%(asctime)s]| %(message)s',
                    datefmt='%Y-%m-%d %I:%M:%S')
logger = logging.getLogger()
logger.info('Iniciando oking __init__.py')
