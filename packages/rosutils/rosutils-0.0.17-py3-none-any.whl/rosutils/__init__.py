...
def write_install_system_dependencies_scripts(packages, filename, output_root, relpath=None, standalone=False):
    with open(filename, "w") as file:
        ...
        for package in packages:
            ...
            for i, filename in enumerate(scripts):
                ...
                if standalone:
                    ...
                    # Copy the file into the output root
                    os.makedirs(output_root, exist_ok=True)
                    ext = os.path.splitext(filename)[1]
                    output_filename = "%s-%02d%s" % (package.name, i, ext)
