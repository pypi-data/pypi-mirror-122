    ...
    if validated:
        ...
        try:
            ...
            if _isPy:
                ...
                if isWindows:
                    print(f"looking for chatbot.py in windows - directory - {current_dir}")

                    _chatbot_p = subprocess.Popen(shlex.split(
                        r'"python3" chatbot.py -p ' + str(p)), stderr=subprocess.STDOUT, shell=True)
                else:
                    print(f"looking for chatbot.py in directory - {current_dir} - unix-like")
                    _chatbot_p = subprocess.Popen(shlex.split(
                        r'"python3" chatbot.py -p ' + str(p)), stderr=subprocess.STDOUT)
