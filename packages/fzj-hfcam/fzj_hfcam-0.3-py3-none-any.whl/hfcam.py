...
def default_view(view, scale = 1, tqdm = notqdm):
    ...
    if view == 'divertor':
    ...
    elif view == 'divertor_and_baffles':
        phi = np.radians(-5)
        theta = np.radians(60)
        r_origin = 5.5
        z_origin = -0.4
        ...
        raise ValueError('view not in [divertor, divertor_pretty, divertor_and_baffles, uport, rbaffles, lbaffles, cbafflesi, cbaffleso]')
    ...
    if view == 'divertor_and_baffles':
        useids = [165, 320]
    elif 'baffles' in view:
        useids = [320]
