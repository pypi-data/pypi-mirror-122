...
class NWService:
    ...
    def __init__(self, context: "NWServiceContext", config_path: str):
        ...
        try:
            ...
            for task in [*self.tasks.values(), *self.approvals.values()]:
                task.requires += self.get_depending_references_for(task.id)
    ...
    def get_depending_references_for(self, id: str) -> List[str]:
...
class NWTask:
    ...
    async def is_changed_async(self) -> bool:
        ...
        if self._is_changed is None:
            ...
            if len(self.service.get_depending_references_for(self.id)) == 0:
