...
def add_numbering_for_pdf(pdf_file, new_pdf_file_name=None, location_on_screen="center", start_page=1, end_page=None,
    ...
    parameters = get_parameters_for_numbering(original_pdf, location_on_screen, start_page, end_page, start_index, size,
                                              font)
    empty_numbered_pdf = create_empty_numbered_pdf(**parameters)
    new_pdf_file = merge_pdf_pages(original_pdf, empty_numbered_pdf)
    if new_pdf_file_name:
        save_file(new_pdf_file, new_pdf_file_name)
    return new_pdf_file
...
def get_parameters_for_numbering(original_pdf, location_on_screen, start_page, end_page, start_index, size,
                                 font) -> dict:
    """Setting parameters for numbering"""
    return {
        "width_of_pages": get_width_of_pages(original_pdf, location_on_screen),
        "height": 15 * mm,
...
def get_width_of_pages(original_pdf, location_on_screen) -> list:
    """Returns width of pages"""
    width_of_pages = []
    for index in range(original_pdf.getNumPages()):
        ratio = original_pdf.getPage(index).mediaBox.getWidth() / 200
        width = location_on_screen_to_width[location_on_screen] * ratio * mm
...
def save_file(new_pdf_file, new_pdf_file_name) -> None:
    """Saves file with new file name"""
    with open(new_pdf_file_name, "bw") as file:
