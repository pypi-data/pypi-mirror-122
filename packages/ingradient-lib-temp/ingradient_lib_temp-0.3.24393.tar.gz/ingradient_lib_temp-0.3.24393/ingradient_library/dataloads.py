...
class DataLoader3D(object):
    ...
    def get_oversample_patch(self, non_zero_coords, shape_to_index):
        oversample_high = np.clip(np.max(non_zero_coords, axis = 1) + 1, self.patch_size/2 - 1, shape_to_index - self.patch_size/2).reshape(1,3)
        oversample_low = np.clip(np.min(non_zero_coords, axis = 1), self.patch_size/2 - 1, shape_to_index - self.patch_size/2).reshape(1,3)
        print(oversample_low.shape, oversample_high.shape, type(oversample_high), type(oversample_low))
        oversample_center = np.random.randint(low = oversample_low.astype(int), high = (oversample_high.astype(int) + 1))
