...
def point_sampler(target, output = None, positive_points = None, negative_points = None):
    ...
    for batch_index in range(bs):
        if (prev_seg[batch_index] != target[batch_index]).sum() == 0:
            ...
            n_candidates = len(candidates[0])
            index = torch.random.randint(low = 0, high = n_candidates, size = (1,))
            negative_points[batch_index, n_candidates[-3][index], n_candidates[-2][index],n_candidates[-1][index]] = 1
            ...
            n_candidates = len(candidates[0])
            index = torch.random.randint(low = 0, high = n_candidates, size = (1,))
            positive_points[batch_index, n_candidates[-3][index], n_candidates[-2][index],n_candidates[-1][index]] = 1
