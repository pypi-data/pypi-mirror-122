...
class CustomDatasetV2(CustomDataset):
    def __init__(self, root_dir, device = None, normalizer = None, mode = 'train', dim = '3d', correct_direction = False, resample = None,
        ...
        super().__init__(root_dir, device = None, normalizer = None, mode = 'train', dim = '3d', correct_direction = False, resample = None)
    ...
    def get_oversample_patch(self, images, seg):
        a = torch.arange(2*2*15).reshape(2,2,3,5)
        b = torch.where(a != 0)
        torch.max(torch.vstack(b), axis = 1)[0]
