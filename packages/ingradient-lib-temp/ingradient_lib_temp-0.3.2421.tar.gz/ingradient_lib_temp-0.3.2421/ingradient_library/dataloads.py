...
class CustomDataset(Dataset):
    ...
    def get_maximum_patch_size(self):
        ...
        for idx in range(self.__len__()):
            ...
            if self.mode == 'test':
                x,  _ =  self.__getitem__(idx)

            shape = torch.tensor((x.shape[-3], x.shape[-2], x.shape[-1]))
            result = torch.max(torch.vstack((result, shape)), dim = 0)[0]
    ...
    def save_files(self, save_path):
        for i in range(self.__len__()):
            if self.mode == 'train':
                ...
                np.savez(save_path + '_train' + str(i) + '.npz', x = x, y= y )
                save_path = os.path.join(self.SAVE_PATH, self.ID+str(i)+'_info.pkl')
                pickle_file = open(save_path + '_' + str(i) + '.pkl', 'wb')
                pickle.dump(info_data, pickle_file)
                pickle_file.close()
            else:
                x, info_data = self.__getitem__(i)
                np.savez(save_path + '_test' + str(i) + '.npz', x = x)
                save_path = os.path.join(self.SAVE_PATH, self.ID+str(i)+'_info.pkl')
                pickle_file = open(save_path + '_' + str(i) + '.pkl', 'wb')
                pickle.dump(info_data, pickle_file)
                pickle_file.close()
...
class DataLoader3D(object):
    ...
    def __init__(self, dataset, num_iteration = 2, patch_size = (1024,512,512), transform= None, device = 0, batch_size = 2, seg_one_hot = False, is_half = False, info_class = None):
...
class Interactive_DataLoader(DataLoader3D):
    def __init__(self, dataset, num_iteration = 1, patch_size = (1024, 512, 512), batch_size = 2, device = None, transform = None):
        self.dataset = dataset
        self.current_index = 0
        self.patch_size = np.array(patch_size)
        self.images, self.seg, self.info = self.get_image_seg(self.current_index)
