...
class DataLoader3D(object):
    ...
    def get_oversample_patch(self, non_zero_coords, shape_to_index):
        ...
        print(oversample_low.shape, oversample_high.shape, type(oversample_high), type(oversample_low), oversample_low, oversample_high)
        oversample_center = np.random.randint(np.int(oversample_low), np.int(oversample_high))
