...
class Daegu_Model10(nn.Module):
    def __init__(self, patch_size, resnet_output = 16):
        super().__init__()
        ...
        self.upsample1 = nn.Sequential(nn.ConvTranspose3d(in_channles = 16, out_channels= out_tconv1, kernel_size =3 , stride = 2, padding = 1, output_padding =1),
                                       nn.InstanceNorm3d(out_tconv1),
                                       nn.LeakyReLU())
        
        self.upsample2 = nn.Sequential(nn.ConvTranspose3d(in_channles = out_tconv1, out_channels= out_tconv2, kernel_size =3 , stride = 2, padding = 1, output_padding =1),
                                       nn.InstanceNorm3d(out_tconv2),
                                       nn.LeakyReLU())
        
        self.final_layer = nn.Sequential(nn.Conv3d(in_channels = out_tconv2, out_channels = 2, kernel_size = 3, stride = 1, padding = 6, dilation = 6),
                                   nn.InstanceNorm3d(2),
                                   nn.LeakyReLU())
