        ...
        for e in range(self.n_epoch):
            ...
            while not self.tr_dl.is_end():
                ...
                for i in range(self.n_point):
                    self.optimizer.zero_grad()
                    positive_points, negative_points = point_sampler(seg, output, positive_points, negative_points)
                    output = self.model(images, positive_points, negative_points, output)
                    loss = self.calculate_loss(output, seg, writer, e)
                    train_loss = loss.item()
                    if self.tr_dl.current_index % visualization_ineterval == 0:
                        print("points : ",i + 1)
