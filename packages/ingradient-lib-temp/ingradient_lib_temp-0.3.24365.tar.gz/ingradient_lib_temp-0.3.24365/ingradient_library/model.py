...
class Daegu_Model10(nn.Module):
    def __init__(self, patch_size):
        super().__init__()
        kwargs = {'sample_input_D':patch_size[0], 'sample_input_H':patch_size[1], 'sample_input_W':patch_size[2],'num_seg_classes':2}
        self.model = resnet10(**kwargs)
        self.final_upsample = nn.Upsample(scale_factor=4, mode = 'trilinear')
...
class Daegu_Model34(nn.Module):
    def __init__(self, patch_size, input_channel = 1, output_channel= 2):
        ...
        out_channel3 = out_channel2 * 2
        kwargs = {'sample_input_D':patch_size[0], 'sample_input_H':patch_size[1], 'sample_input_W':patch_size[2],'num_seg_classes':out_channel2}


        self.a_conv1 = nn.Sequential(nn.Conv3d(in_channels = input_channel, out_channels = out_channel1, kernel_size = 3, stride = 2, padding = 6, dilation = 6),
                                   nn.InstanceNorm3d(out_channel1),
                                   nn.LeakyReLU())
        self.a_conv2 = nn.Sequential(nn.Conv3d(in_channels = input_channel, out_channels = out_channel1, kernel_size = 3, stride = 2, padding = 12, dilation = 12),
                                   nn.InstanceNorm3d(out_channel1),
                                   nn.LeakyReLU())
        self.a_conv3 = nn.Sequential(nn.Conv3d(in_channels = input_channel, out_channels = out_channel1, kernel_size = 3, stride = 2, padding = 18, dilation = 18),
                                   nn.InstanceNorm3d(out_channel1),
                                   nn.LeakyReLU())
        self.a_conv4 = nn.Sequential(nn.Conv3d(in_channels = input_channel, out_channels = out_channel1, kernel_size = 1, stride = 2, padding = 0, dilation = 1),
                                   nn.InstanceNorm3d(out_channel1),
                                   nn.LeakyReLU())
        ...
        kwargs = {'sample_input_D':patch_size[0], 'sample_input_H':patch_size[1], 'sample_input_W':patch_size[2],'num_seg_classes':out_channel2}
        self.resnet = resnet10(**kwargs)
        self.resnet_upsample = nn.Upsample(scale_factor=4, mode = 'trilinear')
        self.asp_upsample = nn.Upsample(scale_factor=2, mode = 'trilinear')

        self.last_layer = nn.Sequential(nn.Conv3d(in_channels = out_channel3, out_channels = output_channel, kernel_size = 1, stride = 1, padding = 0, dilation = 1),
    ...
    def forward(self, X):
        ...
        res_X = self.resnet(X)
        res_X = self.resnet_upsample(res_X)
        asp_X = torch.cat((X1,X2,X3,X4), dim = 1)
        asp_X = self.a_conv2_1(asp_X)
        asp_X = self.asp_upsample(asp_X)
        print(asp_X.shape, res_X.shape)
        X = torch.cat((res_X, asp_X), dim = 1)
        X = self.last_layer(X)
        return torch.softmax(X, dim = -1)
