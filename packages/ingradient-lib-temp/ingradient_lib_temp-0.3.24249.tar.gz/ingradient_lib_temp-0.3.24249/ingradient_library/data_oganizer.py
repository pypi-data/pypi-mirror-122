...
class Data_Organizer(object):
    ...
    def __init__(self, SAVE_PATH, ID = 'ingradient', resampling = None, normalizer = None, mode = 'train', do_set_direction = True):
        self.crop = Cropping()
    ...
    def run(self, seg_path, img_path_list, index, modality = ['CT']):
        ...
        print("current path : ", seg_path, "image shapes :", images.shape, "seg shapes :", seg.shape)
        ...
        if self.resampling != None:
            ...
            if self.mode == 'train' :
                result = self.resampling(torch.vstack((x, y.unsqueeze(0))), save_dict)
                x = result[:-1]
                y = result[-1].squeeze(0)
            
            else:
                x = self.resampling(x, save_dict)
