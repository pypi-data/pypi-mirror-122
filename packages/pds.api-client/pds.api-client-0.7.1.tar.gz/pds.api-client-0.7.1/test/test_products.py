# coding: utf-8

"""
    Planetary Data System API

    Federated PDS API which provides actionable end points standardized between the different nodes.   # noqa: E501

    The version of the OpenAPI document: main
    Contact: pds-operator@jpl.nasa.gov
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest
import datetime

import pds.api_client
from pds.api_client.models.products import Products  # noqa: E501
from pds.api_client.rest import ApiException

class TestProducts(unittest.TestCase):
    """Products unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test Products
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # model = pds.api_client.models.products.Products()  # noqa: E501
        if include_optional :
            return Products(
                summary = pds.api_client.models.summary.summary(
                    q = '', 
                    hits = 56, 
                    took = 56, 
                    start = 56, 
                    limit = 56, 
                    sort = [
                        ''
                        ], 
                    properties = [
                        ''
                        ], ), 
                data = [
                    pds.api_client.models.product.product(
                        id = '', 
                        type = '', 
                        title = '', 
                        description = '', 
                        start_date_time = '', 
                        stop_date_time = '', 
                        investigations = [
                            pds.api_client.models.reference.reference(
                                title = '', 
                                id = '', 
                                href = '', 
                                type = '', 
                                description = '', )
                            ], 
                        observing_system_components = [
                            pds.api_client.models.reference.reference(
                                title = '', 
                                id = '', 
                                href = '', 
                                type = '', 
                                description = '', )
                            ], 
                        targets = [
                            pds.api_client.models.reference.reference(
                                title = '', 
                                id = '', 
                                href = '', 
                                type = '', 
                                description = '', )
                            ], 
                        metadata = pds.api_client.models.metadata.metadata(
                            creation_date_time = '', 
                            update_date_time = '', 
                            version = '', 
                            label_url = '', ), 
                        properties = {
                            'key' : [
                                ''
                                ]
                            }, )
                    ]
            )
        else :
            return Products(
                summary = pds.api_client.models.summary.summary(
                    q = '', 
                    hits = 56, 
                    took = 56, 
                    start = 56, 
                    limit = 56, 
                    sort = [
                        ''
                        ], 
                    properties = [
                        ''
                        ], ),
        )

    def testProducts(self):
        """Test Products"""
        inst_req_only = self.make_instance(include_optional=False)
        inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
