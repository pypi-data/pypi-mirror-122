...
class Node(metaclass=NodeMeta):
    ...
    def _get_fields(self, include_fields=None, exclude_fields=None):
        """

        :param list[str] include_fields:
        :param list[str] exclude_fields:
        :return: list[str]
        """
        if include_fields and exclude_fields:
            raise PentaQuarkConfigurationError(
                "Please specify either include_fields or exclude_fields, "
                "not both"
            )
        ...
        for pn in self._properties:
            if pn in include_fields or (exclude_fields and pn not in exclude_fields):
                res.append(pn)
        return res

    def to_dict(self, include_fields=None, exclude_fields=None, include_nulls=False):
        d = {}
        for field_name in self._get_fields(include_fields, exclude_fields):
            if ((field_value := getattr(self, field_name, None)) is not None) or include_nulls:
