# Any changes to the py_dist library should be reinstalled with
#  pip install --upgrade .
...
from py_dist import Gaussian
from py_dist import Binomial


class TestGaussianClass(unittest.TestCase):
    def setUp(self):
        self.gaussian = Gaussian(25, 2)
        self.gaussian.read_data_file('sample_data/numbers.txt')
    ...
    def test_stdevcalculation(self):
        self.assertEqual(round(self.gaussian.calculate_stdev(sample=True), 2), 92.87, 'sample standard deviation incorrect')
        self.assertEqual(round(self.gaussian.calculate_stdev(sample=False), 2), 88.55, 'population standard deviation incorrect')

    def test_pdf(self):
        self.gaussian = Gaussian(25, 2)
        self.assertEqual(round(self.gaussian.pdf(x=25), 5), 0.19947, \
                         'pdf function does not give expected result')
        self.gaussian.read_data_file('sample_data/numbers.txt')
        self.gaussian.calculate_mean()
        self.gaussian.calculate_stdev()
        self.assertEqual(round(self.gaussian.pdf(x=75), 5), 0.00429, \
...
class TestBinomialClass(unittest.TestCase):
    def setUp(self):
        ...
        self.binomial.read_data_file('sample_data/numbers_binomial.txt')
