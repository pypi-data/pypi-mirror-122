...
class VideoTagApi(TagApi):
    ...
    def remove_from_video(self, tag_id):
        self._api.post(method_name='videos.tags.remove', data={ApiField.ID: tag_id})

    def update_frame_range(self, tag_id, frame_range):
        self._api.post(method_name='videos.tags.update', data={ApiField.ID: tag_id,
                                                               ApiField.FRAME_RANGE: frame_range})

    def update_value(self, tag_id, tag_value):
        self._api.post(method_name='videos.tags.update-value', data={ApiField.ID: tag_id,
