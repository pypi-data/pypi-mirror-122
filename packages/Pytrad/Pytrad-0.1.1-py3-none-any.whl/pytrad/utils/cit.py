...
def kci(data, X, Y, condition_set=None, kernelX='GaussianKernel', kernelY='GaussianKernel', kernelZ='GaussianKernel'):
...
def kci_ui(X, Y, kernelX='GaussianKernel', kernelY='GaussianKernel'):
    '''
     To test if x and y are unconditionally independent

     INPUT:
       X and Y: data matrices of size number_of_samples * dimensionality.
       KernelX/Y: ['GaussianKernel', 'LinearKernel', 'PolunomialKernel']
     Output:
       p_val: the p value.
    ...
    '''
    kernelX = eval(kernelX)()
    kernelY = eval(kernelY)()

    sample_size = X.shape[0]
    hsic = KCI_UInd(sample_size, kernelX, kernelY)
    p = hsic.compute_pvalue(X, Y)
    return p


def kci_ci(X, Y, Z, kernelX='GaussianKernel', kernelY='GaussianKernel', kernelZ='GaussianKernel'):
    """
     To test if x and y are conditionally independent on Z.

     Parameters
     -------
       X, Y and Z: data matrices of size number_of_samples * dimensionality.
       KernelX/Y/Z: ['GaussianKernel', 'LinearKernel', 'PolunomialKernel']
    ...
    kernelX = eval(kernelX)()
    kernelY = eval(kernelY)()
    kernelZ = eval(kernelZ)()

    sample_size = X.shape[0]
    hsic = KCI_CInd(sample_size, kernelX, kernelY, kernelZ)
    p = hsic.compute_pvalue(X, Y, Z)
    return p
