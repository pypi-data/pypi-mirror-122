...
class CausalGraph:
    def __init__(self, no_of_var):
        ...
        for i in range(no_of_var):
            for j in range(i + 1, no_of_var):
                self.G.add_edge(Edge(nodes[i], nodes[j], -1, -1))
    ...
    def find_arrow_heads(self):
        ...
        L = np.where((self.G.graph == 1) | (self.G.graph == 6))
    ...
    def find_bi_directed(self):
        ...
        return [(edge[1], edge[0]) for edge in self.find_arrow_heads() if self.G.graph[edge[1], edge[0]] == 6]
