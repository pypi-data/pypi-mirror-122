...
def insert_validity_test1(G, i, j, T):
    ...
    # condition 1
    Tj = np.intersect1d(np.where(G.graph[:, j] == -1)[0], np.where(G.graph[j, :] == -1)[0])  # neighbors of Xj
...
def insert_vc2_new(G, j, i, NAT):  # validity test for condition 2 of Insert operator
    ...
    while (len(stack)):
        ...
        if (top['value'] == target):  # if find the target, search that pathway to see whether NAT is in that pathway
            ...
            child = np.concatenate((np.where(G.graph[:, top['value']] == 1)[0],
                                    np.intersect1d(np.where(G.graph[top['value'], :] == -1)[0],
                                                   np.where(G.graph[:, top['value']] == -1)[0])))
...
def delete_validity_test(G, i, j, H):
    ...
    # condition 1
    Hj = np.intersect1d(np.where(G.graph[:, j] == -1)[0], np.where(G.graph[j, :] == -1)[0])  # neighbors of Xj
