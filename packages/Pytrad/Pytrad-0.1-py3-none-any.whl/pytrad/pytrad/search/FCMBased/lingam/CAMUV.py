...
def get_neighborhoods(X, alpha):
	...
	for i in range(d):
			for j in range(d)[i+1:]:
				independence = HSIC.hsic_gam(X=np.reshape(X[:,i], [n, 1]), Y=np.reshape(X[:,j], [n, 1]), mode="pvalue")
...
def find_parents(X, alpha, maxnum_vals, N):
	...
	for i in range(d):
		...
		for j in P[i]:
			...
			independence = HSIC.hsic_gam(X=np.reshape(residual_i, [n, 1]), Y=np.reshape(residual_j, [n, 1]), mode="pvalue")
			if independence > alpha:
				non_parents.add(j)
		P[i] = P[i] - non_parents
...
def get_child(X, variables_set, P, N, Y, alpha):
	...
	for child in variables_set:
		...
		independence = HSIC.hsic_gam(X=np.reshape(residual, [n, 1]), Y=np.reshape(Y[:,list(parents)], [n, len(parents)]), mode="pvalue")
...
def check_independence_withou_K(parents, child, P, N, Y, alpha):
	n = Y.shape[0]
	for parent in parents:
		independence = HSIC.hsic_gam(X=np.reshape(Y[:,child], [n, 1]), Y=np.reshape(Y[:,parent], [n, 1]), mode="pvalue")
...
def execute(X, alpha, num_explanatory_vals):
	...
	for i in range(d):
		for j in range(d)[i+1:]:
			...
			independence = HSIC.hsic_gam(X=np.reshape(i_residual, [n, 1]), Y=np.reshape(j_residual, [n, 1]), mode="pvalue")
			if independence < alpha:
				if not set([i,j]) in U:
					U.append(set([i,j]))
