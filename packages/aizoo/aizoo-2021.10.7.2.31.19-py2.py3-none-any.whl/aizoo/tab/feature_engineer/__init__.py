...
class FE(object):
    ...
    def transform(self, df: pd.DataFrame, n_jobs=1):
        """提前计算一下特征值数量，过滤
    ...
    def calculate_featvalue_num(self, df, cat_feats, n_jobs=4):
        keys_list = self.combination_all(cat_feats)

        def func(keys):
            return len(df[keys].drop_duplicates())

        featvalue_num = keys_list | xtqdm('计算特征值数量🐢') | xJobs(func, n_jobs) | xlist
        return zip(featvalue_num, keys_list) | xsort
