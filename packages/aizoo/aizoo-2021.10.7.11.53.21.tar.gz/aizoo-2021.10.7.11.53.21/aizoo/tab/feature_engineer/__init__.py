...
class FE(object):
    ...
    def calculate_featvalue_num(self, df, cat_feats):

        feat2num = df[cat_feats].nunique().to_dict()

        _ = [
            (keys | xmap(lambda k: feat2num.get(k, 1)) | xreduce(lambda x, y: x * y), keys)
            for keys in self.combination_all(cat_feats)
        ]

        return pd.DataFrame(_, columns=['num', 'keys']).sort_values('num', ascending=False, ignore_index=True)
