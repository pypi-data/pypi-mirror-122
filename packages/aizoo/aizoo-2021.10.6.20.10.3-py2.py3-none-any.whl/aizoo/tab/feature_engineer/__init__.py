...
class FE(object):
    ...
    def transform(self, df: pd.DataFrame, n_jobs=1):
        ...
        if n_jobs == 1:
            ...
            def func(keys):
                return self.group_calculate(df, keys), keys

            df2keys = self.cat_feat_combination | xProcessPoolExecutor(func, n_jobs)

            for df_, keys in tqdm(df2keys, desc='🐢'):
                df = df.merge(df_, on=keys)
...
if __name__ == '__main__':
    ...
    ft = FE(cat_feat_combination=[['N0'], ['N1'], ['N2']])
    # ft = FE()
    ...
    #     print(i, 'done!')
    #
    # range(4) | xProcessPoolExecutor(func, 10) | xlist
