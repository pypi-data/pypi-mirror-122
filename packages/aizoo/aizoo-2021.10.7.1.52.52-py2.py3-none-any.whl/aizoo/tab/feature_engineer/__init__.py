...
class FE(object):
    ...
    def __init__(self, cat_feats=None, num_feats=None, cat_funcs=None, num_funcs=None, agg_dict=None,
        ...
        self.cat_feat_combination_ = []
        self.feat_value_num_ = []

    def transform(self, df: pd.DataFrame, n_jobs=1, min_sample_num=100, max_feat_value_num=512):
        """

        @param df:
        @param n_jobs:
        @param min_sample_num: 太少没有统计意义
        @max_feat_value_num: 最大特征值数
        @return:
        """
        ...
        for keys in self.cat_feat_combination:
            if len(keys) > 1:
                feat_value_num = len(df[keys].drop_duplicates())  # 特征值的数量
                self.feat_value_num_.append((feat_value_num, keys))

                if feat_value_num < max_feat_value_num and feat_value_num < len(df) // min_sample_num:
                    self.cat_feat_combination_.append(keys)

        bar = tqdm(self.cat_feat_combination_, desc='agg🐢')
