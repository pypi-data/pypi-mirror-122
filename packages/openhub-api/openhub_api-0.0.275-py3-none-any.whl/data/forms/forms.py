...
class DataTransform(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.fields['parent'] is not None and self.fields['parent'] != '':
            print(str(self.fields['parent']))
            print(str(self.fields['parent'].initial))
            self.fields['accessory'].initial = str(DataTransformer.objects.get(self.fields['parent'].initial.pk).first().accessory.pk)
