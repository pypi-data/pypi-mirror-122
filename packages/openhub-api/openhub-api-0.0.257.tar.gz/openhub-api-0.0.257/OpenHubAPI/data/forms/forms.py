...
class BaseInlineFormSetWithAddMoreButton(BaseInlineFormSet):
    def as_p(self):
        ...
        forms_as_p = super(BaseInlineFormSetWithAddMoreButton, self).as_p()
...
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js">
    ...
    $('#{button_id}').click(function() {{
        var form_idx = $('#{self.prefix}-TOTAL_FORMS').val();
        var empty_form = document.getElementById('{empty_form_id}').html().replace(/__prefix__/g, form_idx));
        var d1 = document.getElementById('{div_name}');
        d1.insertAdjacentHTML('afterend', empty_form);
        $('#{self.prefix}-TOTAL_FORMS').val(parseInt(form_idx) + 1);
    }});
</script>"""
        print(p)
        return p
