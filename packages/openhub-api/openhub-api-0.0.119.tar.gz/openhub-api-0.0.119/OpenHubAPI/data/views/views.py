...
from data.forms.forms import DataTransformerForm,DataTransform
...
class AccessoryViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if accessory.has_data_transformer:
            ...
            datatransformerset = formset_factory(DataTransform, formset=DataTransformerForm, extra=1)
            formset = datatransformerset(initial=[data_transformer])
        else:
            data_transformer = DataTransformer()
            data_transformer.accessory = accessory
            data_transformer.save()
            datatransformerset = formset_factory(DataTransform, formset=DataTransformerForm, extra=1)
            formset = datatransformerset(initial=[])
        ...
        if request.accepted_renderer.format == 'html':
            return Response(
                {"accessory_form": accessory_form, "calibration_form": calibration_form, "calibrations": calibrations, "data_transformer_form":formset},
    ...
    def postDataTransformer(request):
        # request should be ajax and method should be POST.
        if request.is_ajax and request.method == "POST":
            form = DataTransformerForm(request.POST)
            if form.is_valid():
                instance = form.save()
                print(str(instance))
                # serialize in new friend object in json
