...
class DataConstantsTransformerForm(forms.ModelForm):
    ...
    value = forms.CharField(required=False,
                            widget=forms.TextInput(attrs={'class': 'form-control'}))
    data_transformer = forms.ModelChoiceField(required=False, queryset=DataTransformerTypes.objects.all(), widget=forms.HiddenInput(attrs={'class': 'form-control'}))
...
class DataTransform(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # self.fields['type'] = ChoiceField(choices=tuple(types))
        try:
            if hasattr(self.instance, 'get_children') and self.instance.get_children().count() >= 0:
                ...
                dataTransformerInlineFormset = inlineformset_factory(DataTransformer, DataTransformer,
                                                                     fk_name='parent', form=DataTransform,
                                                                     extra=self.instance.get_children().count() + 1,
                ...
                self.nested_data_transformer_form = dataTransformerInlineFormset(
                    ...
                    initial=[{'parent': self.instance, 'accessory': self.instance.accessory, 'id': str(uuid.uuid4())}]
                )
        ...
        self.nested_constants_form = DataConstantsTransformerInlineFormset(
            ...
            data=self.data if self.is_bound else None,
            files=self.files if self.is_bound else None,
            initial=[{'data_transformer': self.instance, 'id': str(uuid.uuid4())}]

        )

        self.nested_hardware_outputs_form = HardwareTransformerInlineFormset(
            queryset=self.instance.hardwaredatatransformer_set,
            data=self.data if self.is_bound else None,
            files=self.files if self.is_bound else None,
            initial=[{'data_transformer': self.instance, 'id': str(uuid.uuid4())}]
    ...
    def as_p(self):
        ...
        if hasattr(self, 'nested_data_transformer_form') and self.nested_data_transformer_form is not None:
            ...
            forms = forms + '\n       -' + ' '.join(form.as_p() for form in self.nested_data_transformer_form)
        forms = forms + '</div></div>'
    ...
    def save(self, commit=True):
        ...
        # if hasattr(self,
        #            'nested_constants_form') and self.nested_constants_form is not None and self.nested_constants_form.is_valid():
        #     self.nested_constants_form.save(commit)
        # if hasattr(self,
        #            'nested_hardware_outputs_form') and self.nested_hardware_outputs_form is not None and self.nested_hardware_outputs_form.is_valid():
        #     self.nested_hardware_outputs_form.save(commit)
        # if hasattr(self,
        #            'nested_hardware_stats_form') and self.nested_hardware_stats_form is not None and self.nested_hardware_stats_form.is_valid():
        #     self.nested_hardware_stats_form.save(commit)
        # if hasattr(self,
        #            'nested_data_transformer_form') and self.nested_data_transformer_form is not None and self.nested_data_transformer_form.is_valid():
...
class DataTransformRoot(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.nested_constants_form = DataConstantsTransformerInlineFormset(
            instance=self.instance,
            queryset=self.instance.datatransformerconstants,
            data=self.data if self.is_bound else None,
            files=self.files if self.is_bound else None,
            initial=[{'data_transformer': self.instance, 'id': str(uuid.uuid4())}]
        ...
        self.nested_hardware_outputs_form = HardwareTransformerInlineFormset(
            instance=self.instance,
            queryset=self.instance.hardwaredatatransformer,
            data=self.data if self.is_bound else None,
            files=self.files if self.is_bound else None,
            initial=[{'data_transformer': self.instance, 'id': str(uuid.uuid4())}]
        ...
        self.nested_hardware_stats_form = HardwareStatsTransformerInlineFormset(
            instance=self.instance,
            queryset=self.instance.hardwarestatsdatatransformer,
            data=self.data if self.is_bound else None,
            files=self.files if self.is_bound else None,
            initial=[{'data_transformer': self.instance, 'id': str(uuid.uuid4())}]
    ...
    def save(self, commit=True):
        ...
        if hasattr(self,
                   'nested_hardware_outputs_form') and self.nested_hardware_outputs_form is not None:
            self.nested_hardware_outputs_form.instance.refresh_from_db()
