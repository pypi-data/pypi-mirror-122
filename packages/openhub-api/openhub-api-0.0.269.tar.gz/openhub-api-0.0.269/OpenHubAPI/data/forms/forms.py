...
class DataTransform(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.parent is not None:
            self.accessory = forms.ModelChoiceField(required=True, queryset=Accessory.objects.all(),
                                                    widget=forms.HiddenInput(attrs={'class': 'form-control'}),
                                                    initial=self.instance.accessory.pk)
        # types = list(
...
class DataTransformRoot(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.accessory = forms.ModelChoiceField(required=True, queryset=Accessory.objects.all(),
                                       widget=forms.HiddenInput(attrs={'class': 'form-control'}), initial=self.instance.accessory.pk)
