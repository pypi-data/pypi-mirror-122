...
class DataConstantsTransformerForm(forms.ModelForm):
    ...
    value = forms.CharField(required=False,
                            widget=forms.TextInput(attrs={'class': 'form-control'}))
    data_transformer = forms.ModelChoiceField(required=False, queryset=DataTransformer.objects.all(), widget=forms.HiddenInput(attrs={'class': 'form-control'}))
...
class DataTransformRoot(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.nested_hardware_outputs_form = HardwareTransformerInlineFormset(
            ...
            files=self.files if self.is_bound else None,
            prefix=str(self.instance.id)+'HardwareTransformerInlineFormset',
            initial=[{'data_transformer': str(self.instance.id), 'id': str(uuid.uuid4())}]
        )
        self.nested_hardware_outputs_form.extra = self.instance.hardwaredatatransformer_set.count() + 1
