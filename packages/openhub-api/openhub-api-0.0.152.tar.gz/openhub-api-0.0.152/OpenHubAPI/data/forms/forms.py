...
class DataTransformerForm(BaseInlineFormSet):
    ...
    def add_fields(self, form, index):
        ...
        dataTransformerInlineFormset = inlineformset_factory(DataTransformer, DataTransformer,formset=self.__class__,
                                                             fk_name='parent',
                                                             extra=1, exclude=['updated_at', 'created_at'], form=DataTransform
                                                             )
        if form.instance is not None and hasattr(form.instance, 'getChildren'):
            form.nested_data_transformer_form = dataTransformerInlineFormset(
                initial=form.instance.getChildren(),
    ...
    def as_p(self):
        ...
        forms = ' '
        for form in self:
            forms.join(form.as_p())
            if form.nested_data_transformer_form is not None:
                forms.join(form.nested_data_transformer_form.as_p())
            if form.nested_constants_form is not None:

                forms.join(form.nested_constants_form.as_p())
            if form.nested_hardware_outputs_form is not None:

                forms.join(form.nested_hardware_outputs_form.as_p())
            if form.nested_hardware_stats_form is not None:

                forms.join(form.nested_hardware_stats_form.as_p())
