...
class EnumExpandShrink:
    ...
    async def expand(self, value,**kwargs):
        return next((key for key, optionvalue in self.options.items() if optionvalue == value), None)
...
class StructureExpandShrink:
    ...
    async def expand(self, value, transpile=True):
        ...
        structure = get_packer_registry().get_structure(self.get_identifier())
    ...
    async def shrink(self, instance,**kwargs):
        ...
        transpiler = get_transpiler_registry().get_transpiler(instance.__class__.__name__, self.get_identifier())
        transpiled_instance = await transpiler.transpile(instance)
        return await transpiled_instance.shrink()

    def to_type(self):
        return get_packer_registry().get_structure(self.get_identifier())
