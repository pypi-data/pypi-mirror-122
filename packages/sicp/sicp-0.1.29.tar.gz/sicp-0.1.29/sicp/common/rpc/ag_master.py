...
@requires_master_secret
@service.route("/api/prepare_jobs")
def prepare_jobs(job_files: List[Dict[str, str]]) -> List[str]:
    """
    Takes a list of file name -> file content mappings, and schedules jobs for each mapping in the list.
    Returns a list of the scheduled job IDs, to be passed to trigger_jobs
    """
    ...
...
def trigger_jobs(command, batch_size, skeleton_path, jobs, script_timeout):
    """Given a list of job IDs, queues autograder workers to begin grading.

    :param command: the command that runs each job
    :type command: str
    :param batch_size: the number of jobs to be run together in a single batch
    :type batch_size: int
    :param skeleton_path: the path to the cloud storage blob holding the skeleton files for this job
    :type skeleton_path: str
    :param jobs: the list of job IDs to queue
    :type jobs: list[str]
    :param script_timeout: the maximum running time of each job, in seconds
    :type script_timeout: optional[int]
...
@requires_master_secret
@service.route("/api/get_results")
def get_results(job_ids: List[str]) -> Dict[str, Optional[str]]:
    ...
