...
from text_sensitivity.data.random.base import SeedMixin, CaseMixin
...
class CityByPopulationMixin(Readable):
    ...
    def cities_by_population(cities: List[str], country_code: str):
        ...
        if os.path.isfile(file):  # already exists, using cached file
            res = pd.read_csv(file, header=False).values.tolist()
            ...
            try:
                ...
                pd.DataFrame(res).to_csv(file, index=False, header=False)
