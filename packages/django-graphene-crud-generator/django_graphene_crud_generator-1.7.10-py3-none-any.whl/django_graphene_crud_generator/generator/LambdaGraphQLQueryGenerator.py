...
class LambdaGraphQLQueryGenerator(AbstractGraphQLQueryGenerator):
    ...
    def _get_return_value_temp_type_name(self, builld_context: GraphQLBuildtimeContext) -> str:
        return f"{stringcase.pascalcase(self.output_name)}OutcomeType"
    ...
    def _generate_action_return_type(self, build_context: GraphQLBuildtimeContext) -> Tuple[str, TGrapheneWholeQueryReturnType]:
        if not isinstance(self.return_value, dict):
            result = {self._get_query_output_name(build_context): self.return_value}
        else:
