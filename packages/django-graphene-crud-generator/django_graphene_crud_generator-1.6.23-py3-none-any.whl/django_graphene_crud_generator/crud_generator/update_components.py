...
class StandardCrudUpdateMutation(AddCRUDContextComponentMixIn, AbstractUpdateGraphQLMutationGenerator):
    ...
    def _check_if_object_exists(self, django_type: TDjangoModelType, runtime_context: GraphQLRuntimeContext) -> Tuple[
        ...
        query_dict = dict()
        available_field = list(django_helpers.get_primitive_fields(django_type))
        ...
        for f in fields_to_check:
            if f not in available_field:
                ...
                LOG.info(f"""The field \"{f}\" is specified but it is not considered while fetchng object to update.
                The fields considered are just {', '.join(available_field)}""")
                continue
