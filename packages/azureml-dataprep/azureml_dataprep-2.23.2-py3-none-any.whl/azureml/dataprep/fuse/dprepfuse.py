...
def rslex_uri_volume_mount(uri: str, mount_point: str, options: MountOptions = None):
    try:
        log.info('Running rslex direct volume mount')
        from azureml.dataprep.rslex import RslexDirectURIMountContext, PyVolumeMountOptions
...
def rslex_mount(dataflow: Optional[Dataflow],
    ...
    if (destination is None and (rslex_fuse_flight == "1" or rslex_fuse_flight.lower() == "true")) \
        ...
        try:
            log.info('Running rslex direct volume mount')
            from azureml.dataprep.rslex import RslexDirectMountContext, PyVolumeMountOptions, PyVolumeMountDatastoreSource
            from azureml.dataprep.api._rslex_executor import ensure_rslex_environment
            ...
            mount_context = RslexDirectMountContext(mount_point, datastore_source, mount_options)
            log.info("Rslex direct volume mount context created")
            return mount_context
