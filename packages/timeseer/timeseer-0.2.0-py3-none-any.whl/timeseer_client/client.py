...
class Client(KukurClient):
    ...
    def upload_data(self, metadata: Metadata, table: pa.Table):
        ...
        """
        metadata_json = metadata.camelcase()
        metadata_json['series'] = {'source': metadata.series.source, 'name': metadata.series.name}
        ...
        while True:
            results = list(client.do_action(('get_flow_evaluation_state', json.dumps(response).encode())))
            state = json.loads(results[0].body.to_pybytes())
            if state['completed'] == state['total'] and state['blockCompleted'] == state['blockTotal']:
                break
            time.sleep(1)
