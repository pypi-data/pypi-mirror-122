...
class Binomial(Distribution):
    ...
    def pdf(self, k):
        ...
        return factorial(self.n)/(factorial(k)*factorial(self.n-k)) * self.p**k * (1-self.p)**(self.n-k)
    ...
    def plot_bar_pdf(self):
        ...
        """
        pdf_plot_y = [self.pdf(k) for k in range(0,self.n+1)]
        pdf_plot_x = [k for k in range(0,self.n+1)]
        
        fig, ax = plt.subplots()
        ax.bar(pdf_plot_x, pdf_plot_y)
    ...
    def __repr__(self):
        ...
        Returns:
            string: characteristics of the Binominal
