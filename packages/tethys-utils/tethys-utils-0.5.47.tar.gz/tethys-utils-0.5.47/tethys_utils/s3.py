...
def put_remote_station(s3, bucket, station, run_date=None, version=3):
    ...
    while counter > 0:
        try:
            ...
            print('S3 connection closed error...trying again...')
            counter = counter - 1
            if counter == 0:
                raise bc_exceptions.ConnectionClosedError
...
def put_remote_agg_stations(s3, bucket, dataset_id, threads=30, version=3):
    ...
    if not list2.empty:
        ...
        while counter > 0:
            try:
                _ = s3.put_object(Body=stns_obj, Bucket=bucket, Key=stns_key, ContentType='application/json', Metadata={'run_date': run_date_key})
                break
            except bc_exceptions.ConnectionClosedError:
                print('S3 connection closed error...trying again...')
                counter = counter - 1
        ...
        while counter > 0:
            try:
                _ = s3.put_object(Body=ro_obj, Bucket=bucket, Key=ro_key, ContentType='application/json', Metadata={'run_date': run_date_key})
                break
            except bc_exceptions.ConnectionClosedError:
                print('S3 connection closed error...trying again...')
                counter = counter - 1
...
def put_remote_agg_datasets(s3, bucket, threads=30, version=3):
    ...
    while counter > 0:
        try:
            _ = s3.put_object(Body=dss_obj, Bucket=bucket, Key=dss_key, ContentType='application/json', Metadata={'run_date': run_date_key})
            break
        except bc_exceptions.ConnectionClosedError:
            print('S3 connection closed error...trying again...')
            counter = counter - 1
