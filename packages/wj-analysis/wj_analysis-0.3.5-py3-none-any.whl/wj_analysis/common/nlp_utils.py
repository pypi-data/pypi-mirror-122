...
class Features:
    ...
    def pos_tags(self, input_txt):
        ...
        try:
            file_name = "utils/dataframes/" + str(
                int(datetime.timestamp(datetime.now()))
            )
            file_to_send = BASE_DIR + "/" + file_name
            pickle.dump(self.input_txt.lower(), open(file_to_send, "wb"))
            file_send = {"model_json": open(file_to_send, "rb")}
            ms_models = URL_MS_MODELOS + "/features/pos_tags"
            doc_response = requests.post(url=ms_models, files=file_send)
            delete_file_dataframe(file_to_send, file_send)
            out_dict = pickle.loads(doc_response.content)
        except Exception as e:
            print(e)
...
class Polarity(object):
    ...
    def tokenize_texts(self, text_list, max_length=128):
        try:
            ...
            encoded_dict = df_to_formatted(
                text_list, SECRETS["URL_MS_MODELOS"], model_name, model_type
            )
        except Exception as e:
            print(e)
            error_1 = sys.exc_info()[0]
    ...
    def forward_model(self, data):
        try:
            ...
            model_type = "tokenize_texts"
            (out,) = df_to_formatted(
                data, SECRETS["URL_MS_MODELOS"], model_name, model_type
            )
        except Exception as e:
            print(e)
            error_1 = sys.exc_info()[0]
    ...
    def polarity(self, df_text, text_column_name="processed_text"):
        ...
        try:
            model_name = "polarity"
            model_type = "polarity"
            polarity = df_to_formatted(
                df_text, SECRETS["URL_MS_MODELOS"], model_name, model_type
            )
            return polarity
        except Exception as e:
            print(e)
    ...
    def sttm_model(self, K=20, alpha=0.1, beta=0.2, n_iters=100):
        ...
        try:
            ...
            self.df_data = df_to_formatted(
                self.df_data, SECRETS["URL_MS_MODELOS"], model_name, model_type
            )
        except Exception as e:
            print(e)
            error_1 = sys.exc_info()[0]
...
def send_to_model(value, ms_models):
    values = {"value": value}
    response = requests.post(url=ms_models, data=json.dumps(values))
    return response.text


def df_to_formatted(df, url_model, model_name, model_type):
    ms_models = url_model + "_reg/" + model_name + "/" + model_type
    # TODO: cambiar iterrows por .applylamdba
    for index, row in df.iterrows():
        row = row.copy()
        polarity_value = send_to_model(row.processed_text, ms_models)
        df.loc[index, "polarity"] = polarity_value
    return df
