...
import sys
#TODO Remove this development import!!
...
def fit_cyclotron(subcases_pattern, source_name, reference_location="subcases/scw_lists", nscw=-1,
        ...
        for ys in instruments:
            if ys == 'J1':
                par_dict.update({"jemx_num": "1",
                                 ...
                                 "product": "jemx_spectrum", 'integral_data_rights': 'public'})
            if ys == 'J2':
    ...
    ignore_string=['**-20.0,100.0-**', '**-3.5,23.0-**', '**-3.5,23.0-**' ]
    if jemx1_spec_fn =='none' or jemx2_spec_fn =='none':
        ignore_string=['**-20.0,100.0-**', '**-3.5,12.0-**']
    if jemx1_spec_fn == 'none' and jemx2_spec_fn == 'none':
        ignore_string = ['**-20.0,100.0-**']
    ...
    with open('res_%s.txt' % subcases_pattern, 'w') as f:
        ...
        x = fit_by_bin[subcases_pattern]['LineE__01']
        out_str += "%.3f %.3f %.3f " % (x[0], x[1]-x[0], x[2]-x[0] )
        x = fit_by_bin[subcases_pattern]['LineE__04']
        out_str += "%.3f %.3f %.3f " % (x[0], x[1]-x[0], x[2]-x[0] )
        f.write('%s' % out_str)
