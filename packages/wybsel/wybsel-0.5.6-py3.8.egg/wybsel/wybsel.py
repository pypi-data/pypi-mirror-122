...
from selenium import webdriver as rred
...
class Browser:
    ...
    def __init__(self, showWindow=True, proxy=None , binary_location = None, potions = None, headers=None, downloadPath:str=None, arguments=["--disable-dev-shm-usage","--no-sandbox"]):
        ...
        if potions:
           self.driver = webdriver.Chrome(seleniumwire_options=potions, executable_path=driverpath, options=options)
        else:
            self.driver = rred.Chrome(executable_path=driverpath, options=options)
        if headers:
...
if __name__ == '__main__':
    headers= {'Referer':'','HTTP_SEC_CH_UA':'JAY QUEST'}
    #bina = "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe"


    aton = Browser(headers=headers) #binary_location = bina)
    #response = aton.req('POST', 'https://pl.4everproxy.com/request/u=http%3A%2F%2Fazenv.net&u_default=http%3A%2F%2Fwww.google.com%2F&urlsub=&selip=random&customip=64.124.38.141%3A8080&allowCookies=on',allow_redirects=False)
    aton.go_to('https://pl.4everproxy.com/process/ZW5jb2RlUGFnZT0wJnN0cmlwVGl0bGU9MCZzdHJpcEpTPTAmYWxsb3dDb29raWVzPTEmYXV0b1Blcm1hbGluaz0wJnN1YmRvbWFpbj1wbCZ1cmw9aHR0cCUyNTNBJTI1MkYlMjUyRmF6ZW52Lm5ldA==')
    print(aton.get_page_source())

    jav = """function postForm(path, params, method) {
            method = method || 'post';
      ...
      postForm('https://pl.4everproxy.com/request', {"u":"http://azenv.net","u_default":"https://www.google.com","selip":"random","customip":"64.124.38.141:8080","allowCookies":"on"});"""
    aton.execute_script(jav)
    page_state = aton.execute_script('return document.readyState;')
    print(page_state)
    aton.execute_script("""document.write('<script>window.location.href = "http://azenv.net";</script>')""")
    #response = aton.req('POST', 'https://pl.4everproxy.com/request/u=http%3A%2F%2Fazenv.net&u_default=http%3A%2F%2Fwww.google.com%2F&urlsub=&selip=random&customip=64.124.38.141%3A8080&allowCookies=on',allow_redirects=False)
    #print(response.text)
