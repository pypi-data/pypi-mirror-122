...
class Browser:
    ...
    def __init__(self, showWindow=True, proxy=None , binary_location = None, potions = None, headers=None, downloadPath:str=None, arguments=["--disable-dev-shm-usage","--no-sandbox"]):
        ...
        if potions:
           self.driver = webdriver.Chrome(wire_options=potions, options=options)
        else:
...
if __name__ == '__main__':
    headers= {'Referer':'https://me.myself.and.i'}
    


    aton = Browser(headers=headers)
    aton.go_to('http://azenv.net')
