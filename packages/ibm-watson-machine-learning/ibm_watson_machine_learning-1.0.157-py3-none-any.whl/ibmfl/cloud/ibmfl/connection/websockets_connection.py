...
class SWSReceiver(FLReceiver):
    ...
    async def establish_websocket_connection(self, loop, stop):
        logger.info("METHOD = SWSReceiver.establish_websocket_connection")
        uri = "wss://" + self.settings.get("ip") + ":" + str(self.settings.get("port"))
        logger.info("SWSReceiver.establish_websocket_connection uri = "+uri)
        ...
        while not self.stopped:
            try:
                ...
                async with websockets.connect(uri, close_timeout=1000, max_size=2 ** 29, read_limit=2 ** 29, write_limit=2**29, loop=loop, extra_headers=headers) as websocket:
            ...
            try:
                ...
                for future in done:
                    if not future.cancelled():
                        logger.info("Future Processed successfully")
                if len(pending) == 0 :
    ...
    async def receive_message_handler(self, websocket):
        logger.info("METHOD = SWSReceiver.receive_message_handler")
        try:
