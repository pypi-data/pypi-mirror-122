...
class TestDrbFileNode(unittest.TestCase):
    ...
    def test_get_children_count(self):
        node = DrbFileNode(self.test_path)
        self.assertEqual(2, node.get_children_count())
        node = DrbFileNode(os.path.join(self.test_path, 'a'))
        self.assertEqual(3, node.get_children_count())
    ...
    def test_get_named_child(self):
        ...
        node = parent_node.get_named_child('ab', occurrence=1)
        self.assertIsNotNone(node)
        self.assertIsInstance(node, DrbFileNode)
        self.assertEqual('ab', node.name)

        node = parent_node.get_named_child('ab')
        self.assertIsNotNone(node)
        ...
        with self.assertRaises(DrbException):
            parent_node.get_named_child('ab', 'https://foobar.org/foo', 0)
        with self.assertRaises(DrbException):
            self.assertListEqual(parent_node.get_named_child('foobar'))
        with self.assertRaises(DrbException):
            parent_node.get_named_child('foobar', occurrence=1)

    def test_get_first_child(self):
        node = DrbFileNode(os.path.join(self.test_path, 'a'))
        first = node.get_first_child()
    ...
    def test_insert_child(self):
        ...
        self.assertEqual(node.get_child_at(0), child_node)
    ...
    def test_replace_child(self):
        ...
        self.assertEqual(node.get_child_at(0), child_2)
    ...
    def test_has_impl(self):
        ...
        node = node.get_named_child('ac', occurrence=1)
