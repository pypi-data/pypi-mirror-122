...
from lnsync_pkg.prefixdbname import pick_db_basename, DEFAULT_DBPREFIX
from lnsync_pkg.filetree import FileTree
from lnsync_pkg.hashtree import FileHashTree, ONLINE, OFFLINE
...
def lnsync_online_db(dbdir, topdir=None, dbprefix=DEFAULT_DBPREFIX, exclude_patterns=None):
    """
    Return hashdb.
    ...
    hashdb = FileHashTree(mode=ONLINE, topdir_path=topdir,
                          dbkwargs={"dbpath":dbpath},
                          exclude_patterns=exclude_patterns)
    return hashdb

def lnsync_offline_db(dbpath):
    hashdb = FileHashTree(mode=OFFLINE, topdir_path=None, dbkwargs={"dbpath":dbpath})
    return hashdb
