...
class GLTF(object):
    ...
    def get_bounding_box(self, size=True):
        g = self.copy()
    ...
    def render(self, binary=False, embed=None, split_to=None, gltf_only=None):
        ...
        if binary:
            ...
            if len(json_chunk) % 4:
                ...
                json_chunk.extend(bytearray([0x20] * (4 - len(json_chunk) % 4)))
            ...
            if binary_chunk:
                if len(binary_chunk) % 4:
                    ...
                    binary_chunk.extend(bytearray([0x00] * (4 - len(binary_chunk) % 4)))
