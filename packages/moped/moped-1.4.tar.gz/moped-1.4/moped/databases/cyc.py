...
class Cyc:
    ...
    def parse(self) -> Tuple[list[Compound], list[Reaction], Dict[str, str]]:
        """Parse the database."""
        path = self.path
        parse_compounds, compound_types = CompoundParser(path / "compounds.dat").parse()
        parse_reactions = ReactionParser(path / "reactions.dat").parse()
...
def _add_type(dictionary: Dict[str, ParseReaction | ParseCompound], id_: str, type_: str) -> None:
    """Short description."""
...
def _set_substrate(reactions: Dict[str, ParseReaction], id_: str, substrate: str) -> None:
    substrate = _rename(substrate) + "_c"
    reactions[id_].substrates[substrate] = -1
    reactions[id_].substrate_compartments[substrate] = "CCO-IN"


def _set_product(reactions: Dict[str, ParseReaction], id_: str, product: str) -> None:
...
def _set_substrate_coefficient(
    reactions: Dict[str, ParseReaction], id_: str, coefficient: str, substrate: str
) -> None:
    try:
...
def _set_product_coefficient(
    reactions: Dict[str, ParseReaction], id_: str, coefficient: str, product: str
) -> None:
    try:
...
def _set_substrate_compartment(
    reactions: Dict[str, ParseReaction], id_: str, compartment: str, substrate: str
) -> None:
...
def _set_product_compartment(
    reactions: Dict[str, ParseReaction], id_: str, compartment: str, product: str
) -> None:
    if compartment == "CCO-OUT":
