...
from ..databases import Cyc  # type: ignore
...
class Model:
    ...
    def read_from_pgdb(
        self,
        pgdb_path: Union[str, pathlib.Path],
    ...
    def read_from_sbml(self, sbml_file: Union[str, pathlib.Path]) -> None:
    ...
    def read_from_bigg(self, bigg_sbml_file: Union[str, pathlib.Path]) -> None:
    ...
    def add_reaction_from_reference(
        self, reference_model: "Model", reaction_id: str, update_compounds: bool = True
    ) -> None:
    ...
    def add_reactions_from_reference(
        self, reference_model: "Model", reaction_ids: Iterable[str], update_compounds: bool = True
    ) -> None:
    ...
    def get_pathway_ids(self) -> Tuple[str, ...]:
        """Get all pathway ids.
    ...
    def get_producing_reactions(
        self, cobra_solution: cobra.Solution, compound_id: str, cutoff: float = 0
    ) -> pd.DataFrame:
    ...
    def to_kinetic_model(
        ...
        for reaction in sorted(self.reactions.values()):
            rxn_id = reaction.id
            _substrates, _products = reaction._split_stoichiometries()
    ...
    def to_sbml(self, filename: str = "model.sbml") -> None:
    ...
    def blast_sequences(self, sequences: Iterable[str], genome_file: str | pathlib.Path) -> pd.DataFrame:
    ...
    def blast_reactions(self, reaction_ids: Iterable[str], genome_file: str | pathlib.Path) -> pd.DataFrame:
    ...
    def blast_all_reactions(self, genome_file: str | pathlib.Path) -> pd.DataFrame:
    ...
    def _get_reactions_from_blast_results(
        self,
        filtered_blast_monomers: Set[str],
        ...
        if require_all_complex_monomers:
            ...
            for monomer in filtered_blast_monomers:
                for rxn_id in self._monomers[monomer]:
                    blast_reactions.add(rxn_id)
            return blast_reactions
    ...
    def create_submodel_from_blast_monomers(
        ...
        min_pident: float = 85,
        require_all_complex_monomers: bool = True,
    ...
    def to_yaml(self, filename: str | pathlib.Path) -> None:
        from ..utils.serialize import serialize
    ...
    @staticmethod
    def load_from_pickle(filename: str | pathlib.Path) -> "Model":
    ...
    @staticmethod
    def load_from_json(filename: str | pathlib.Path) -> "Model":
        with open(filename, "r") as file:
            obj = json.load(file)
        return _deserialize(obj)

    @staticmethod
    def load_from_yaml(filename: str | pathlib.Path) -> "Model":
        with open(filename, "r") as file:
...
def _deserialize_compounds(obj: Dict[str, Any]) -> list[Compound]:
    return [
        Compound(
            ...
            charge=float(charge) if (charge := i["charge"]) is not None else None,
...
def _deserialize_reactions(obj: Dict[str, Any]) -> list[Reaction]:
    return [
        Reaction(
            ...
            monomers={k: set(v) for k, v in i["monomers"].items()},
            enzrxns={
                k: {k2: {k3: float(v3) for k3, v3 in v2.items()} for k2, v2 in v.items()}
                for k, v in i["enzrxns"].items()
            },
