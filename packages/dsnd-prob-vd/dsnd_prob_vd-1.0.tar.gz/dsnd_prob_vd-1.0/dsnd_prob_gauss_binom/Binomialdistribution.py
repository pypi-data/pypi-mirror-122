...
class Binomial(Distribution):
    ...
    def plot_bar(self):
        ...
        x_ones = sum(self.data)
        x_zeros = len(self.data) - x_ones    
        plt.bar([0, 1], (x_ones, x_zeros))
        plt.title('BiNomial Distribution Barchart')
        plt.xlabel('value zero or one')
        plt.ylabel('count of results for each case')
        plt.xticks([0,1])
        plt.yticks(range(0, (x_ones+1)));    
