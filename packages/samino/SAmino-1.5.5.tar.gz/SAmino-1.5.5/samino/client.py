...
class Client(Recall, Socket):
    ...
    def get_user_info(self, userId: str):
        req = requests.get(f"{self._api}/g/s/user-profile/{userId}",
                           headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
    ...
    def invite_to_voice_chat(self, userId: str = None, chatId: str = None):
        ...
        req = requests.post(f'{self.api}/g/s/chat/thread/{chatId}/vvchat-presenter/invite', headers=self._headers,
                            data=data)
        if req.status_code != 200: return CheckExceptions(req.json())
        return Json(req.json())
    ...
    def get_chat_members(self, start: int = 0, size: int = 25, chatId: str = None):
        req = requests.get(f"{self._api}/g/s/chat/thread/{chatId}/member?start={start}&size={size}&type=default&cv=1.2",
                           headers=self.headers)
        if req.status_code != 200: return CheckExceptions(req.json())
    ...
    def register(self, nickname: str, email: str, password: str, verifyCode: str = None, deviceId: str = None):
        ...
        req = requests.post(f"{self.api}/g/s/auth/register", headers=self._headers, data=data)
        if req.status_code != 200: return CheckExceptions(req.json())
        return Json(req.json())
