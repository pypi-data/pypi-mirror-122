import inspect
from typing import List, Dict, Optional, Union, Iterable, Type, Any

from pandas import DataFrame, isna, Series, merge
...
def _infer_datatype(pd_dtype: Type) -> SqlValType:
    """Map the type of a python object to its corresponding SQL column type.

    Args:
        pd_dtype (Any): the input value to infer the type of.
...
class PandasProvider(BaseProvider):
    ...
    def __init__(
        ...
        Args:
            df (DataFrame): the dataframe to serve data from.
        ...
        cols = [ColumnMeta(c, _infer_datatype(t)) for c, t in zip(self.df.columns, self.df.dtypes)]
        params = [ParamMeta(
            "UsePandasFilter",
            SqlValType.Boolean,
        ...
        super().__init__(name, cols, params, description=description)
    ...
    def _restriction_table(self, restriction_dict: Dict[str, List[Any]]):
        ...
        res_df = DataFrame(restriction_dict)
