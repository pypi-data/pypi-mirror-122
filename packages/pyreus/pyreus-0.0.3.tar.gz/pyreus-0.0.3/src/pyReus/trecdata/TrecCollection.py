...
class TrecCollection:
    ...
    def _import_paths(self, collectionName):
        ...
        conf_path = configuration().get_configs()['PATHS']['COLLECTIONS_CONFIG']
        # load the internal json file containing the position of the principal collections
        cpaths = json.load(open(conf_path, "r"))

        if collectionName not in cpaths:
            raise ValueError(
                f"collection {collectionName} not available in configuration file \"{conf_path}\".")
