...
def split_page_and_run_textract(textract_client, page_image: np.ndarray, split_ratio: float = 0.6):
...
def run_textract(textract_client, image_bytes):
...
def run_newspaper_segmentation_on_image(textract_client, image_bytes, api_key):
...
def run_newspaper_segmentation(textract_client, image_path, api_key):
