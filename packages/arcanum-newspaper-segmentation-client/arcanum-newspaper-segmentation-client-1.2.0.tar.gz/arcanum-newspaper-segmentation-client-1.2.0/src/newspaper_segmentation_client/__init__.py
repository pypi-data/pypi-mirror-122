...
def run_textract_on_image(textract_client, page_image):
    ...
    page_image.save(image_bytes_io, format='JPG')
...
def split_page_and_run_textract(textract_client, page_image: Image, split_ratio: float = 0.6):
    ...
    bottom_half = page_image.crop(0, split_height, width, height)
...
def visualize_textract_output(image, textract_output):
    ...
    for block in textract_output["Blocks"]:
        ...
        new_image = draw.rectangle((int(left), int(top)), (int(right), int(bottom)), outline=(0, 255, 0), width=3)
    return new_image
...
def run_newspaper_segmentation_on_image(textract_client, image_bytes: bytes, api_key: str):
...
def run_newspaper_segmentation(textract_client, image_path: str, api_key: str):
    with open(image_path, "rb") as input_image:
        image_bytes = input_image.read()

    return run_newspaper_segmentation_on_image(textract_client, image_bytes, api_key)
