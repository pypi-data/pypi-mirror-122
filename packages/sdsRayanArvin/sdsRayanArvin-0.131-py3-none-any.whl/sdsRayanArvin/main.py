...
class Factory:
    ...
    def __API(self, api='Old', Url=None):
        ...
        if api == 'Old':
            request = Request('http://samacontrol.ir/sama/api/')

        if api == 'Data':
            request = Request('http://samacontrol.ir/device_data_managment/')

        if api == 'Workflow':
            request = Request('http://samacontrol.ir/workflow-service/')

        if api == 'Rule':
            request = Request('http://samacontrol.ir/rule-service/')

        if api == 'Weather':
            request = Request('http://samacontrol.ir/weather/')

        if api == 'RuleExe':
            request = Request('http://samacontrol.ir/rule-exe/')

        if api == 'WorkflowsExe':
            request = Request('http://samacontrol.ir/workflow-exe/')
