...
def run_batch(
    ...
    fv_or_fp = FeaturePackageOrView.from_fv(fv_proto)
    ...
    if aggregate_tiles is True:
        materialized_spark_df = construct_partial_time_aggregation_df(
            ...
            fv_or_fp.trailing_time_window_aggregation,
            fv_proto.feature_store_format_version,
            anchor_column_name="window_start_time",
        )

    # Filter output rows which are not withing feature time range.
    materialized_spark_df = materialized_spark_df.filter(
        materialized_spark_df[fv_or_fp.timestamp_key] >= feature_start_time
    )
    materialized_spark_df = materialized_spark_df.filter(
        materialized_spark_df[fv_or_fp.timestamp_key] < feature_end_time
    )
