...
def combine_videos(path=None):
    ...
    df = pd.DataFrame(columns=['fname', 'start', 'stop'])
    i = 0
    for file in sorted(glob.glob(filePath)):
        ...
        print(fname,time_created_str,start_time,stop_time)
        df.loc[i] = [fname, start_time, stop_time]
        i = i + 1
    ...
    start_datetime = df.loc[0, 'start'].to_pydatetime().strftime("%Y-%m-%d_%H-%M-%S")
    stop_datetime = df.loc[num - 1, 'stop'].to_pydatetime().strftime("%Y-%m-%d_%H-%M-%S")
    concatenated_file = os.path.join(tmp_fold, start_datetime + '_TO_' + stop_datetime + '.MP4')

    st_time_str = df.loc[num - 1, 'stop'].to_pydatetime().strftime("%Y-%m-%dT%H:%M:%S.%fZ")
    print(st_time_str)
