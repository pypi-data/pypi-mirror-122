...
class BusinessLogic:
    ...
    def create_calculated_measure(self, new_name: str, formula: str) -> Dict:
        ...
        Правила написания формул:
            1. При объявлении фунции между именем функции и открывающей скобкой не должно быть пробелов. Таким образом
            ...
            3. Формат задания фактов с учётом верхних размерностей: [Апрель,Payment], где до запятой указывается
                название верхней размерности, а после - название факта.
            4. В случае, если в мультисфере вынесено несколько верхних размерностей, то в выражении (формуле) участвует
                только самая верхняя из них.
            5. В качестве разделителя параметров в методах "top" и "corr" необходимо использовать точку с запятой.
                Например: "top([Сумма];100)", "corr([Март,Payment];[Апрель,Commission])".
            6. Все использующиеся операнды, круглые скобки, числа, факты и функции должны быть разделены пробелами.
                Например:
                    "100 + [Сумма]",
                    "corr([Сумма];[Выручка]) - top([Сумма];100)",
                    "20 + total([Выручка]) * ( [Май,Сумма] + [Май,Выручка] ) / 3"
        ...
        :param new_name: (str) название создаваемого вычислимого факта; обязательны буквы, допустимы символы;
            нельзя использовать слова "top", "total", "corr", "not", "and", "or".
        ...
        def get_measure_form(measure_content: str) -> str:
            ...
            # отличаем эти два кейса по запятой внутри квадратной скобки
            if ',' in measure_content:
        ...
        try:
            for i in formula_lst:
                if (i in OPERANDS) or (i in LOGIC_FUNCS) or (i in ("(", ")")) or self._is_numeric(i):
                    ...
                    if ',' in measure:
                        measure = '[{}'.format(measure.split(',')[1])
    ...
    def run_scenario_by_user(self, scenario_name: str, user_name: str, units: int = 500, timeout: int = None):
        ...
        sc = BusinessLogic(login=user_name, url=self.base_url)
    ...
    def clone_olap_module(self, module: str = None) -> Union[str, str]:
        """
        Создать копию указанного OLAP-модуля. Если модуль не указан, то копируется текущий OLAP-модуль.
    ...
    def _create_multisphere_module(self, num_row: int = 10000, num_col: int = 100) -> Dict:
        """
    ...
    def rename_grouped_elems(self, name: str, new_name: str) -> Dict:
        ...
        :param name: старое название группы элементов.
