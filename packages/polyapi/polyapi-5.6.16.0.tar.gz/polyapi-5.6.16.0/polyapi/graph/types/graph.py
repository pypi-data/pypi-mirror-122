...
class Graph(BaseGraph):
    ...
    def _get_other_settings(self) -> dict:
        ...
        other_settings.update({
            'node_colors': {'first': first_node_color.lower(), 'second': second_node_color.lower()},
        ...
        # если проверки все пройдены - добавляем в словарь
        other_settings.update({
            'edge_colors': {'first': first_edge_color.lower(), 'second': second_edge_color.lower()},
    ...
    def draw(self):
        ...
        graph_config['plotData'][self.graph_type]['config'].update({
            'base': base_setting,
            'edges': edges_setting,
            # TODO: не удалось определить, на что влияет этот параметр и для чего он нужен,
            # но с интерфейса он отправляется для любой конфигурации графика (и всегда в таком виде); нужно разобраться
            'order': [0, 1, 2, 3]
        })
