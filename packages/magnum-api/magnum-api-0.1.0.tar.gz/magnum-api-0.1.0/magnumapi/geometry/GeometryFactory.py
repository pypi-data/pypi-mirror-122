...
class GeometryFactory:
    ...
    @classmethod
    def init_with_dict(cls, block_dct_defs: List[Dict], layer_dct_defs: List[Dict], cadata: CableDatabase) -> Geometry:
        ...
        if geom_type == GeometryType.ABSOLUTE:
            return cls.init_absolute_with_dict(block_dct_defs, layer_defs, cadata)

        if geom_type == GeometryType.RELATIVE:
            return cls.init_relative_with_dict(block_dct_defs, layer_defs, cadata)
    ...
    @staticmethod
    def init_absolute_with_dict(block_defs: List[Dict],
    ...
    @staticmethod
    def init_relative_with_dict(block_dct_defs: List[Dict],
                                layer_defs: List[LayerDefinition],
                                cadata: CableDatabase) -> Geometry:
    ...
    @classmethod
    def init_with_csv(cls, block_csv_file_path: str, layer_csv_file_path: str, cadata: CableDatabase) -> Geometry:
    ...
    def init_with_df(cls, block_df: pd.DataFrame, layer_df, cadata: CableDatabase) -> Geometry:
        ...
        if geom_type == GeometryType.ABSOLUTE:
            return cls.init_absolute_with_df(block_df, layer_defs, cadata)

        if geom_type == GeometryType.RELATIVE:
            return cls.init_relative_with_df(block_df, layer_defs, cadata)
    ...
    @staticmethod
    def init_absolute_with_df(block_df: pd.DataFrame,
                              layer_defs: List[LayerDefinition],
                              cadata: CableDatabase) -> Geometry:
        ...
        return Geometry(blocks, layer_defs)

    @staticmethod
    def init_relative_with_df(block_df: pd.DataFrame,
                              layer_defs: List[LayerDefinition],
                              cadata: CableDatabase) -> Geometry:
