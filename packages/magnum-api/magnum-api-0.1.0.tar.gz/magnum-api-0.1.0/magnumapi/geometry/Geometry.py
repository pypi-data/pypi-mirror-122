...
class Geometry(ABC):
    ...
    def __init__(self, blocks: List[Block], layer_defs: List[LayerDefinition]) -> None:
    ...
    def to_rel_geometry(self) -> "RelativeCosThetaGeometry":
        ...
        geometry_rel = RelativeCosThetaGeometry(blocks=blocks, layer_defs=geometry.layer_defs)
...
class RelativeCosThetaGeometry(Geometry):
    ...
    def to_abs_geometry(self) -> "Geometry":
        ...
        geometry_abs = Geometry(blocks=blocks, layer_defs=deepcopy(self.layer_defs))
        # Return to the external context of the geometry before entering
...
class HomogenizedCosThetaGeometry(Geometry):
    ...
    def with_cos_theta_geometry(cls, geometry: "Geometry") -> "HomogenizedCosThetaGeometry":
        ...
        """
        if not all([isinstance(block, CosThetaBlock) for block in geometry.blocks]):
        ...
        return HomogenizedCosThetaGeometry(blocks=blocks, layer_defs=geometry.layer_defs)
