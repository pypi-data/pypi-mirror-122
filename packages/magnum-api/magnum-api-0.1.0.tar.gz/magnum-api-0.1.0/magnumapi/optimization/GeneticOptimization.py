...
class RoxieGeneticOptimization(GeneticOptimization):
    ...
    def update_model_parameters(self, decoded_chromosome: Dict) -> Dict:
        ...
        blocks_def_update = geometry_model.to_rel_geometry().to_dict()
        for block_variable, value in decoded_chromosome.items():
            if ':' in block_variable:
                block, variable = block_variable.split(':')
                block = int(block)
                blocks_def_update['block_defs'][block - 1][variable] = value

        # Remove empty blocks
        # ToDo: Update layer definition as well
        block_defs = blocks_def_update['block_defs']
        blocks_def_update['block_defs'] = [block for block in block_defs if block['nco'] > 0]
