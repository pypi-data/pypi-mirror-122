...
copyright FEMTO ENGINEERING, 2020-2021
license   GPL v3.0+
...
class Ad9915Dev():
    ...
    def set_reg_w16(self, address, values):
        ...
        while request <= MAX_REQUEST:
            ...
            reg_value = list(self.get_reg_w16(address))
            if reg_value == values:
                return
            else:
    ...
    def set_reg(self, address, value):
        """Set register value
        :param address: register address (int)
        :param value: register value (int)
        :returns: None
        """
        values = []
    ...
    def set_reg_bit(self, address, idx, value):
        ...
        if reg_value & (2 << idx) == value:
            return
        self.set_reg(address, reg_value ^ (1 << idx))
    ...
    def set_ofreq_fine_state(self, state):
        """Configure DDS to (not) use fine tuning of output frequency.
        :param state: True to enable fine tuning else False (bool)
        :returns: None
        """
        if state is True:
            # Enable the programmable modulus enable bit
            self.set_reg_bit(REGNAME2ADDR['CFR2'], 16, 1)
        else:
            self.set_reg_bit(REGNAME2ADDR['CFR2'], 16, 0)

    def set_ofreq_fine(self, ifreq, ofreq):
        """Set output frequency using fine tuning capabilities of DDS.
        ...
        :param ofreq: Output frequency value (str).
        :returns: Output frequency (float)
        """
        # Compute FTW, A and B parameters
        ftw, a, b = compute_modulus_parameters(str(ifreq),
                                               str(ofreq))
        # Set FTW, A and B
